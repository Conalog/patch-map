...
export const CORE_V2_VIEWPORT_RUNTIME_REVISION = 'core-v2-viewport-runtime/1';
export const CORE_V2_VIEWPORT_CLEANUP_REVISION = 'core-v2-viewport-cleanup/1';

export const CORE_V2_VIEWPORT_CASE_IDS = Object.freeze([
  'VIE-001',
  'VIE-002',
  'VIE-003',
  'VIE-004',
...
export type CoreV2ViewportCaseId = (typeof CORE_V2_VIEWPORT_CASE_IDS)[number];
...
interface ProductResourceProbeRequest {
  readonly caseId: CoreV2ViewportCaseId;
...
export interface CoreV2ViewportProductAdapter {
...
export interface CoreV2ViewportRuntime {
  readonly product: CoreV2ViewportProductAdapter;
 ...
 */
export function createCoreV2ViewportRuntime(
  caseId: CoreV2ViewportCaseId,
): CoreV2ViewportRuntime {
  ...
  const product: CoreV2ViewportProductAdapter = Object.freeze({
    ...
    resourceProbe(inputValue: ProductResourceProbeRequest) {
      ...
      return deepFreeze({
        revision: CORE_V2_VIEWPORT_RUNTIME_REVISION,
        caseId,
        engine: {
          snapshot,
          semantic,
  ...
  return Object.freeze({
    ...
    postDestroyProductProbe(): Readonly<Record<string, unknown>> {
      ...
      cleanupProbe = deepFreeze({
        revision: CORE_V2_VIEWPORT_CLEANUP_REVISION,
        caseId,
...
function productTaskMeasureRequest(value: unknown): ProductTaskMeasureRequest {
  ...
  invariant(
    CORE_V2_VIEWPORT_CASE_IDS.includes(input.caseId as CoreV2ViewportCaseId),
  ...
  return Object.freeze({
    caseId: input.caseId as CoreV2ViewportCaseId,
...
function requireCaseId(value: unknown): CoreV2ViewportCaseId {
  invariant(
    typeof value === 'string'
      && CORE_V2_VIEWPORT_CASE_IDS.includes(value as CoreV2ViewportCaseId),
    'unsupported case identity',
  );
  return value as CoreV2ViewportCaseId;
}
...
function invariant(condition: boolean, message: string): asserts condition {
  if (!condition) throw new Error(`Invalid Core v2 viewport runtime: ${message}`);
}
