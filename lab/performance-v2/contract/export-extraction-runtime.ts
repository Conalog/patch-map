import {
  ...
  type CoreV2Engine,
  type CoreV2EngineExtractionRequest,
  type CoreV2EngineExtractionResult,
  type CoreV2PublishedTuple,
} from '../../../src/core-v2';

export const CORE_V2_EXPORT_EXTRACTION_RUNTIME_REVISION =
  'core-v2-export-extraction-runtime/1';
export const CORE_V2_EXPORT_EXTRACTION_CLEANUP_REVISION =
  'core-v2-export-extraction-cleanup/1';

export const CORE_V2_EXPORT_EXTRACTION_CASE_IDS = Object.freeze([
  'DET-004',
  'PIX-004',
  'PRF-008',
  'CSM-035',
...
export type CoreV2ExportExtractionCaseId =
  (typeof CORE_V2_EXPORT_EXTRACTION_CASE_IDS)[number];

interface ExtractRequest {
  readonly caseId: CoreV2ExportExtractionCaseId;
  readonly engine: CoreV2Engine;
  readonly request: CoreV2EngineExtractionRequest;
...
export interface CoreV2ExportExtractionProductAdapter {
  ...
  probeHostImageFailure(engine: CoreV2Engine): Promise<Readonly<Record<string, unknown>>>;
  resourceProbe(): Readonly<Record<string, unknown>>;
}

export interface CoreV2ExportExtractionRuntime {
  readonly product: CoreV2ExportExtractionProductAdapter;
 ...
 */
export function createCoreV2ExportExtractionRuntime(
  caseId: CoreV2ExportExtractionCaseId,
): CoreV2ExportExtractionRuntime {
  ...
  function trackCanvas(engine: CoreV2Engine): ReturnType<CoreV2Engine['canvasHandle']> {
  ...
  async function restoreCanvas(engine: CoreV2Engine): Promise<void> {
  ...
  const product: CoreV2ExportExtractionProductAdapter = Object.freeze({
    observeEngine(engine: CoreV2Engine) {
      assertActive(released, 'observeEngine');
      const canvas = trackCanvas(engine);
      return deepFreeze({
        revision: CORE_V2_EXPORT_EXTRACTION_RUNTIME_REVISION,
        caseId,
    ...
    async probeHostImageFailure(engine: CoreV2Engine) {
  ...
  return Object.freeze({
    ...
    postDestroyProductProbe() {
      ...
      cleanupProbe = deepFreeze({
        revision: CORE_V2_EXPORT_EXTRACTION_CLEANUP_REVISION,
        caseId,
...
function retainExtraction(result: CoreV2EngineExtractionResult): RetainedExtraction {
...
function validateCanonicalDataset(dataset: unknown): Readonly<Record<string, unknown>> {
  ...
  try {
    const materialized = materializeCoreV2Dataset(dataset);
    return deepFreeze({
      rootKind: 'array',
      schemaValid: true,
...
function requireCaseId(caseId: CoreV2ExportExtractionCaseId): void {
  invariant(CORE_V2_EXPORT_EXTRACTION_CASE_IDS.includes(caseId), 'unsupported case identity');
}
...
function invariant(condition: unknown, message: string): asserts condition {
  if (!condition) {
    throw new Error(`Invalid Core v2 export/extraction runtime: ${message}`);
  }
}
