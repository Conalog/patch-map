...
export const CORE_V2_REPLACEMENT_RECOVERY_RUNTIME_REVISION =
  'core-v2-replacement-recovery-runtime/1';
export const CORE_V2_REPLACEMENT_RECOVERY_CLEANUP_REVISION =
  'core-v2-replacement-recovery-cleanup/1';

export const CORE_V2_REPLACEMENT_RECOVERY_CASE_IDS = Object.freeze([
  'ERR-002',
  'ERR-005',
  'LIF-003',
  'CSM-002',
...
export type CoreV2ReplacementRecoveryCaseId =
  (typeof CORE_V2_REPLACEMENT_RECOVERY_CASE_IDS)[number];
...
export interface CoreV2ReplacementRecoveryProductAdapter {
...
export interface CoreV2ReplacementRecoveryRuntime {
  readonly product: CoreV2ReplacementRecoveryProductAdapter;
 ...
 */
export function createCoreV2ReplacementRecoveryRuntime(
  caseId: CoreV2ReplacementRecoveryCaseId,
): CoreV2ReplacementRecoveryRuntime {
  invariant(
    CORE_V2_REPLACEMENT_RECOVERY_CASE_IDS.includes(caseId),
  ...
  const product: CoreV2ReplacementRecoveryProductAdapter = Object.freeze({
    ...
    resourceProbe() {
      ...
      return deepFreeze({
        revision: CORE_V2_REPLACEMENT_RECOVERY_RUNTIME_REVISION,
        caseId,
        unmanagedOverlayCount: overlays.size,
        replacementCount,
        replacementCycleCount: Math.floor(replacementCount / 2),
  ...
  return Object.freeze({
    ...
    postDestroyProductProbe() {
      ...
      cleanupProbe = deepFreeze({
        revision: CORE_V2_REPLACEMENT_RECOVERY_CLEANUP_REVISION,
        caseId,
...
function invariant(condition: unknown, message: string): asserts condition {
  if (!condition) {
    throw new Error(`Invalid Core v2 replacement/recovery runtime: ${message}`);
  }
}
