...
export const CORE_V2_SECURITY_OPERATIONS_RUNTIME_REVISION =
  'core-v2-security-operations-runtime/1' as const;
export const CORE_V2_SECURITY_OPERATIONS_CLEANUP_REVISION =
  'core-v2-security-operations-cleanup/1' as const;

export const CORE_V2_SECURITY_OPERATIONS_CASE_IDS = Object.freeze([
  'SEC-002',
  'SEC-003',
  'SEC-004',
  'OPS-001',
...
export type CoreV2SecurityOperationsCaseId =
  (typeof CORE_V2_SECURITY_OPERATIONS_CASE_IDS)[number];

export interface CoreV2SecurityOperationsProductAdapter {
...
export interface CoreV2SecurityOperationsRuntime {
  readonly product: CoreV2SecurityOperationsProductAdapter;
 ...
 */
export function createCoreV2SecurityOperationsRuntime(
  caseId: CoreV2SecurityOperationsCaseId,
): CoreV2SecurityOperationsRuntime {
  requireCaseId(caseId);
  const logs: unknown[] = [];
  const observers: unknown[] = [];
  const telemetry: unknown[] = [];
  const redactionOperations = new CoreV2OperationsAuthority({
  ...
  const subscriptions: CoreV2OperationalSubscription[] = [
  ...
  const product: CoreV2SecurityOperationsProductAdapter = Object.freeze({
    observeEngine(engine: CoreV2Engine): Readonly<Record<string, unknown>> {
      assertActive(released, 'observeEngine');
      return deepFreeze({
        runtimeRevision: CORE_V2_SECURITY_OPERATIONS_RUNTIME_REVISION,
  ...
  return Object.freeze({
    ...
    postDestroyProductProbe(): Readonly<Record<string, unknown>> {
      ...
      telemetry.splice(0);
      return deepFreeze({
        revision: CORE_V2_SECURITY_OPERATIONS_CLEANUP_REVISION,
        caseId,
...
function requireCaseId(value: string): asserts value is CoreV2SecurityOperationsCaseId {
  invariant(
    (CORE_V2_SECURITY_OPERATIONS_CASE_IDS as readonly string[]).includes(value),
...
function invariant(condition: boolean, message: string): asserts condition {
  if (!condition) throw new Error(`Invalid Core v2 security/operations runtime: ${message}`);
}
