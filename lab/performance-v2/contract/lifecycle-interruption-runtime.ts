...
export const CORE_V2_LIFECYCLE_INTERRUPTION_RUNTIME_REVISION =
  'core-v2-lifecycle-interruption-runtime/1';
export const CORE_V2_LIFECYCLE_INTERRUPTION_CLEANUP_REVISION =
  'core-v2-lifecycle-interruption-cleanup/1';

export const CORE_V2_LIFECYCLE_INTERRUPTION_CASE_IDS = Object.freeze([
  'ERR-004',
  'ERR-006',
  'PRF-007',
  'CSM-017',
...
export type CoreV2LifecycleInterruptionCaseId =
  (typeof CORE_V2_LIFECYCLE_INTERRUPTION_CASE_IDS)[number];

interface ProductionDatasetRequest {
  readonly caseId: CoreV2LifecycleInterruptionCaseId;
...
export interface CoreV2LifecycleInterruptionProductAdapter {
  productionDataset(
...
export interface CoreV2LifecycleInterruptionRuntime {
  readonly product: CoreV2LifecycleInterruptionProductAdapter;
 ...
 */
export function createCoreV2LifecycleInterruptionRuntime(
  caseId: CoreV2LifecycleInterruptionCaseId,
): CoreV2LifecycleInterruptionRuntime {
  ...
  const product: CoreV2LifecycleInterruptionProductAdapter = Object.freeze({
    ...
    resourceProbe(input: ResourceProbeRequest) {
      ...
      return deepFreeze({
        revision: CORE_V2_LIFECYCLE_INTERRUPTION_RUNTIME_REVISION,
        caseId,
        label: input.label,
  ...
  return Object.freeze({
    ...
    postDestroyProductProbe(): Promise<Readonly<Record<string, unknown>>> {
      ...
      cleanupPromise = (async () => {
        ...
        return deepFreeze({
          revision: CORE_V2_LIFECYCLE_INTERRUPTION_CLEANUP_REVISION,
          caseId,
...
function requireCaseId(
  ...
  invariant(
    CORE_V2_LIFECYCLE_INTERRUPTION_CASE_IDS.includes(caseId),
...
function invariant(condition: unknown, message: string): asserts condition {
  if (!condition) {
    throw new Error(`Invalid Core v2 lifecycle/interruption runtime: ${message}`);
  }
}
