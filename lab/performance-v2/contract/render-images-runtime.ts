...
const FIXTURE_REGISTRATIONS: readonly CoreV2AssetRegistration[] = Object.freeze([
  Object.freeze({
    alias: FIXTURE_ALIAS,
...
export interface CoreV2RenderImagesProductAdapter {
...
export interface CoreV2RenderImagesRuntime {
  readonly assetRuntime: CoreV2AssetRuntime;
  readonly assetPolicy: CoreV2AssetPolicy;
  readonly product: CoreV2RenderImagesProductAdapter;
 ...
 */
export function createCoreV2RenderImagesRuntime(): CoreV2RenderImagesRuntime {
  const backend = new RenderImagesFixtureBackend();
  const assetRuntime = new CoreV2AssetRuntime(backend);
  const assetPolicy: CoreV2AssetPolicy = (context) => validateFixturePolicy(context);
...
class RenderImagesFixtureBackend implements CoreV2AssetBackend {
  ...
  private ensureRecord(request: CoreV2AssetBackendRequest): FixtureRequestRecord {
...
): CoreV2RenderImagesProductAdapter {
...
function validateFixturePolicy(context: CoreV2AssetPolicyContext): void {
...
function describeFixture(descriptor: CoreV2AssetDescriptor): FixtureDescription {
...
function isResolutionTwoDescriptor(descriptor: CoreV2AssetDescriptor): boolean {
...
async function pollEngine(
  ...
  throw new Error(`Core v2 REN-005 runtime timed out waiting for ${label}`);
}
...
function invariant(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(`Core v2 REN-005 runtime invalid: ${message}`);
}
