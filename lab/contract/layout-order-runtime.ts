...
export const PATCH_MAP_LAYOUT_ORDER_EXTENSION_CASE_IDS = Object.freeze([] as const);

type LayoutOrderCaseId = (typeof PATCH_MAP_LAYOUT_ORDER_ACTIVE_CASE_IDS)[number];
...
export interface PatchMapLayoutOrderProductAdapter {
...
export interface PatchMapLayoutOrderRuntime {
  readonly product: PatchMapLayoutOrderProductAdapter;
 ...
 */
export function createPatchMapLayoutOrderRuntime(
  caseId: LayoutOrderCaseId,
): PatchMapLayoutOrderRuntime {
  ...
  const product: PatchMapLayoutOrderProductAdapter = Object.freeze({
...
function invariant(condition: boolean, message: string): asserts condition {
  if (!condition) throw new Error(`Invalid PatchMap layout-order runtime: ${message}`);
}
