...
type PresentationDynamicsCaseId =
  (typeof PATCH_MAP_PRESENTATION_DYNAMICS_CASE_IDS)[number];
...
export interface PatchMapPresentationDynamicsProductAdapter {
...
export interface PatchMapPresentationDynamicsRuntime {
  readonly product: PatchMapPresentationDynamicsProductAdapter;
...
export function createPatchMapPresentationDynamicsRuntime(
  caseId: PresentationDynamicsCaseId,
): PatchMapPresentationDynamicsRuntime {
  invariant(
    PATCH_MAP_PRESENTATION_DYNAMICS_CASE_IDS.includes(caseId),
  ...
  const product: PatchMapPresentationDynamicsProductAdapter = Object.freeze({
    recordSchedule(input: Readonly<{
      caseId: 'ANI-002';
      scheduleIndex: number;
      values: readonly number[];
    ...
    resourceProbe(input: Readonly<{
      ...
      return deepFreeze({
        revision: PATCH_MAP_PRESENTATION_DYNAMICS_RUNTIME_REVISION,
        caseId,
  ...
  return Object.freeze({
    ...
    postDestroyProductProbe(): Readonly<Record<string, unknown>> {
      ...
      cleanupProbe = deepFreeze({
        revision: PATCH_MAP_PRESENTATION_DYNAMICS_CLEANUP_REVISION,
        caseId,
...
function invariant(condition: boolean, message: string): asserts condition {
  if (!condition) throw new Error(`Invalid PatchMap presentation dynamics runtime: ${message}`);
}
