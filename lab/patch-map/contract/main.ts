...
interface PatchMapContractUiRunMetrics {
...
function renderRunObserver(prefix: PatchMapRunObserverPrefix): string {
...
function renderComponentAssetInspector(route: PatchMapContractRoute): string {
...
type PatchMapRunObserverPrefix =
  | 'ren-005'
  | 'ren-006'
  | 'ren-008'
  | 'ren-010'
...
function runObserverPrefix(scenario: string): PatchMapRunObserverPrefix | null {
...
function resetRunPerformance(root: HTMLElement, prefix: PatchMapRunObserverPrefix): void {
  const observer = root.querySelector<HTMLElement>(`[data-testid="${prefix}-run-observation"]`);
  if (!observer) return;
  for (const suffix of [
    'run-index',
