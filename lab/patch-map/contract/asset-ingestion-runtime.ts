...
export interface PatchMapAssetIngestionProductAdapter {
...
export interface PatchMapAssetIngestionRuntime {
  readonly assetRuntime: PatchMapAssetRuntime;
  readonly assetPolicy: PatchMapAssetPolicy;
  readonly product: PatchMapAssetIngestionProductAdapter;
  postDestroyProductProbe(): Promise<Readonly<Record<string, unknown>>>;
}

export function createPatchMapAssetIngestionRuntime(): PatchMapAssetIngestionRuntime {
  ...
  const descriptorSession = assetRuntime.createSession({
    instanceId: 'patch-map-asset-descriptors',
    policy: fixtureAssetPolicy,
  });
  const descriptorAcquisitions: PatchMapAssetAcquisition[] = [];
...
class AssetIngestionFixtureBackend implements PatchMapAssetBackend {
  public readonly keyNamespace = 'patch-map-asset-ingestion-fixture/1';
...
function createProductAdapter(
  ...
  descriptorSession: PatchMapAssetSession,
  descriptorAcquisitions: PatchMapAssetAcquisition[],
  raceRequests: Map<string, RaceRequest>,
): PatchMapAssetIngestionProductAdapter {
  return Object.freeze({
    ...
    validateDescriptor(inputValue: unknown): Readonly<Record<string, unknown>> {
      ...
      try {
        normalizePatchMapAssetDescriptor(input.descriptor as never);
      } catch (error) {
        code = error instanceof PatchMapAssetError ? error.code : 'INTERNAL_FAILURE';
      }
    ...
    runSecurityMatrix(inputValue: unknown): Readonly<Record<string, unknown>> {
      ...
      return runSecurityMatrix(
        input.policy as PatchMapAssetIngestionPolicyProfile,
    ...
    ingestHostAsset(
      ...
      const result = engine.ingestHostAsset(input.payload as PatchMapHostAssetIngestionInput);
...
function runSecurityMatrix(
  ...
  const preflight = createPatchMapAssetIngestionPolicy(profile);
  ...
  for (const caseId of caseIds) {
    ...
    try {
      ...
      invariant(
        error instanceof PatchMapAssetError && error.code === 'ASSET_POLICY_REJECTED',
    ...
    const response = evaluatePatchMapAssetResponsePolicy(profile, candidate);
...
function descriptorArray(value: unknown): readonly Readonly<{
  ...
  return Object.freeze(value.map((entry, index) => {
    ...
    const normalized = normalizePatchMapAssetDescriptor({
...
function descriptorResourceIdentity(descriptor: PatchMapAssetDescriptor): string {
  return `decoded:${stableSerialize(descriptor)}`;
}

function fixtureAssetPolicy(context: PatchMapAssetPolicyContext): void {
  if (
...
function projectRuntime(runtime: PatchMapAssetRuntime): Readonly<Record<string, unknown>> {
...
function invariant(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(`PatchMap asset ingestion runtime: ${message}`);
}
