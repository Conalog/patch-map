...
export interface PatchMapExecutableRuntimeDescriptor {
  ...
  createRun(plan: PatchMapExecutableCasePlan): Readonly<{
  ...
  handlerEntries(plan: PatchMapExecutableCasePlan): readonly HandlerEntry[];
...
type PatchMapRuntimeFoldInput = Parameters<PatchMapExecutableRuntimeDescriptor['fold']>[0];
...
const migrationHandlers =
  migrationHandlersModule as unknown as HandlerFactoryRuntime;
...
const migrationFold =
  migrationFoldModule as unknown as FoldRuntime;
...
const INTERACTION_EDITOR_CASE_IDS = new Set<PatchMapInteractionEditorCaseId>(
  PATCH_MAP_INTERACTION_EDITOR_CASE_IDS,
);
const AUTHORING_CASE_IDS = new Set<PatchMapAuthoringCaseId>(
  PATCH_MAP_AUTHORING_CASE_IDS,
);
const EDITOR_WORKFLOW_CASE_IDS = new Set<PatchMapEditorWorkflowCaseId>(
  PATCH_MAP_EDITOR_WORKFLOW_CASE_IDS,
);
const HISTORY_CASE_IDS = new Set<PatchMapHistoryCaseId>(
  PATCH_MAP_HISTORY_CASE_IDS,
);
const REPLACEMENT_RECOVERY_CASE_IDS = new Set<PatchMapReplacementRecoveryCaseId>(
  PATCH_MAP_REPLACEMENT_RECOVERY_CASE_IDS,
);
const LIFECYCLE_INTERRUPTION_CASE_IDS = new Set<PatchMapLifecycleInterruptionCaseId>(
  PATCH_MAP_LIFECYCLE_INTERRUPTION_CASE_IDS,
);
const DETERMINISM_LIFECYCLE_CASE_IDS =
  new Set<PatchMapDeterminismLifecycleCaseId>(
    PATCH_MAP_DETERMINISM_LIFECYCLE_CASE_IDS,
  );
const EXPORT_EXTRACTION_CASE_IDS = new Set<PatchMapExportExtractionCaseId>(
  PATCH_MAP_EXPORT_EXTRACTION_CASE_IDS,
);
...
const PACKAGE_INTEGRATION_CASE_IDS = new Set<PatchMapPackageIntegrationCaseId>(
  PATCH_MAP_PACKAGE_INTEGRATION_CASE_IDS,
);
const PERFORMANCE_CASE_IDS = new Set<PatchMapPerformanceCaseId>(
  PATCH_MAP_PERFORMANCE_CASE_IDS,
);
...
const SECURITY_OPERATIONS_CASE_IDS = new Set<PatchMapSecurityOperationsCaseId>(
  PATCH_MAP_SECURITY_OPERATIONS_CASE_IDS,
);
const ACCESSIBILITY_CASE_IDS = new Set<PatchMapAccessibilityCaseId>(
  PATCH_MAP_ACCESSIBILITY_CASE_IDS,
);
const MIGRATION_CASE_IDS = new Set<PatchMapMigrationCaseId>(
  PATCH_MAP_MIGRATION_CASE_IDS,
);
...
export function resolvePatchMapExecutableRuntime(
  ...
  if (isMigrationCaseId(caseId)) return MIGRATION_DESCRIPTOR;
  ...
  throw new Error(`Unsupported PatchMap executable runtime: ${String(caseId)}`);
}
...
function isExportExtractionCaseId(
  ...
  return EXPORT_EXTRACTION_CASE_IDS.has(
    caseId as PatchMapExportExtractionCaseId,
  );
}
...
function isPackageIntegrationCaseId(
  ...
  return PACKAGE_INTEGRATION_CASE_IDS.has(
    caseId as PatchMapPackageIntegrationCaseId,
  );
}
...
function isInteractionEditorCaseId(
  ...
  return INTERACTION_EDITOR_CASE_IDS.has(
    caseId as PatchMapInteractionEditorCaseId,
  );
}
...
const AST_DEVICE_SOURCE = 'patch-map-builtin://images/device.svg';
...
function requireFold(
  value: ((options: Readonly<Record<string, unknown>>) => PatchMapFoldedExecution) | undefined,
  label: string,
): (options: Readonly<Record<string, unknown>>) => PatchMapFoldedExecution {
  invariant(typeof value === 'function', `${label} export`);
  return value;
}
