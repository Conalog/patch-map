...
export type PatchMapInteractionEditorCaseId =
  (typeof PATCH_MAP_INTERACTION_EDITOR_CASE_IDS)[number];

export interface PatchMapInteractionEditorProductAdapter {
  productionDataset(input: Readonly<{
    readonly caseId: 'CSM-018';
    readonly generatorRef: 'production-shaped';
  }>): readonly Readonly<Record<string, unknown>>[];
  resolveEditorMount(blockedPlant: boolean): PatchMapEditorMountDecision;
  ...
  resourceProbe(input: Readonly<{
    readonly caseId: PatchMapInteractionEditorCaseId;
    readonly engine: PatchMap;
  }>): Readonly<Record<string, unknown>>;
}

export interface PatchMapInteractionEditorRuntime {
  readonly product: PatchMapInteractionEditorProductAdapter;
 ...
 */
export function createPatchMapInteractionEditorRuntime(
  caseId: PatchMapInteractionEditorCaseId,
): PatchMapInteractionEditorRuntime {
  ...
  const product: PatchMapInteractionEditorProductAdapter = Object.freeze({
    productionDataset(input: Readonly<{
      readonly caseId: 'CSM-018';
      readonly generatorRef: 'production-shaped';
    }>) {
    ...
    resolveEditorMount(blockedPlant: boolean) {
      ...
      return resolvePatchMapEditorMount(blockedPlant);
    },
    ...
    resourceProbe(input: Readonly<{
      ...
      return deepFreeze({
        revision: PATCH_MAP_INTERACTION_EDITOR_RUNTIME_REVISION,
        caseId,
  ...
  return Object.freeze({
    ...
    postDestroyProductProbe(): Readonly<Record<string, unknown>> {
      ...
      tooltipHost.release();
      cleanup = deepFreeze({
        revision: PATCH_MAP_INTERACTION_EDITOR_CLEANUP_REVISION,
        caseId,
        datasetBuildCount,
        resourceProbeCount,
        tooltipHost: tooltipHost.probe(),
...
class TooltipHostLedger {
  private readonly caseId: PatchMapInteractionEditorCaseId;
  private subscription: PatchMapHostTooltipSubscription | null = null;
  private element: HTMLElement | null = null;
  private virtualVisible = false;
  private callbackCount = 0;
  private staleCallbackCount = 0;
  private released = false;
  private latestState: PatchMapHostTooltipState | null = null;

  public constructor(caseId: PatchMapInteractionEditorCaseId) {
    this.caseId = caseId;
  }

  public attach(engine: PatchMap): void {
  ...
  private consume(publication: PatchMapHostTooltipPublication): void {
    if (this.released) {
    ...
    element.dataset.patchMapTooltipHost = this.caseId;
...
function requireCaseId(value: unknown): PatchMapInteractionEditorCaseId {
  invariant(
    typeof value === 'string'
      && PATCH_MAP_INTERACTION_EDITOR_CASE_IDS.includes(
        value as PatchMapInteractionEditorCaseId,
      ),
    'unsupported case identity',
  );
  return value as PatchMapInteractionEditorCaseId;
}
