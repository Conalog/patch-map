import { PatchMapAssetError, PatchMap } from '../../src/patch-map';
import type {
  MainParityAction,
...
async function load(input: unknown): Promise<MainParityObservation> {
  ...
  // replaced PatchMap's intentional pending placeholder.
...
async function act(action: MainParityAction): Promise<MainParityObservation> {
  ...
  switch (action.type) {
    ...
    case 'browser-drag':
      ...
      break;
    default:
      throw new TypeError(`unsupported PatchMap parity action: ${String(action.type)}`);
  }
...
function requireEngine(operation: string): PatchMap {
  if (engine === null) throw new Error(`${operation} requires initialized PatchMap`);
  return engine;
}
...
function requireHost(): HTMLElement {
  ...
  if (!(element instanceof HTMLElement)) {
    throw new Error('missing PatchMap parity host');
  }
  return element;
}
