...
import { ZodValidationError } from '../src/model/validation';
...
describe('view target resolution and fitting', () => {
  ...
  it('normalizes only the documented axis padding forms', () => {
    ...
    expect(() => normalizeFitPadding({ top: 2 } as never)).toThrow(
      ZodValidationError,
    );
    expect(() => normalizeFitPadding(-1)).toThrow(TypeError);
  });

  it('computes a contain scale after expanding world bounds by padding', () => {
    expect(
      fitScaleFor(
