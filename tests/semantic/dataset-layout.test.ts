...
interface GridFixture {
  readonly id: string;
  readonly cells: readonly (readonly PatchMapGridCellValue[])[];
  readonly itemSize: readonly [number, number];
...
interface Dat005Params {
  ...
  readonly edgeMatrices: Readonly<{
    ...
    empty: readonly (readonly PatchMapGridCellValue[])[];
    duplicateLabels: readonly (readonly PatchMapGridCellValue[])[];
  }>;
}
...
describe('PatchMap DAT-003 semantic dimensions', () => {
  ...
  it('accepts only the bounded numeric, percentage, unit-object, and calc grammar', () => {
    expect(resolvePatchMapDimension(12, 200)).toBe(12);
    expect(resolvePatchMapDimension('25%', 200)).toBe(50);
    expect(resolvePatchMapDimension({ value: 25, unit: '%' }, 200)).toBe(50);
    expect(resolvePatchMapDimension({ value: 12, unit: 'px' }, 200)).toBe(12);
    expect(resolvePatchMapDimension('calc(50% + 12px)', 200)).toBe(112);
    expect(resolvePatchMapDimension('calc(12px + 50%)', 200)).toBe(112);

    expectInvalidValue(() => resolvePatchMapDimension('12', 200, '$.numeric-string'), '$.numeric-string');
    expectInvalidValue(() => resolvePatchMapDimension('12px', 200, '$.px-string'), '$.px-string');
    expectInvalidValue(
      () => resolvePatchMapDimension('calc(100% - 10px + 2px)', 200, '$.multi-calc'),
      '$.multi-calc',
    );
  });
...
function requireCell(layout: PatchMapGridLayout, id: string): PatchMapMaterializedGridCell {
