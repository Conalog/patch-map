...
describe('PatchMap authoring product planner', () => {
  it('creates every supported element at one world center with detached unique identities', () => {
    ...
    const submitted: PatchMapAuthoringAction[] = [];
    ...
    const duplicate = planPatchMapAuthoringAction(current, {
      type: 'create-element',
      kind: 'rect',
      id: 'created-item',
      positionWorld: [0, 0],
  ...
  it('edits, aligns, and distributes ordered mixed-size targets idempotently', () => {
    let current = materializePatchMapDataset(interactiveScene());
    ...
    const second = planPatchMapAuthoringAction(current, {
      type: 'distribute-targets',
      targets: ['item-a', 'rect-b', 'text-c'],
      axis: 'horizontal',
      basis: 'bounds',
    ...
    expect(planPatchMapAuthoringAction(current, {
      type: 'distribute-targets',
  ...
  it('normalizes a valid text style and rejects invalid alpha without a candidate', () => {
    const current = materializePatchMapDataset(interactiveScene());
    ...
    const invalid = planPatchMapAuthoringAction(styled, {
      type: 'apply-style',
      target: 'text-c',
      changes: { alpha: 2 },
      strict: true,
  ...
  it('groups and duplicates trees while preserving external references and atomic selection intent', () => {
    let current = materializePatchMapDataset(groupingScene());
    current = applyAction(current, {
      type: 'group-targets',
      targets: ['a', 'b'],
      groupId: 'g',
    ...
    expect(planPatchMapMutationTransaction(current, collisionPlan.transaction)).toMatchObject({
      status: 'rejected',
...
): Extract<PatchMapAuthoringPlan, { readonly status: 'planned' }> {
  const plan = planPatchMapAuthoringAction(current, action, { selectionIds });
...
function applyPlan(
  current: MaterializedPatchMapDataset,
  plan: Extract<PatchMapAuthoringPlan, { readonly status: 'planned' }>,
): MaterializedPatchMapDataset {
  const transaction = planPatchMapMutationTransaction(current, plan.transaction);
...
function allElementIds(elements: readonly NormalizedPatchMapElement[]): string[] {
