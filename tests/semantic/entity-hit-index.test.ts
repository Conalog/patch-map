...
describe('PatchMap bounded entity hit index', () => {
  it('bounds huge finite AABBs in overflow and merges candidates in topmost order', () => {
    ...
    const index = PatchMapEntityHitIndex.build(snapshot(entities), null, new Set(), {
      cellSize: 64,
      maxCellsPerEntity: 4,
    });
  ...
  it('preserves exact skewed negative-scale containment and stale-projection fallback', () => {
    ...
    const affine = freezePatchMapAffine(-1, 0.5, 0.75, 1, 100, 50);
    const projection: PatchMapEntityProjection = Object.freeze({
      entityId: dense.id,
      localBounds: Object.freeze([0, 0, 20, 10] as const),
      affine,
      worldBasis: Object.freeze([-1, 0.5, 0.75, 1] as const),
      visibleCenter: applyPatchMapAffine(affine, [10, 5]),
      rotationDegrees: 0,
      scaleX: -1,
      scaleY: 1,
      contentOrientation: 'follow-item',
    });
    const projectedPoint = applyPatchMapAffine(affine, [5, 5]);

    expect(patchMapEntityContainsWorldPoint(
      dense,
    ...
    expect(hitTestPatchMapEntityIndex(
      projectedIndex,
      { x: projectedPoint[0], y: projectedPoint[1] },
      {},
      () => dense,
    ...
    const denseIndex = PatchMapEntityHitIndex.build(snapshot([dense]), projectionIndex, stale);
    expect(hitTestPatchMapEntityIndex(
      denseIndex,
      { x: 5, y: 5 },
      {},
  ...
  it('keeps interpolation endpoints in stable broad-phase buckets', () => {
    ...
    const liveProjection = projectionIndex(
      bar,
      freezePatchMapAffine(1, 0, 0, 1, 0, 90),
      [0, 0, 20, 10],
    );
    const destinationProjection = projectionIndex(
      bar,
      freezePatchMapAffine(1, 0, 0, 1, 0, 20),
      [0, 0, 20, 80],
    );
    const index = PatchMapEntityHitIndex.build(
      snapshot([bar]),
      liveProjection,
      new Set(),
  ...
  it('applies visible, interactive, kind, and topmost filters after the broad phase', () => {
    ...
    const index = PatchMapEntityHitIndex.build(snapshot(entities), null);
    const getEntity = (ref: EntityRef): EntitySnapshot | null => bySlot.get(ref.slot) ?? null;

    expect(hitTestPatchMapEntityIndex(index, { x: 10, y: 10 }, {}, getEntity, null)).toEqual(
      bottom.ref,
    );
    expect(hitTestPatchMapEntityIndex(
      index,
  ...
  it('uses dense spatial buckets for orthogonal scenes and keeps the exact fallback lazy', async () => {
    ...
    type UnsafePatchMapConstructor = new (rendererValue: unknown, options: unknown) => PatchMapRuntime;
    const UnsafePatchMap = PatchMapRuntime as unknown as UnsafePatchMapConstructor;
    const core = new UnsafePatchMap(renderer, { autoRender: false });
...
function projectionIndex(
  ...
  affine: ReturnType<typeof freezePatchMapAffine>,
  ...
  const projection: PatchMapEntityProjection = Object.freeze({
    ...
    visibleCenter: applyPatchMapAffine(affine, [
