...
import fixtureCatalog from '../../contracts/patch-map/evidence/catalog-fixtures.v1.json';
import {
...
describe('PatchMap PixiJS color resolution', () => {
  ...
  it('publishes a theme replacement only after every new entry validates', () => {
    const resolver = createPatchMapColorResolver({ stable: '#112233ff' });
...
function expectInvalid(operation: () => unknown, inputPath: string): void {
  expect(operation).toThrowError(
    expect.objectContaining<Partial<PatchMapColorResolutionError>>({
