...
describe('PatchMap signed affine orientation projection', () => {
  it('composes and inverts generic finite signed 2D transforms', () => {
    const parent = createPatchMapAffine(12, -7, 37, -2, 3);
    const local = createPatchMapAffine(4, 9, -22, 0.5, -1.25);
    const composed = multiplyPatchMapAffine(parent, local);
    const point = Object.freeze([3, -5] as const);
    const sequential = applyPatchMapAffine(parent, applyPatchMapAffine(local, point));

    const composedPoint = applyPatchMapAffine(composed, point);
    expect(composedPoint[0]).toBeCloseTo(sequential[0], 12);
    expect(composedPoint[1]).toBeCloseTo(sequential[1], 12);
    const restored = applyPatchMapAffine(invertPatchMapAffine(composed), sequential);
  ...
  it.each([
    ...
    const actual = patchMapAffineBasis(createPatchMapAffine(0, 0, angle, scaleX, scaleY));
  ...
  it('publishes readable bar geometry at the visible bottom without leaving its owner', () => {
    ...
    const world = createPatchMapAffine(0, 0, 45);
    ...
    const ownerLocalCenter = applyPatchMapAffine(ownerScreenInverse, screenCenter);
  ...
  it.each([
    ...
    ({ rotation, flipY }) => {
      ...
      const world = createPatchMapAffine(0, 0, 0, 1, flipY ? -1 : 1);
      const rotatedWorld = multiplyPatchMapAffine(
        world,
        createPatchMapAffine(0, 0, rotation),
      );
      const ownerScreen = applyPatchMapAffine(rotatedWorld, owner.visibleCenter);
      const [x, y, width, height] = bar.screenBounds;
