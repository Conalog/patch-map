...
describe('PatchMap staged semantic transaction planner', () => {
  ...
  it('runs ordered nested merges against one detached staged candidate', () => {
    ...
    const result = planPatchMapMutationTransaction(current, request);
  ...
  it('replaces arrays as whole values and classifies a no-op target once', () => {
    const current = makeScene();
    const result = planPatchMapMutationTransaction(current, {
      strict: true,
      operations: [
        {
          op: 'merge',
  ...
  it('rejects unknown envelope fields, empty transactions, and non-serializable values', () => {
    const unknown = planPatchMapMutationTransaction(makeScene(), {
      strict: true,
      operations: [merge(elementTarget('rect-b'), ['attrs', 'x'], 1)],
      surprise: true,
    });
    const empty = planPatchMapMutationTransaction(makeScene(), { strict: true, operations: [] });
    const nonFinite = planPatchMapMutationTransaction(makeScene(), {
      strict: true,
  ...
  it('rejects replacement identity conflicts and cross-scope discriminators', () => {
    const identityConflict = planPatchMapMutationTransaction(makeScene(), {
      strict: true,
      operations: [
        {
          op: 'replace',
    ...
    const crossScope = planPatchMapMutationTransaction(makeScene(), {
      strict: true,
      operations: [
        {
          op: 'replace',
  ...
  it('rejects duplicate component identities, merge reconciliation, and non-item owners', () => {
    const duplicate = planPatchMapMutationTransaction(makeScene(), {
      strict: true,
      operations: [
        {
          op: 'reconcile-components',
    ...
    const mergeMode = planPatchMapMutationTransaction(makeScene(), {
      strict: true,
      operations: [
        {
          op: 'reconcile-components',
    ...
    const nonItem = planPatchMapMutationTransaction(makeScene(), {
      strict: true,
      operations: [
        {
          op: 'reconcile-components',
  ...
  it('requires subtree cascade for non-empty groups', () => {
    const rejected = planPatchMapMutationTransaction(makeScene(), {
      strict: true,
      operations: [{ op: 'remove', target: elementTarget('nested'), cascade: 'reject' }],
    });
    const accepted = planPatchMapMutationTransaction(makeScene(), {
      strict: true,
  ...
  it('separates invalid schema versions, unknown operations, and unsupported contract operations', () => {
    ...
    const schema = planPatchMapMutationTransaction(makeScene(), request, 'future/2');
    const unknown = planPatchMapMutationTransaction(makeScene(), {
      strict: true,
      operations: [{ op: 'teleport', target: elementTarget('rect-b') }],
    });
    const unsupported = planPatchMapMutationTransaction(makeScene(), {
      strict: true,
      operations: [
        {
          op: 'unset',
