...
describe('CoreV2Engine async and transient presentation substrate', () => {
...
class FeatureSurface implements CoreV2EngineSurface {
...
async function createEngine(
  ...
  instanceId: string,
): Promise<Readonly<{ engine: CoreV2Engine; surface: FeatureSurface }>> {
...
function refreshLabel(target: CoreV2SemanticTarget): string {
  return target.kind === 'component' ? `${target.ownerId}/${target.id}` : target.id;
}

function rectX(engine: CoreV2Engine): number | null {
