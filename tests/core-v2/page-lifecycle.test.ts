...
describe('Core v2 page lifecycle authority', () => {
  ...
  it('invalidates cancellable work generations and accepts only authoritative tokens', () => {
    const authority = new CoreV2PageLifecycleAuthority();
  ...
  it('cancels motion and root gesture ownership and publishes one resume frame', async () => {
    ...
    const engine = new CoreV2Engine({
      surfaceFactory: (_options: CoreV2SurfaceOptions) => Promise.resolve(surface),
    });
    engines.push(engine);
...
class PageLifecycleSurface implements CoreV2EngineSurface {
