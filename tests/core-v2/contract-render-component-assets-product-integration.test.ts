...
import { AggregateMeshLayer } from '../../src/core-v2/renderers/mesh-layer';
import type {
...
describe('Core v2 REN-008 / REN-010 executable product integration', () => {
  it.each(CASES)(
    ...
    async ({ caseId, actionCount, assertionCount }) => {
      ...
      try {
        ...
        const detachedRun = descriptor.createRun(
          materializeCoreV2ExecutableCase(caseId, '100', 319),
        );
...
class HeadlessComponentAssetRenderer {
  ...
  private readonly aggregate = new AggregateMeshLayer({
    label: 'Core v2 component assets executable integration mesh',
