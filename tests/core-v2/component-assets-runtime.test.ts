...
import {
  createCoreV2RenderComponentAssetsRuntime,
  type CoreV2RenderComponentAssetsRuntime,
} from '../../lab/performance-v2/contract/render-component-assets-runtime';
...
describe('Core v2 REN-008 / REN-010 local Pixi runtime adapter', () => {
  it('settles icon aliases through CoreV2Engine, Pixi Texture/Sprite leases, and WebGL facts', async () => {
...
async function createHarness(
  runtime: CoreV2RenderComponentAssetsRuntime,
  caseId: CaseId,
): Promise<Readonly<{ engine: CoreV2Engine; renderer: HeadlessPixiWebGLRenderer }>> {
  ...
  const surfaceFactory: CoreV2EngineSurfaceFactory = (options) => {
    ...
    const TestCoreV2 = CoreV2 as unknown as new (
      rendererValue: PixiCoreV2Renderer,
      optionsValue: CoreV2Options,
    ) => CoreV2;
...
class HeadlessPixiWebGLRenderer {
  ...
  private world: CoreV2WorldOrientation = Object.freeze({
    rotationDegrees: 0,
    flipX: false,
    flipY: false,
  });
  ...
  public setProjection(projection: CoreV2ProjectionIndex): boolean {
...
function emptyLane(role: CoreV2RenderLaneRole): CoreV2RenderLaneProbe {
  return Object.freeze({
    role,
    label: `core-v2:${role}`,
    renderObjectCount: 0,
