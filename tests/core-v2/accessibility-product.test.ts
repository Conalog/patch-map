...
describe('Core v2 accessibility product authority', () => {
  ...
  it('derives stable focus order and owner-aggregate bounds without Pixi identities', () => {
    ...
    const derived = deriveCoreV2AccessibilityTargets(
      logical.targets(),
      geometryEntities(),
    );
  ...
  it('filters hidden targets and rejects non-finite geometry without losing locked focus', () => {
    ...
    expect(deriveCoreV2AccessibilityTargets(targets, geometries)).toEqual({
      targets: [{
        id: 'item-a',
        label: 'Item A',
        type: 'item',
  ...
  it('deduplicates Pixi aliases while preserving logical focus and selection intent', () => {
    const authority = new CoreV2AccessibilityAuthority();
    authority.reconcile({
      targets: [{
        id: 'item-a',
        label: 'Item A',
...
describe('Core v2 Engine accessibility integration', () => {
...
class AccessibilitySurface implements CoreV2EngineSurface {
  ...
  public activationListener:
    ...
    | null = null;
  public lastReconcileOptions: CoreV2SurfaceReconcileOptions = {};
  private nodes: readonly CoreV2AccessibilityRenderNode[] = [];
...
function geometryEntities(): readonly CoreV2SurfaceEntityGeometry[] {
  return Object.freeze([
