...
import {
  ...
  type CoreV2FrameLoopTarget,
  type FrameDriver,
} from '../../src/core-v2/scheduler';
...
describe('CoreV2AdaptiveFrameBudget', () => {
  it('publishes viewport-only frames before the next large presentation upload', () => {
    const budget = new CoreV2AdaptiveFrameBudget();
...
describe('CoreV2FrameLoop', () => {
  it('owns one RAF and applies the shared viewport-first budget to a target', () => {
    ...
    let destroyed = false;
    const target: CoreV2FrameLoopTarget = {
    ...
    const loop = new CoreV2FrameLoop(target, { driver });
    loop.request();
    expect(driver.pending()).toBe(1);
