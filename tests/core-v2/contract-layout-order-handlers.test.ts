...
import { createCoreV2LayoutOrderRuntime } from '../../lab/performance-v2/contract/layout-order-runtime';
import { createCoreV2ExecutableLabBridge } from '../../lab/performance-v2/contract/executable-bridge';
import type { CoreView, SceneDocument, SlotRange } from '../../src/core-v1/contracts';
import {
  RenderFlags,
  RenderKind,
...
} from '../../src/core-v2/paint-order-product';
...
describe('Core v2 LAY-002 layout-order actual-only handlers', () => {
  ...
  it.each(['LAY-002', 'LAY-003'] as const)(
    'runs and repeats %s through the focused Lab bridge with the CoreV2 product surface',
    async (caseId) => {
...
async function execute(
  ...
  product: ReturnType<typeof createCoreV2LayoutOrderRuntime>['product'],
...
class LayoutSurface implements CoreV2EngineSurface {
...
class LayoutOrderRendererTestDouble {
  ...
  private readonly paintById = new Map<string, CoreV2EntityPaintProbe>();
...
function paintProbe(entityId: string): CoreV2EntityPaintProbe {
  return Object.freeze({
    entityId,
