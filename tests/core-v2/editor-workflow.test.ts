...
} from '../../src/core-v2/editor-workflow';
import {
  CoreV2Engine,
...
import type { NormalizedCoreV2Element } from '../../src/core-v2/semantic/dataset';

class EditorSurface implements CoreV2EngineSurface {
  public canvasCount = 1;
  public loaded: readonly NormalizedCoreV2Element[] = Object.freeze([]);
  ...
  public load(input: readonly NormalizedCoreV2Element[]): void {
    this.loaded = input;
    this.selectionIds = Object.freeze([]);
  }

  public reconcile(
    input: readonly NormalizedCoreV2Element[],
...
describe('CoreV2 editor workflow product', () => {
  ...
  it('executes all twelve editor mutation kinds as real reversible history entries', async () => {
    ...
    const result = engine.runEditorMutationMatrix({
      mutationKinds: CORE_V2_EDITOR_MUTATION_KINDS,
      oneActionEach: true,
      companion,
    });
  ...
  it('destroys logical workflow sessions without retaining editor ownership', () => {
    const authority = new CoreV2EditorWorkflowAuthority();
