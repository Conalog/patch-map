...
describe('Core v2 deterministic text projection', () => {
  ...
  it('keeps the standalone authored frame separate from natural geometry and affine inputs', () => {
    ...
    const parsed = parsePatchMapV010(materializeCoreV2Dataset(input).dataset);
  ...
  it('projects empty, wrapped, requested-font fallback, and rapid text as distinct stable shapes', () => {
    const materialized = materializeCoreV2Dataset(catalogProfiles.datasets['standalone-text']);
  ...
  it('preserves zero, positive, and negative grapheme split plus bidi component identity', () => {
    const parsed = parsePatchMapV010(
      materializeCoreV2Dataset(catalogProfiles.datasets['item-text-corpus']).dataset,
    );
  ...
  it('derives all supplemental placement, auto-font, wrap, overflow, and upright facts from product input', () => {
    const results = new Map(createCoreV2RenderTextSpecimens().map((specimen) => {
      const parsed = parsePatchMapV010(materializeCoreV2Dataset(specimen.dataset).dataset);
