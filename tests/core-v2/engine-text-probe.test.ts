...
import type { CoreV2TextProductProbe, CoreV2TextTarget } from '../../src/core-v2/core';
...
describe('CoreV2Engine O(1) text product seam', () => {
  ...
  it('does not add a dataset or Pixi traversal fallback to the Engine probe', () => {
    const source = CoreV2Engine.prototype.textProbe.toString();
...
class LegacyTextSurface implements CoreV2EngineSurface {
  public canvasCount = 1;
  public destroyed = false;
  protected view: CoreV2SurfaceView = Object.freeze({ x: 0, y: 0, scale: 1, rotation: 0 });
  ...
  public setView(view: CoreV2SurfaceView): void { this.view = Object.freeze({ ...view }); }
...
class IndexedTextSurface extends LegacyTextSurface {
  ...
  private probes = new Map<string, CoreV2TextProductProbe | null>();
  ...
  public corruptLastRenderedLayout(target: CoreV2TextTarget): void {
  ...
  private rebuildProbes(): void {
    ...
    const probes = new Map<string, CoreV2TextProductProbe | null>();
    if (!parsed) {
      this.probes = probes;
      return;
    }
    ...
    for (const entityId of Object.keys(parsed.projection.textsByEntityId ?? {}).sort()) {
      ...
      const target: CoreV2TextTarget = semantic.targetKind === 'element'
...
function createSurfaceProbe(input: Readonly<{
  ...
  const attached = current
    ...
    : null;
  const renderer: CoreV2TextRendererProbe = Object.freeze({
  ...
  const paint: CoreV2EntityPaintProbe | null = current
    ? Object.freeze({
        entityId: input.entity.id,
        lane: 'text',
        rendererKind: 'text',
...
function addSurfaceProbe(
  probes: Map<string, CoreV2TextProductProbe | null>,
  target: CoreV2TextTarget,
  probe: CoreV2TextProductProbe,
): void {
  const key = textTargetKey(target);
...
function textTargetKey(target: CoreV2TextTarget): string {
