...
describe('Core v2 focused contract Lab presenters', () => {
  ...
  it('materializes only exact selected fixtures, actions, size, and seed without expected evidence', () => {
    expect(CORE_V2_EXECUTABLE_ACTION_DEFINITIONS).toHaveLength(45);
    expect(CORE_V2_EXECUTABLE_CASE_IDS).toHaveLength(19);
    expect(CORE_V2_CONTRACT_STUB_COUNT).toBe(154);
