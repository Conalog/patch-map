...
import type { EntityInput, EntitySnapshot, SceneSnapshot } from '../../src/core-v1/contracts';
import type { ParsePatchMapResult } from '../../src/core-v2/contracts';
import {
  buildCoreV2RelationHitIndex,
  createCoreV2SurfaceGeometrySnapshot,
  hitTestCoreV2SurfaceRelations,
  queryCoreV2RelationHitIndex,
  type CoreV2SurfaceRelationGeometry,
...
import {
  ...
  selfLinkWorldPoints,
} from '../../src/core-v2/semantic/relations';

describe('Core v2 aggregate relation paths', () => {
  it('deduplicates ordered pairs, preserves reverse links, and builds a finite self loop', () => {
    ...
    const geometry = createCoreV2SurfaceGeometrySnapshot(snapshotFromParse(parse), parse.projection);
  ...
  it('projects nested endpoint centers through relation-local inverse and F*R screen space', () => {
    ...
    expect(hitTestCoreV2SurfaceRelations(geometry.relations, { x: 200, y: 350 })).toMatchObject({
      relationId: 'nested-links',
      key: 'grid.0.0>nested-item',
    });
    expect(() => hitTestCoreV2SurfaceRelations(
      geometry.relations,
      { x: Number.NaN, y: 0 },
  ...
  it('scales stroke normals once, applies zoom in CSS space, and preserves the 4 CSS px floor', () => {
    ...
    expect(hitTestCoreV2SurfaceRelations(geometry.relations, { x: 100, y: 15.5 })).not.toBeNull();
    expect(hitTestCoreV2SurfaceRelations(geometry.relations, { x: 100, y: 16.5 })).toBeNull();
    ...
    const thin = createCoreV2SurfaceGeometrySnapshot(snapshotFromParse(thinParse), thinParse.projection);
    expect(hitTestCoreV2SurfaceRelations(thin.relations, { x: 50, y: 8.5 })).not.toBeNull();
    expect(hitTestCoreV2SurfaceRelations(
      thin.relations,
      { x: 50, y: 8.5 },
      { toleranceCssPx: 3 },
  ...
  it('bounds long-path indexing and merges overflow candidates in scene order', () => {
    ...
    const index = buildCoreV2RelationHitIndex(relations);

    expect(index.cells.size).toBeLessThan(1_000);
    expect(index.overflow).toEqual([1]);
    const candidates = queryCoreV2RelationHitIndex(index, { x: 32, y: 32 });
    expect(candidates).toEqual([0, 1]);
    expect(new Set(candidates).size).toBe(candidates.length);
    expect(hitTestCoreV2SurfaceRelations(
      candidates.map((candidate) => relations[candidate] as CoreV2SurfaceRelationGeometry),
      { x: 32, y: 32 },
  ...
  it('hides incident paths with retained endpoint geometry and surfaces omitted links', () => {
    ...
    expect(hitTestCoreV2SurfaceRelations(geometry.relations, { x: 50, y: 10 })).toBeNull();
  });

  it('normalizes compatibility opacity and atomically stages structural relation links', () => {
    const initial = materializeCoreV2Dataset(relationMatrixDataset());
