...
class ProbeSurface implements CoreV2EngineSurface {
...
function createSurfaceFactory(): Readonly<{
  factory: CoreV2EngineSurfaceFactory;
...
describe('Core v2 actual-only semantic product probe', () => {
  it('reports deterministic preorder hierarchy, text integrity, and converted paint intent', () => {
    ...
    const firstMaterialized = materializeCoreV2Dataset(input);
    const secondMaterialized = materializeCoreV2Dataset(input);

    const first = createCoreV2SemanticProbe(firstMaterialized, {
    ...
    const second = createCoreV2SemanticProbe(secondMaterialized, {
      lifecycle: 'scene-ready',
      datasetRef: 'probe-scene',
      interactionMode: 'select',
      selectionIds: ['item-a'],
  ...
  it('preserves generated defaults and owner-qualifies item and grid-template components', () => {
    ...
    const probe = createCoreV2SemanticProbe(materialized, { lifecycle: 'scene-ready' });
