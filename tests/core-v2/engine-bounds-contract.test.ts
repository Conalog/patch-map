...
import { removeCoreV2SemanticTarget } from '../../src/core-v2/semantic/mutation';
import { planCoreV2SceneReconcile } from '../../src/core-v2/semantic/reconcile';
import { screenToWorld } from '../../src/core-v2/view';

const boundsDataset = catalogProfiles.datasets.bounds;

describe('Core v2 LAY-005 bounds product contract', () => {
  it('publishes exact local, world, screen, visibility, and signed-scale geometry', () => {
    const materialized = materializeCoreV2Dataset(boundsDataset);
    const parsed = parsePatchMapV010(materialized.dataset);
    const scene = new CoreScene();
    scene.load(parsed.document);
    const geometry = createCoreV2SurfaceGeometrySnapshot(scene.snapshot(), parsed.projection);
  ...
  it('uses transformed dense hit testing for transparent interactive targets only', () => {
    const parsed = parsePatchMapV010(
      materializeCoreV2Dataset(boundsDataset).dataset,
    );
    const scene = new CoreScene();
    scene.load(parsed.document);
  ...
  it('plans one atomic removal while preserving unaffected dense identity', () => {
    const current = materializeCoreV2Dataset(boundsDataset);
    const inputBefore = JSON.stringify(boundsDataset);
    const removal = removeCoreV2SemanticTarget(current, { kind: 'element', id: 'rotated' });
    ...
    const plan = planCoreV2SceneReconcile(currentParsed.document, candidateParsed.document);
    const scene = new CoreScene();
...
describe('CoreV2Engine atomic destroyTarget seam', () => {
  ...
  it('rejects missing targets and refuses legacy surfaces without full-load fallback', async () => {
    ...
    const legacy = new CoreV2Engine({ surfaceFactory: () => Promise.resolve(legacySurface) });
...
class SurfaceBase implements CoreV2EngineSurface {
...
class ReconcileSurface extends SurfaceBase {
  ...
  public reconcile(): CoreV2SurfaceReconcileResult {
