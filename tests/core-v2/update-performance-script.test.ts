...
import { CoreV2Engine } from '../../src/core-v2/engine';

// @ts-expect-error -- verification runner is authored as ESM JavaScript.
import * as updatePerformanceSource from '../../scripts/verification/core-v2-update-performance.mjs';
...
interface UpdatePerformanceModule {
  ...
  runCoreV2UpdateTransactionTrial(
    this: void,
    runtime: unknown,
    scale: number,
    trial: number,
  ...
  validateCoreV2UpdatePerformanceOutput(this: void, output: unknown): boolean;
}
...
const {
  ...
  validateCoreV2UpdatePerformanceOutput,
} = updatePerformanceSource as unknown as UpdatePerformanceModule;

describe('Core v2 update transaction performance checkpoint', () => {
  ...
  it('exercises the actual public Engine transaction path without renderer timing', async () => {
    const result = await runCoreV2UpdateTransactionTrial(
      { CoreV2Engine, createSyntheticPatchMap },
      10,
      0,
    );
  ...
  it('rejects drift in raw trials, summaries, and atomic outcome observations', () => {
    ...
    expect(validateCoreV2UpdatePerformanceOutput(output)).toBe(true);

    const wrongSummary = structuredClone(output);
    wrongSummary.runs[0]!.summary.fullBulkPatchMs.median = 999;
    expect(() => validateCoreV2UpdatePerformanceOutput(wrongSummary)).toThrow(
    ...
    expect(() => validateCoreV2UpdatePerformanceOutput(missingRollback)).toThrow(
  ...
  it('rejects duplicate or reordered combined trial indices and seeds', () => {
    ...
    expect(() => validateCoreV2UpdatePerformanceOutput(duplicateIndex)).toThrow(
    ...
    expect(() => validateCoreV2UpdatePerformanceOutput(reorderedIndices)).toThrow(
    ...
    expect(() => validateCoreV2UpdatePerformanceOutput(duplicateSeed)).toThrow(
    ...
    expect(() => validateCoreV2UpdatePerformanceOutput(reorderedSeeds)).toThrow(
  ...
  it('stays expected-blind and labels the renderer exclusion explicitly', async () => {
    const source = await readFile(
      fileURLToPath(
        new URL('../../scripts/verification/core-v2-update-performance.mjs', import.meta.url),
      ),
      'utf8',
    );
    expect(source).toContain('public CoreV2Engine transaction validation');
    expect(source).toContain('PixiJS renderer reconciliation');
    expect(source).toContain("vite.ssrLoadModule('/src/core-v2/index.ts')");
