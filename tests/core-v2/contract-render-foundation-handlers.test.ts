...
describe('Core v2 render-foundation actual-only handlers', () => {
  ...
  it.each([
    ...
    'executes %s in order through one real CoreV2Engine',
...
class ProjectionSurface implements CoreV2EngineSurface {
...
function geometryEntity(
  ...
  visible: boolean,
  interactive: boolean,
): CoreV2SurfaceGeometrySnapshot['entities'][number] {
  return Object.freeze({
    id,
    kind,
