...
describe('Core v2 render browser checkpoint script', () => {
  ...
  it('pins exactly the sixty-two selected render routes and their 768 canonical assertions', () => {
    ...
    expect(records).toEqual([
      ...
      { id: 'SEL-007', expectedAssertions: 9 },
      { id: 'SEL-008', expectedAssertions: 9 },
      { id: 'SEL-009', expectedAssertions: 9 },
      ...
      { id: 'TRN-002', expectedAssertions: 5 },
      { id: 'TRN-003', expectedAssertions: 7 },
      { id: 'TRN-010', expectedAssertions: 6 },
      { id: 'CSM-009', expectedAssertions: 21 },
      { id: 'CSM-010', expectedAssertions: 22 },
    ]);
    expect(records.reduce((total, record) => total + record.expectedAssertions, 0)).toBe(768);
    expect(source).toContain('const EXPECTED_ASSERTION_TOTAL = 768;');
    expect(source).toContain('const EXPECTED_ASSERTION_PASS_TOTAL = 759;');
    ...
    expect(source).toContain(
      "'canonical comparison must be exactly 759 pass and 9 observed immutable conflicts'",
    );
    expect(source).toContain(
      "'repeat comparison must be exactly 759 pass and 9 observed immutable conflicts'",
    );
    expect(source).toContain(
      "'fresh comparison must be exactly 759 pass and 9 observed immutable conflicts'",
    );
  ...
  it('separates observed query/update conflicts from latent UPD-007 revision conflicts', () => {
    ...
    expect(source).toContain("'render checkpoint observed immutable conflict inventory must remain 7'");
    expect(source).toContain("'render checkpoint declared immutable conflict inventory must remain 9'");
