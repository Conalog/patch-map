...
class FakeAssetBackend implements CoreV2AssetBackend {
  public readonly lookupRequests: CoreV2AssetBackendRequest[] = [];
  public readonly loadRequests: CoreV2AssetBackendRequest[] = [];
  ...
  public get(request: CoreV2AssetBackendRequest): unknown {
...
describe('Core v2 shared asset runtime', () => {
  it('registers immutable builtins without allocating resources and rejects conflicts by the closed code', () => {
    ...
    expect(sessionA.registerAssets()).toMatchObject({
      registeredAliases: CORE_V2_BUILTIN_ASSETS.map(({ alias }) => alias),
      duplicateAliases: [],
    });
    expect(sessionB.registerAssets()).toMatchObject({
      registeredAliases: [],
      duplicateAliases: CORE_V2_BUILTIN_ASSETS.map(({ alias }) => alias),
    });
  ...
  it('trusts only the exact package catalog identity and uses closed invalid-input codes', async () => {
    ...
    session.registerAssets([{
      ...
      descriptor: 'core-v2-builtin://images/device.svg',
    }]);
  ...
  it('deduplicates pending work, revalidates each instance, and unloads after the final lease', async () => {
    const backend = new FakeAssetBackend();
    const runtime = new CoreV2AssetRuntime(backend);
    const policyCalls: CoreV2AssetPolicyContext[] = [];
    const policy = (context: CoreV2AssetPolicyContext): void => {
  ...
  it('coordinates leases across runtimes that share one physical backend', async () => {
    ...
    const runtimeA = new CoreV2AssetRuntime(backend);
    const runtimeB = new CoreV2AssetRuntime(backend);
  ...
  it('uses public Pixi cache keys for exact external borrowing and maps package builtins', async () => {
    ...
    const backend = createCoreV2PixiAssetBackend({
      fetchAsset: (src) => {
    ...
    const externalRequest: CoreV2AssetBackendRequest = Object.freeze({
      key: 'core-v2-asset:external',
    ...
    const optionedRequest: CoreV2AssetBackendRequest = Object.freeze({
      ...externalRequest,
    ...
    const builtinRequest: CoreV2AssetBackendRequest = Object.freeze({
      key: 'core-v2-asset:device',
      descriptor: Object.freeze({ src: 'core-v2-builtin://images/device.svg' }),
    ...
    expect(fontLoad).toMatchObject({
      alias: 'core-v2-asset:font',
      parser: 'web-font',
      src: 'blob:core-v2/builtin-2',
      data: { family: 'Fira Code', weights: ['300', '400', '500', '600', '700'] },
    });
    expect(fetchedSources[1]).not.toContain('core-v2-builtin://');
  });
  ...
  it('isolates owned URL loads from Pixi source cache identity before unloading', async () => {
    ...
    const first: CoreV2AssetBackendRequest = Object.freeze({
      key: 'core-v2-asset:first',
      descriptor: Object.freeze({
        src: 'https://assets.example.test/icon.svg',
    ...
    const second: CoreV2AssetBackendRequest = Object.freeze({
      ...first,
      key: 'core-v2-asset:second',
      descriptor: Object.freeze({
