...
describe('Core v2 logical query and selection substrate', () => {
  it('queries elements and owner-qualified components in deterministic logical order', () => {
    ...
    const index = new CoreV2LogicalSceneIndex(
      materializeCoreV2Dataset(QUERY_DATASET).dataset,
    );
  ...
  it('shares hierarchy, lock, predicate, and paint-order facts across selection modes', () => {
    const index = new CoreV2LogicalSceneIndex(
      materializeCoreV2Dataset(SELECTION_DATASET).dataset,
    );
...
function keys(
  result: ReturnType<CoreV2LogicalSceneIndex['query']>,
): readonly string[] {
...
function createTestSurfaceFactory(surfaces: TestSurface[]): CoreV2EngineSurfaceFactory {
  return () => {
