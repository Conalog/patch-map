...
import {
  createCoreV2RenderImagesRuntime,
} from '../../lab/performance-v2/contract/render-images-runtime';
import type {
  CoreV2AssetRegistration,
...
describe('Core v2 REN-005 deterministic Pixi asset runtime', () => {
  it('creates a private runtime and closes policy to the declared fixture sources', async () => {
    const first = createCoreV2RenderImagesRuntime();
    const second = createCoreV2RenderImagesRuntime();
...
class FixtureImageEngine {
  public registrations: readonly CoreV2AssetRegistration[] = [];
  public imageProbe: JsonRecord = pendingSceneProbe();

  public constructor(private readonly session: CoreV2AssetSession) {}
