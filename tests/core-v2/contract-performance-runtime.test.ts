...
import {
  ...
  CORE_V2_PERFORMANCE_CLEANUP_REVISION,
  CORE_V2_PERFORMANCE_RUNTIME_REVISION,
  createCoreV2PerformanceRuntime,
} from '../../lab/performance-v2/contract/performance-runtime';
import {
...
describe('Core v2 performance contract automation substrate', () => {
  it('shares one collision-free handler family across seven cases', () => {
    expect(CORE_V2_PERFORMANCE_RUNTIME_REVISION)
      .toBe('core-v2-performance-runtime/1');
    expect(CORE_V2_PERFORMANCE_CASE_IDS).toEqual([
      'PRF-001',
      'PRF-002',
      'PRF-003',
      'PRF-004',
    ...
    expect(handlers.PERFORMANCE_CASE_IDS).toEqual(CORE_V2_PERFORMANCE_CASE_IDS);
    expect(handlers.PERFORMANCE_ACTION_TYPES).toHaveLength(13);

    const runtime = createCoreV2PerformanceRuntime('PRF-001', {
      evidence: evidence(),
    });
  ...
  it('folds committed PRF-001 evidence and independently passes all approved assertions', async () => {
    const plan = materializeCoreV2ExecutableCase('PRF-001', '100', 319);
    const runtime = createCoreV2PerformanceRuntime('PRF-001', {
      evidence: evidence(),
    });
  ...
  it('keeps product transport, handlers, and fold outside expected/comparator data', async () => {
    const urls = [
      new URL(
        '../../lab/performance-v2/contract/performance-runtime.ts',
        import.meta.url,
      ),
      new URL(
