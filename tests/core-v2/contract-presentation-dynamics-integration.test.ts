...
import { createCoreV2PresentationDynamicsRuntime } from '../../lab/performance-v2/contract/presentation-dynamics-runtime';
import type {
...
describe('Core v2 shared presentation dynamics contract runtime', () => {
  it('registers one expected-blind handler family and one zero-resource runtime', async () => {
    const runtime = createCoreV2PresentationDynamicsRuntime('ANI-002');
    ...
    const moduleSources = await Promise.all([
      '../../scripts/verification/core-v2-contract/handlers/presentation-dynamics.mjs',
      '../../scripts/verification/core-v2-contract/fold-presentation-dynamics.mjs',
      '../../lab/performance-v2/contract/presentation-dynamics-runtime.ts',
  ...
  it.each<CaseId>(['UPD-005', 'REN-009', 'ANI-001', 'ANI-002'])(
    ...
    async (caseId) => {
      ...
      const runtime = createCoreV2PresentationDynamicsRuntime(caseId);
      ...
      const execution = await executeContractCase({
        ...
        engineFactory: () => new CoreV2Engine({ surfaceFactory: testSurfaceFactory() }),
...
function testSurfaceFactory(): CoreV2EngineSurfaceFactory {
  return (options: CoreV2SurfaceOptions) => {
