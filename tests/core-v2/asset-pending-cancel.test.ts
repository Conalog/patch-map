...
class PendingAssetBackend implements CoreV2AssetBackend {
...
describe('Core v2 pending asset cancellation', () => {
  it('publishes decoder identities once without replacing the coordinator cache identity', async () => {
    ...
    const describeResource = vi.fn((request: CoreV2AssetBackendRequest, value: unknown) => {
    ...
    const backend: CoreV2AssetBackend = {
      get: () => undefined,
