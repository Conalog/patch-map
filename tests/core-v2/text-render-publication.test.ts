...
describe('Core v2 text render publication', () => {
  ...
  it('dirties only the text slot when the semantic text sidecar changes', () => {
    ...
    const projection = requireTextProjection(parsed.projection, 'text');
    const changed: CoreV2ProjectionIndex = Object.freeze({
      ...parsed.projection,
...
function requireTextProjection(index: CoreV2ProjectionIndex, entityId: string) {
...
interface RendererProbeHarness {
  projectionIndex: CoreV2ProjectionIndex;
  ...
  textRendererProbe(entityId: string): CoreV2TextRendererProbe | null;
}
...
function rendererProbeHarness(
  ...
  const renderer = Object.create(PixiCoreV2Renderer.prototype) as RendererProbeHarness & {
    leaves: AggregateLeafLayer;
  };
