...
describe('Core v2 production operations authority', () => {
  ...
  it('keeps runtime snapshots bounded, immutable, instance-local, and cheap when disabled', () => {
    const disabled = new CoreV2OperationsAuthority();
    ...
    const first = new CoreV2OperationsAuthority({
      collectionEnabled: true,
      capacity: 2,
      instanceId: 'A',
    });
    const second = new CoreV2OperationsAuthority({
      collectionEnabled: true,
      capacity: 2,
      instanceId: 'B',
    });
  ...
  it('classifies extraction readability without retaining source material', () => {
    const security = new CoreV2ExtractionSecurityAuthority();
  ...
  it('bounds hashing work for cyclic and hostile inputs', () => {
    ...
    const diagnostic = redactCoreV2OperationalDiagnostic({
