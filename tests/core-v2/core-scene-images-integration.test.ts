...
describe('Core v2 scene image integration', () => {
  ...
  it('preserves nested affine authority when intrinsic size resolves and on reload', async () => {
    ...
    expect(coreV2AffineHasSkew(fallback)).toBe(true);
  ...
  it('atomically rejects direct image mutations that cannot update the parser sidecar', () => {
    ...
    for (const batch of unsafeBatches) {
      expect(() => core.commit(batch)).toThrow(/CoreV2\.reconcile/u);
...
function createCore(allocated: CoreV2[]): Readonly<{
  core: CoreV2;
...
function affineAabb(
  ...
  const corners = coreV2AffineCorners(affine, [0, 0, width, height]);
