...
} from '../../src/core-v2/transformer-edit';

describe('Core v2 transformer edit planning', () => {
  it('plans integer move, dominant-axis lock, and atomic mixed-set refusal', () => {
    const dataset = scene();
    const before = JSON.stringify(dataset);

    const move = planCoreV2MoveTransform(dataset, {
      selectionIds: ['rect-b'],
      deltaWorld: [10.4, 5.4],
    });
    ...
    expect(planCoreV2MoveTransform(dataset, {
      selectionIds: ['rect-b'],
      deltaWorld: [20, 8],
      axisLock: true,
    })).toMatchObject({
  ...
  it('plans all eight resize directions, image resize, and integer minimum size', () => {
    ...
    for (const [handle, deltaWorld] of Object.entries(cases)) {
      const plan = planCoreV2ResizeTransform(dataset, {
        selectionIds: ['rect-b'],
    ...
    expect(planCoreV2ResizeTransform(dataset, {
  ...
  it('rotates only the eligible mixed subset around the full selection center', () => {
    const dataset = scene();
    const plan = planCoreV2RotateTransform(dataset, {
  ...
  it('resolves final-angle snap across zero and preserves edge-pan world identity', () => {
    expect(resolveCoreV2RotationSnap(350, 7, false)).toEqual({
      startDegrees: 350,
      pointerDegrees: 7,
    ...
    expect(resolveCoreV2RotationSnap(350, 7, true)).toMatchObject({
      continuousDeltaDegrees: 17,
      appliedDegrees: 0,
      snapped: true,
    });
    expect(resolveCoreV2RotationSnap(350, 2, true)).toMatchObject({
      continuousDeltaDegrees: 12,
      appliedDegrees: 0,
    });

    const pan = resolveCoreV2EdgeAutoPan(
      [799, 300],
      [20, 0],
      [400, 300],
      1,
