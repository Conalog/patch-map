...
import {
  ...
  type CoreV2ViewportGeometryEntity,
} from '../../src/core-v2/viewport';

describe('Core v2 viewport substrate', () => {
  const dataset = materializeCoreV2Dataset(
  ...
  it('normalizes padding and accounts for rotation when fitting', () => {
    expect(normalizeCoreV2ViewportPadding()).toEqual({ x: 16, y: 16 });
    expect(normalizeCoreV2ViewportPadding(24)).toEqual({ x: 24, y: 24 });
    expect(normalizeCoreV2ViewportPadding([20, 30])).toEqual({ x: 20, y: 30 });
    expect(() => normalizeCoreV2ViewportPadding([-1, 16])).toThrow(
      'viewport padding must contain two finite non-negative values',
    );

    const bounds = [10, 20, 190, 80] as const;
    expect(coreV2BoundsCenter(bounds)).toEqual([105, 60]);
    expect(coreV2ViewportFitScale(
      bounds,
      [800, 600],
      normalizeCoreV2ViewportPadding(0),
      90,
      [0.25, 4],
  ...
  function resolve(
    ...
    return resolveCoreV2ViewportContributors(dataset, geometry, {
      targets,
      ...options,
    });
  }
...
function viewportGeometry(): CoreV2ViewportGeometry {
