...
} from '../../src/core-v2/semantic/screen-region-index';

describe('CoreV2ScreenRegionIndex', () => {
  it('returns nearby screen buckets in original geometry order', () => {
    ...
    const index = CoreV2ScreenRegionIndex.build(entities, relations);
  ...
  it('falls back to the complete ordered geometry for unsafe query coverage', () => {
    ...
    const index = CoreV2ScreenRegionIndex.build(entities, Object.freeze([]));
