...
describe('LIF-004 actual-only lifecycle resize handler', () => {
  ...
  it('observes resize, viewport, geometry, hit-test, and conversion through CoreV2Engine', async () => {
...
function createHarness(): {
  ...
  const engineFactory = (nextMetadata: EngineFactoryMetadata): CoreV2Engine => {
    ...
    return new CoreV2Engine({ surfaceFactory });
  };
  return { surfaces, metadata, engineFactory };
}

class GeometrySurface implements CoreV2EngineSurface {
  ...
  private entities(): CoreV2SurfaceGeometrySnapshot['entities'] {
    return Object.freeze(this.dataset.flatMap((record) => this.elementGeometries(record)));
  }

  private elementGeometries(record: JsonRecord): CoreV2SurfaceGeometrySnapshot['entities'] {
