...
import type {
  ...
  LeafSceneImageProbe,
} from '../../src/core-v2/renderers/leaf-layer';
import {
  CORE_V2_SCENE_IMAGE_ATTEMPT_LIMIT,
  CoreV2SceneImageController,
  type CoreV2SceneImageRendererBridge,
} from '../../src/core-v2/scene-images';
import type { CoreV2AssetSource } from '../../src/core-v2/semantic/dataset';
...
class MockImageRenderer implements CoreV2SceneImageRendererBridge {
...
describe('CoreV2SceneImageController', () => {
  it('routes alias/direct/data/descriptor sources once and never retains mutable input', async () => {
    ...
    const controller = new CoreV2SceneImageController(renderer, { onInvalidate: invalidate });
    const descriptor = {
  ...
  it('bounds pending shared-binding attempts while a second target churns', async () => {
    ...
    for (let index = 0; index < CORE_V2_SCENE_IMAGE_ATTEMPT_LIMIT * 4; index += 1) {
  ...
  it('bounds source-churn history and prunes removed target diagnostics', async () => {
    const renderer = new MockImageRenderer();
    const controller = new CoreV2SceneImageController(renderer);
    for (let index = 0; index < CORE_V2_SCENE_IMAGE_ATTEMPT_LIMIT + 5; index += 1) {
    ...
    expect(retainedAttempts).toHaveLength(CORE_V2_SCENE_IMAGE_ATTEMPT_LIMIT);
    expect(retainedAttempts[0]).toMatchObject({
      generation: 1,
      authoredSource: 'fixture-image-0',
    });
    expect(retainedAttempts.at(-1)).toMatchObject({
      generation: CORE_V2_SCENE_IMAGE_ATTEMPT_LIMIT + 5,
      authoredSource: `fixture-image-${CORE_V2_SCENE_IMAGE_ATTEMPT_LIMIT + 4}`,
    });
  ...
  it('publishes decoded size once only for a current intrinsic generation', async () => {
    ...
    const controller = new CoreV2SceneImageController(renderer, { onIntrinsicSize });
...
function projection(
  ...
  bindingKey: string,
  cacheIdentity: string,
  dimensionMode: CoreV2ImageDimensionMode = 'authored',
): CoreV2ImageProjection {
  return {
    entityId,
    authoredSource,
...
function imageIndex(images: readonly CoreV2ImageProjection[]): CoreV2ProjectionIndex {
  return {
...
function sourceKind(request: LeafAssetBindingRequest): CoreV2ImageSourceKind {
...
function stringAssetSource(source: CoreV2AssetSource, kind: string): string {
...
function retainedPendingAttemptCount(controller: CoreV2SceneImageController): number {
