...
const [catalogRuntime, materializeRuntime, foldRuntime, compareRuntime] = await Promise.all([
  ...
  loadRuntime<FoldRuntime>('../../verification/contract/fold-layout-order.mjs'),
...
describe('PatchMap LAY-002 actual-only layout-order fold', () => {
  it('is browser-safe, import-free, and independent of answer evidence', async () => {
    const source = await readFile(
      fileURLToPath(new URL(
        '../../verification/contract/fold-layout-order.mjs',
        import.meta.url,
      )),
      'utf8',
    );
...
describe('PatchMap LAY-003 actual-only layout-order fold', () => {
