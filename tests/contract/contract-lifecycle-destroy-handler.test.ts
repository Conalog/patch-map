...
const [
  ...
  loadRuntime<LifecycleRuntime>(
    '../../verification/contract/handlers/lifecycle-destroy.mjs',
  ),
  loadRuntime<LifecycleFoldRuntime>(
    '../../verification/contract/fold-lifecycle-destroy.mjs',
  ),
...
function createHarness(options: Readonly<{
  ...
  const engineFactory = (nextMetadata: EngineFactoryMetadata): PatchMap => {
    ...
    const engine = new PatchMap({ surfaceFactory });
  ...
  const product: Readonly<JsonRecord> = Object.freeze({
    inspectEngineResources(engine: unknown): JsonRecord {
      if (!(engine instanceof PatchMap)) throw new Error('Unexpected lifecycle product engine');
...
class InstrumentedSurface implements PatchMapEngineSurface {
