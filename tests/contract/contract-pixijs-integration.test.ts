...
import * as handlerModule from '../../verification/contract/handlers/pixijs-integration.mjs';
// @ts-expect-error -- browser-safe contract folds are authored as ESM JavaScript.
import * as foldModule from '../../verification/contract/fold-pixijs-integration.mjs';
...
describe('PatchMap PixiJS integration automation substrate', () => {
  ...
  it('keeps handler and fold browser-safe and outside the expected/comparator boundary', async () => {
    const handlerSource = await readFile(
      new URL(
        '../../verification/contract/handlers/pixijs-integration.mjs',
        import.meta.url,
      ),
      'utf8',
    );
    const foldSource = await readFile(
      new URL(
        '../../verification/contract/fold-pixijs-integration.mjs',
        import.meta.url,
      ),
      'utf8',
    );
