...
const [
  ...
  loadRuntime<HandlerRuntime>('../../verification/contract/handlers/render-bounds.mjs'),
  loadRuntime<WorkerRuntime>('../../verification/contract/execute-worker.mjs'),
  loadRuntime<FoldRuntime>('../../verification/contract/fold-render-bounds.mjs'),
  ...
  it('is import-free, browser-safe, expected-blind, and revisioned', async () => {
    const source = await readFile(
      fileURLToPath(new URL(
        '../../verification/contract/fold-render-bounds.mjs',
        import.meta.url,
      )),
      'utf8',
    );
