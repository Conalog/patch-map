...
const [
  ...
  loadRuntime<FoldRuntime>('../../verification/contract/fold-render-orientation.mjs'),
...
describe('PatchMap LAY-004 render-orientation actual-only fold', () => {
  it('is import-free, browser-safe, expected-blind, and revisioned', async () => {
    const source = await readFile(
      fileURLToPath(new URL(
        '../../verification/contract/fold-render-orientation.mjs',
        import.meta.url,
      )),
      'utf8',
    );
