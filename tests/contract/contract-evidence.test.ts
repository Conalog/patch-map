...
const [evidenceRuntime, observerRuntime, comparatorRuntime, verifierRuntime] = await Promise.all([
  loadRuntime<EvidenceRuntime>('../../verification/contract/evidence.mjs'),
  loadRuntime<ObserverRuntime>('../../verification/contract/observe.mjs'),
  loadRuntime<ComparatorRuntime>('../../verification/contract/compare.mjs'),
  loadRuntime<VerifierRuntime>('../../verification/contract/verify-results.mjs'),
]);
...
describe('append-only PatchMap execution evidence', () => {
  ...
  it('keeps the offline verifier free of implementation launch dependencies', async () => {
    const source = await readFile(
      fileURLToPath(new URL('../../verification/contract/verify-results.mjs', import.meta.url)),
      'utf8',
    );
