...
import expectedJson from '../../contracts/evidence/catalog-normalized-expected.v1.json';
...
} from '../../lab/contract/performance-runtime';
import {
...
import * as handlerModule from '../../verification/contract/handlers/performance.mjs';
// @ts-expect-error -- browser-safe contract folds are authored as ESM JavaScript.
import * as foldModule from '../../verification/contract/fold-performance.mjs';
...
describe('PatchMap performance contract automation substrate', () => {
  ...
  it('keeps product transport, handlers, and fold outside expected/comparator data', async () => {
    const urls = [
      new URL(
        '../../lab/contract/performance-runtime.ts',
        import.meta.url,
      ),
      new URL(
        '../../verification/contract/handlers/performance.mjs',
        import.meta.url,
      ),
      new URL(
        '../../verification/contract/fold-performance.mjs',
        import.meta.url,
      ),
    ];
...
function evidence(): Readonly<Record<string, unknown>> {
  ...
  return {
    ...
    rawArtifact: {
      path: 'contracts/evidence/qualification/contract-performance-raw.test.json',
      sha256: RAW_DIGEST,
      sampleCount: 42,
    },
    browser: {
