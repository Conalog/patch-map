...
const [catalogRuntime, materializeRuntime, foldRuntime, compareRuntime] = await Promise.all([
  ...
  loadRuntime<FoldRuntime>('../../verification/contract/fold-render-images.mjs'),
...
describe('PatchMap REN-005 render-images actual-only fold', () => {
  it('is import-free, browser-safe, expected-blind, and revisioned', async () => {
    const source = await readFile(
      fileURLToPath(new URL(
        '../../verification/contract/fold-render-images.mjs',
        import.meta.url,
      )),
      'utf8',
    );
    ...
    await assertCommittedVerifierEntryImportFirewall('fold-render-images.mjs', 'fold');
  });
