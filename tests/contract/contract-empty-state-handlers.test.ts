...
const [catalogRuntime, materializeRuntime, emptyStateRuntime, foundationRuntime, workerRuntime] = await Promise.all([
  ...
  loadRuntime<EmptyStateRuntime>('../../verification/contract/handlers/empty-state.mjs'),
  loadRuntime<FoundationRuntime>('../../verification/contract/handlers/foundation.mjs'),
...
describe('empty-state actual-only handler registry', () => {
  it('registers exactly the approved set-host-state and query-target actions behind the expected firewall', async () => {
    ...
    const sources = await Promise.all([
      '../../verification/contract/execute-worker.mjs',
      '../../verification/contract/handlers/empty-state.mjs',
      '../../verification/contract/handlers/foundation.mjs',
