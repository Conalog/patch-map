...
2. Require `CI` and `Shared · Workflow security` before merging release PRs. Workflow policy runs for every `release/1.0` PR, including version-only release PRs, so the required status cannot remain pending because of a path filter. Protect `release/1.0`, historical `v*`, `js-v*` and `dart-v*` against unauthorized changes.
