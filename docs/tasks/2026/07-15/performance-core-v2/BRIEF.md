...
Connect every approved Core v2 decision and case to the PixiJS implementation, independent actual automation, the same focused Lab route, packed integration, and release evidence. Completion means 38/38 decisions and all 173 cases satisfy immutable expected observations and production-readiness gates.
...
- Contract revision `core-v2-functional-contract/2026-07-16.2` contains 173 records, 381 action definitions, 646 actions, and 1,388 assertions; all 38 journeys are migration-blocking P0.
- Canonical generation proves specification integrity only. Promotion requires expected-blind product execution, immutable actuals, independent comparison, the exact focused route, packed-host evidence where declared, cleanup, determinism, and review.
- Core v2 accepts array-root PATCH MAP JSON directly, uses a dense store and aggregate Mesh/WebGL renderer, preserves caller input and stable identity, and reports unsupported projection instead of restoring per-entity Pixi objects.
- WebGL2 is mandatory. Chromium 4× is a proxy; WebGPU and native Windows, accessibility-device, security, migration, and target-performance evidence remain pending.
