...
| `State` | Subclassable state with handled events and lifecycle | ABI-003, independent state tests | planned |
| `PROPAGATE_EVENT` | Passes an input event to the next stacked state | independent state tests | planned |
| `Command` | Subclassable command with ID, `execute`, and `undo` | HIS-001~002, ABI-003 | planned |
| `UndoRedoManager` | Limit, execute/undo/redo/clear/destroy, events, grouping | HIS-001~002 | planned |
| `selector` | Standalone public JSONPath resolver | independent selector tests | planned |
...
| `uid` | Public generated-ID helper | independent helper tests; exact contract pending oracle | planned |

## Patchmap surface

| Surface | Required observations | Evidence | Status |
| --- | --- | --- | --- |
| Construction/properties | Pre/post-init public state, materialized theme, transformer replacement, history recreation | LIF-001~002, ABI-001 | planned |
| `init` / `destroy` | Async idempotence, event readiness, DOM/resource cleanup, re-init | LIF-001~002, MEM-001 | planned |
| `draw` | Validation transaction, input immutability, defaults, replacement, async coalescing | DRW-001~006 | planned |
| `update` | Direct/path targeting, merge/replace, refresh, relative/center transform, history, silent immediate update | UPD-001~006, HIS-002 | planned |
| `selector` | Root/direct/recursive traversal, filters, boolean/string expressions, projections, live refs | SEL-001~002, ABI-001 | planned |
...
| --- | --- | --- | --- |
| Seven element kinds | `group`, `grid`, `item`, `relations`, `image`, `text`, `rect`; defaults and live handles | DRW-001~003, ABI-001 | planned |
| Four component kinds | `background`, `bar`, `icon`, `text`; defaults, placement, sizing, matching | DRW-001, UPD-002 | planned |
| Grid materialization | Cell IDs/labels/geometry and destroy/hide inactive strategies | DRW-002 | planned |
| Primitive normalization | Size, gap, margin, padding, placement, color/theme lookup | independent data contract tests | planned |
| Assets | Alias, URL, inline descriptors, rectangle textures, stale async protection | AST-001~002 | planned |
| Observable scene facade | Parent/children, identity/type/props/transforms/dimensions/visibility/bounds/destroyed | ABI-001 | planned |
| Rendering invariants | Geometry, text, stacking, visibility, pixels within approved tolerance | DRW-001~002 and independent visual tests | planned |

## Cross-cutting gates

| Gate | Evidence | Status |
| --- | --- | --- |
| Fourteen approved oracle fixtures | LIF-001~002, DRW-001~006, UPD-001~006 | planned |
| Fresh-run determinism | Repeat normalized fixture runs in new browser sessions | planned |
| Input/reference/event/error edge cases | Independent contract suite | planned |
| Build/typecheck/lint/unit | Package scripts | planned |
