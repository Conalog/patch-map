...
공개 메서드와 프로퍼티를 옵션 단위로 빠르게 훑고 싶다면 먼저 `public-api.md`를 보고, 세부 데이터/상호작용/히스토리 동작은 각 주제 문서로 내려가는 방식이 가장 효율적이다. 구현 수정에 필요한 파일 책임과 검증 루틴은 `developer-context.md`를 보면 된다.
