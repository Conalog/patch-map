...
visual: Core v2 is a consumable package and its renderer is genuinely PixiJS-based.

## PIX-001 — Use a Real PixiJS Renderer and Stage

- **Goal:** Benefit from PixiJS rendering, lifecycle, and developer tooling.
- **Action:** Initialize Core v2 in a supported browser and inspect its active canvas.
- **Result:** The visible scene is rendered by a live PixiJS Application/renderer and
  ...
  the application and inspect the stage-level scene. Core v2 may aggregate thousands
...
- **Result:** Core v2 either recreates renderer resources from the current semantic
...
- **Goal:** Install Core v2 in a separate host project.
- **Action:** Build and pack the actual package, install it in a fresh offline consumer,
  and import the documented Core v2 entry with strict TypeScript settings.
...
- **Result:** Every required capability is reachable without importing Original API
  names, live handles, selectors, or command classes. The adapter contains orchestration
...
- **Result:** All journeys pass against the same package digest; host/Core revisions and
  ...
  engine behavior or import restricted/Original internals.
...
- **Goal:** Let consumers adopt and operate a redesigned API without undocumented
  assumptions.
- **Action:** Build the package and compile/run every public example and migration path.
