...
- **When:** The host initializes Core v2.
...
- **Then:** Core v2 pauses or cancels work according to the action's deterministic time
...
## DAT-006 — Load legacy dataset input

- **Priority:** P1
- **User goal:** Open persisted maps that use the supported pre-array schema.
- **Given:** A valid legacy root object.
- **When:** The host loads it through the supported ingestion boundary.
- **Then:** It is converted to the current dataset meaning before validation and renders the equivalent hierarchy, geometry, text, color, and relations without mutating the legacy input.
- **Edges:** Ambiguous or malformed legacy records fail with a path-aware diagnostic instead of being partially converted.
- **Automation:** Compare legacy and canonical semantic snapshots in fresh sessions.
- **Lab:** `Load legacy fixture` beside its canonical equivalent.
