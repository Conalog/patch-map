...
- **Given:** Rectangles using fill, stroke, opacity, uniform/per-corner radius, transform, and stacking values.
...
- **Given:** Multiline text using supported family, weight, size, fill, wrap, line height, letter spacing, transform, and optional bounds.
- **When:** It is rendered or its content/style changes.
- **Then:** Text content, line breaks, baseline intent, spacing, bounds, stacking, opacity, visibility, rotation, and hit area update together.
- **Edges:** Empty text, long unbroken text, Unicode, mixed line endings, missing font, and rapid content replacement have explicit fallback and deterministic bounds.
...
- **Given:** Ordered links connecting elements across nested groups and grids, with stroke style and a transformed relations container.
- **When:** The scene draws, endpoints move/resize/hide, link sets change, or the world rotates, flips, focuses, or zooms.
- **Then:** Every valid link connects the exact centers of source and target world bounds after conversion into the relations element's local frame; style and visibility update; relation stacking remains deterministic relative to elements and overlays.
...
- **Then:** The background stays aligned with the full item area, preserves corner and
  border intent, and switches source without showing stale texture or bounds. An authored
  background `size` is preserved for roundtrip but never changes that geometry.
...
- **Edges:** Empty, multiline, grapheme/CJK/bidi/RTL/emoji/combining-mark, very long,
  ...
  geometry. `split=0` preserves the original string; positive and historically accepted
  negative split values follow immutable fixture output and never split a grapheme or hang.
...
- **Then:** Each named placement occupies its declared anchor relative to the current
  content frame; margins offset the expected edges. Historically accepted
  `placement: none` remains distinct and uses its immutable normalized-geometry fixture,
  never an inferred named-anchor remap.
...
- **Edges:** Query strings, URL fragments, data URIs, deprecated parser aliases, and cyclic/reused option objects have deterministic validation and cache identity.
