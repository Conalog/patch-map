...
`core-v2-semantic-observation/1`.
...
- World and local coordinates use +X right and +Y down. Sizes are nonnegative world
  units and local element origin is its untransformed top-left unless the dataset
  explicitly authors a pivot.
- Core v2 uses PixiJS v8 public transform semantics for position, scale, skew, pivot,
  and rotation. Parent-to-child composition and `toGlobal`/`toLocal` results are the
  compatibility surface; private transform storage is irrelevant.
...
Negative padding intentionally expands the frame. A non-finite or non-positive
result may remain valid for semantic geometry only when the concrete component still
resolves to finite nonnegative dimensions; otherwise validation rejects the action.
...
Top/left margin moves inward positively; bottom/right margin moves inward negatively
relative to those edges. Negative margins therefore move outward. Exact numeric
fixtures cover every anchor with asymmetric margins, rotation, flip, and negative
padding before LAY-001/002 can become verified.
...
- Relation path bounds include stroke alignment. Point, box, and paint hit expand the
  path in screen space by `max(4 CSS px, visible stroke width / 2)` after zoom and DPR.
  ...
  Caps, joins, endpoints, self-links, and each segment participate in the expanded
  path. Zero/transparent but interaction-eligible strokes use the 4px floor; hidden
  relations have no hit. Semantic relation geometry remains the authored stroke path.
...
`evidence/decision-evidence-manifest.v1.json` freezes all 38 owner decisions and
...
a Core v2 runner, Lab, packed host, or release platform has executed the case.
