...
This document closes the field-level language accepted by Core v2. It is normative
alongside `dataset.md`; prose examples cannot widen it. The persisted root remains the
existing ordered element array. Core v2 adds no dataset version field, envelope, or
...
| `relations` | `links` | `style` | ordered link list; default relation stroke |
...
| `type` | Required additional keys | Optional additional keys | Defaults / constraints |
| --- | --- | --- | --- |
| `background` | `source` | `tint`, `size` | semantic size is 100% of full item; white tint; authored `size` is preserved but inert |
...
- `gap`, margin/padding, radius, asset descriptor, texture style, stroke style, text
  style, placement, and orientation accept only the documented keys/enums.
- `inactiveCellStrategy` is exactly `destroy` or `hide`; `contentOrientation` is exactly
  `follow-item` or `upright`.
- A texture style has optional `type: rect` plus optional `fill`, `borderWidth`,
  `borderColor`, and `radius`. An omitted discriminator normalizes to rectangular
  texture semantics. Omitted values normalize to transparent, `0`, black, and `0`.
- Background `size` is accepted and preserved for compatibility but is inert; the
  background remains the full item frame.
- Numeric render fields must be finite. Alpha and stroke alignment are closed to
  `0..1`. Component-dimension plain numeric strings and `px` scalar strings are
  rejected; documented percentage/`calc()` dimensions and text-style CSS size branches
  remain valid. Durations and border widths are nonnegative.
...
| `scale` | finite scalar or strict `{x,y}` | scalar expands to equal axes |
...
values fail. Other attrs keys are inert to Core v2 rendering unless the host maps them
to a visual update.
...
Historically runtime-accepted legacy object roots enter through a compatibility
ingestion boundary and normalize to the same ordered element array. Canonical export
uses that array root; semantic roundtrip compatibility does not preserve the legacy
wrapper shape.
