...
implementation. They are the starting corpus for Core v2 schema tests.
...
[
  ...
  },
  {
    ...
    "style": { "color": "primary.dark", "width": 2, "cap": "round", "join": "round" }
  },
  {
    "type": "image",
    "id": "image-1",
...
| background size/padding | background fills the full item, while other percentage components use padded content box |
...
| --- | --- | --- |
| malformed legacy root | unknown or ambiguous legacy wrapper | compatibility ingestion rejects atomically; every historically accepted wrapper has a separate accepted fixture |
...
| unknown component key | valid component plus `"extra": true` | strict component key rejection; public text/stroke style and `attrs` extension points remain allowed |
...
| placement | unknown value other than accepted `none` | rejected; `none` has an accepted fixture with exact normalized geometry |
...
| text split | non-integer | integer required; negative accepted variants have finite fixture-owned output |
...
- numeric local/world/screen transforms and bounds for nested position, scale, skew,
  pivot, angle/rotation, negative spacing, all nine placements, grid/ragged matrix,
...
`evidence/decision-evidence-manifest.v1.json` closes all 38 owner decisions.
