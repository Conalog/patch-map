# patch-service P0 Consumer Journeys

These are migration-blocking product journeys. They describe what a user must be able
to accomplish; plant-domain calculation, persistence, routing, dialogs, and server
commands remain host responsibilities. Core v2 supplies the mapped engine capability.
...
diagnostic output returned to host DOM UI; callback/failure/rollback ownership; and the
...
| ID | User scenario and observable outcome | Core v2 capability | Lab case |
| --- | --- | --- | --- |
...
| CSM-036 | Leave or remount an editor; host guards navigation while Core v2 removes every canvas, texture, ticker, listener, observer, gesture, and hotkey scope. | ERR-006, PRF-007 | `consumer/editor-cleanup` |
...
Core v2 must not hardcode plant wiring, registry/latest/time-series merge rules,
...
callbacks. Core v2 guarantees deterministic execution, rendering, interaction,
...
for this digest-bound integration result. Migration, canary, save guards, and rollback
follow `production-readiness.md` and MIG-001–003.
