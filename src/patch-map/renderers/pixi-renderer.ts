...
export class PatchMapPixiRenderer implements CoreRenderer {
  ...
  private presentationStoreFor(store: RenderStoreView): RenderStoreView {
    ...
    if (
      ...
      this.presentationStore = new PatchMapPresentationStoreView(store, policy);
      this.presentationBaseStore = store;
    ...
    nodes.forEach((node, index) => {
      ...
      if (container === undefined) {
        container = new Container({
          label: `PatchMap / accessibility / ${node.id}`,
        });
  ...
  private ensureAccessibilityRoot(): void {
    ...
    const root = new Container({
      label: 'PatchMap / accessibility overlay',
    });
...
function rangesTouchPatchMapRelationTopology(
  store: RenderStoreView,
  ranges: readonly SlotRange[],
