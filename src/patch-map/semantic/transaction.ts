...
const EMPTY_OPERATIONS: readonly PatchMapMutationOperation[] = Object.freeze([]);
...
function isIndexStructuralPath(path: readonly PatchMapMutationPathSegment[]): boolean {
  const root = path[0];
...
function targetKey(target: PatchMapMutationTarget): string {
  return target.kind === 'element'
    ? `element:${target.id.length}:${target.id}`
    : `component:${target.ownerId.length}:${target.ownerId}:${target.id.length}:${target.id}`;
}

function targetLabel(target: PatchMapMutationTarget): string {
  return target.kind === 'element'
    ? `element:${target.id}`
