export const CORE_V2_OPERATIONS_REVISION = 'core-v2-operations/1' as const;
export const CORE_V2_RUNTIME_DIAGNOSTICS_REVISION =
  'core-v2-runtime-diagnostics/1' as const;
export const CORE_V2_EXTRACTION_SECURITY_REVISION =
...
const DISABLED_RUNTIME_DIAGNOSTICS = Object.freeze({
  revision: CORE_V2_RUNTIME_DIAGNOSTICS_REVISION,
  enabled: false,
  capacity: 0,
...
export interface CoreV2OperationsRevisionStamp {
...
export interface CoreV2OperationalDiagnosticInput {
  readonly code: string;
  readonly category: string;
  readonly operation: string;
  readonly lifecycleGeneration?: number;
  readonly sceneRevision?: number;
  readonly revisionStamp?: CoreV2OperationsRevisionStamp;
...
export interface CoreV2SanitizedDiagnostic {
  readonly code: string;
  readonly category: string;
  readonly operation: string;
  readonly lifecycleGeneration: number;
...
export interface CoreV2OperationalEventInput {
...
export interface CoreV2OperationalEvent {
  readonly type: string;
  readonly operation: string;
  readonly revisionStamp: CoreV2OperationsRevisionStamp;
  readonly logicalId: string | null;
...
export interface CoreV2OperationalCallbackControl {
  enqueue(id: string, action: () => void): void;
}

export type CoreV2OperationalCallback = (
  event: CoreV2OperationalEvent,
  control: CoreV2OperationalCallbackControl,
) => void;

export interface CoreV2OperationalSubscription {
  readonly id: string;
  dispose(): boolean;
}

export interface CoreV2OperationalDispatchResult {
...
export interface CoreV2RuntimeDiagnosticsState {
  ...
  readonly revisions: CoreV2OperationsRevisionStamp;
...
export interface CoreV2RuntimeDiagnosticRecord extends CoreV2RuntimeDiagnosticsState {
  ...
  readonly lastError: CoreV2SanitizedDiagnostic | null;
}

export interface CoreV2RuntimeDiagnosticsSnapshot {
  readonly revision: typeof CORE_V2_RUNTIME_DIAGNOSTICS_REVISION;
  readonly enabled: boolean;
  readonly capacity: number;
  readonly records: readonly CoreV2RuntimeDiagnosticRecord[];
  readonly current: CoreV2RuntimeDiagnosticRecord | null;
}

export interface CoreV2OperationsProbe {
  readonly revision: typeof CORE_V2_OPERATIONS_REVISION;
  ...
  readonly lastCallbackFailure: CoreV2SanitizedDiagnostic | null;
}

export interface CoreV2OperationsAuthorityOptions {
  readonly collectionEnabled?: boolean;
  readonly telemetryEnabled?: boolean;
  readonly capacity?: number;
  ...
  readonly logger?: (diagnostic: CoreV2SanitizedDiagnostic) => void;
}
 ...
 */
export class CoreV2OperationsAuthority {
  ...
  private readonly logger: ((diagnostic: CoreV2SanitizedDiagnostic) => void) | null;
  private readonly diagnosticObservers = new Map<
    string,
    CallbackEntry<(diagnostic: CoreV2SanitizedDiagnostic) => void>
  >();
  private readonly telemetryObservers = new Map<string, CallbackEntry<CoreV2OperationalCallback>>();
  private readonly records: CoreV2RuntimeDiagnosticRecord[] = [];
  private readonly queuedActions: QueuedAction[] = [];
  private sequence = 0;
  private deliveryDepth = 0;
  private disposed = false;
  private lastAction: CoreV2RuntimeDiagnosticRecord['lastAction'] = null;
  private lastError: CoreV2SanitizedDiagnostic | null = null;
  private lastCallbackFailure: CoreV2SanitizedDiagnostic | null = null;

  public constructor(options: CoreV2OperationsAuthorityOptions = {}) {
  ...
  public configureInstance(instanceId: string): void {
    ...
    if (this.instanceId !== null && this.instanceId !== normalized) {
      throw new Error('Core v2 operations instance identity cannot change');
    }
  ...
  public setCollectionEnabled(enabled: boolean): boolean {
    if (typeof enabled !== 'boolean') {
      throw new TypeError('Core v2 operations collection flag must be boolean');
    }
  ...
  public setTelemetryEnabled(enabled: boolean): boolean {
    if (typeof enabled !== 'boolean') {
      throw new TypeError('Core v2 operations telemetry flag must be boolean');
    }
  ...
  public reportDiagnostic(
    input: CoreV2OperationalDiagnosticInput,
  ): CoreV2SanitizedDiagnostic {
    const diagnostic = redactCoreV2OperationalDiagnostic(input);
  ...
  public emitTelemetry(
    ...
    return this.dispatchTelemetry(redactCoreV2OperationalEvent(input), false);
  }

  public captureRuntimeDiagnostics(
    state: CoreV2RuntimeDiagnosticsState,
  ): CoreV2RuntimeDiagnosticsSnapshot {
    ...
    } satisfies CoreV2RuntimeDiagnosticRecord);
  ...
  public exportEvidence(): Readonly<{
    readonly revision: typeof CORE_V2_OPERATIONS_REVISION;
    readonly diagnostics: readonly CoreV2SanitizedDiagnostic[];
    readonly runtimeRecords: readonly CoreV2RuntimeDiagnosticRecord[];
  }> {
    return deepFreeze({
      revision: CORE_V2_OPERATIONS_REVISION,
      diagnostics: this.lastError === null ? [] : [this.lastError],
      runtimeRecords: [...this.records],
    });
  }

  public probe(): CoreV2OperationsProbe {
  ...
  private registerCallback<T>(
    ...
    if (this.disposed) throw new Error(`Core v2 ${label} authority is disposed`);
    if (typeof callback !== 'function') throw new TypeError(`${label} must be a function`);
    const id = controlledValue(idValue, label);
    if (registry.has(id)) throw new Error(`Duplicate Core v2 ${label} ${id}`);
  ...
  private deliverDiagnostic(diagnostic: CoreV2SanitizedDiagnostic): void {
    ...
    try {
      for (const entry of snapshot) {
        ...
        try {
          entry.callback(event, Object.freeze({
            enqueue: (id: string, action: () => void): void => {
              if (typeof action !== 'function') {
                throw new TypeError('Core v2 queued host action must be a function');
              }
          ...
          const failure = redactCoreV2OperationalDiagnostic({
...
export type CoreV2ExtractionReadability =
  | 'readable'
  | 'tainted'
  | 'readback-failed';

export interface CoreV2ExtractionSecurityProbe {
  readonly revision: typeof CORE_V2_EXTRACTION_SECURITY_REVISION;
 ...
 */
export class CoreV2ExtractionSecurityAuthority {
  private readonly assets = new Map<string, CoreV2ExtractionReadability>();

  public setAssetReadability(
    logicalAssetId: string,
    readability: CoreV2ExtractionReadability,
  ): void {
    const id = controlledValue(logicalAssetId, 'asset');
    if (!['readable', 'tainted', 'readback-failed'].includes(readability)) {
      throw new TypeError('Unknown Core v2 extraction readability');
    }
  ...
  public preflight(): CoreV2ExtractionSecurityProbe {
    let firstFailed: readonly [string, CoreV2ExtractionReadability] | null = null;
    ...
    return Object.freeze({
      revision: CORE_V2_EXTRACTION_SECURITY_REVISION,
...
export function redactCoreV2OperationalDiagnostic(
  input: CoreV2OperationalDiagnosticInput,
): CoreV2SanitizedDiagnostic {
...
export function redactCoreV2OperationalEvent(
  input: CoreV2OperationalEventInput,
): CoreV2OperationalEvent {
  const stable = {
...
function telemetryFromDiagnostic(
  diagnostic: CoreV2SanitizedDiagnostic,
): CoreV2OperationalEvent {
  return deepFreeze({
    type: 'diagnostic',
...
function emptyDispatchResult(): CoreV2OperationalDispatchResult {
  return Object.freeze({
    deliveredCount: 0,
...
function normalizeRevisionStamp(
  value: CoreV2OperationsRevisionStamp,
): CoreV2OperationsRevisionStamp {
  return Object.freeze({
...
function normalizeCapacity(value: number): number {
  if (!Number.isSafeInteger(value) || value < 1 || value > MAX_DIAGNOSTIC_CAPACITY) {
    throw new RangeError(`Core v2 diagnostic capacity must be between 1 and ${MAX_DIAGNOSTIC_CAPACITY}`);
  }
  return value;
}
