export const CORE_V2_POINTER_GESTURE_REVISION = 'core-v2-pointer-gesture/1' as const;

export type CoreV2PointerInputType =
  | 'down'
  | 'move'
  | 'up'
  | 'up-outside'
...
export interface CoreV2PointerModifiers {
...
export interface CoreV2PointerInput {
  readonly type: CoreV2PointerInputType;
  ...
  readonly modifiers: CoreV2PointerModifiers;
  readonly viewRevision: number;
}

export type CoreV2SemanticPointerEventType =
  | 'down'
  | 'up'
  | 'click'
  | 'drag-start'
...
export interface CoreV2SemanticPointerPayload {
...
export interface CoreV2SemanticPointerEvent {
  readonly type: CoreV2SemanticPointerEventType;
  readonly payload: CoreV2SemanticPointerPayload;
}

export interface CoreV2PointerDispatchResult {
  readonly events: readonly CoreV2SemanticPointerEvent[];
  readonly hoverTarget: string | null;
  readonly clickSuppressed: boolean;
  readonly semanticCompletionCount: number;
}

export interface CoreV2PointerGestureProbe {
...
export interface CoreV2PointerGestureOptions {
...
export type CoreV2OwnedGestureKind =
  | 'click'
  | 'box'
  | 'paint'
  | 'pan'
...
export type CoreV2GestureCancelReason =
  | 'escape'
  | 'pointer-cancel'
  | 'lost-capture'
  | 'blur'
...
export type CoreV2GestureTerminationReason =
  | 'pointer-up-outside'
  | CoreV2GestureCancelReason;

export interface CoreV2OwnedGestureTermination {
  readonly kind: CoreV2OwnedGestureKind;
  readonly reason: CoreV2GestureTerminationReason;
...
export interface CoreV2RegionEntityGeometry {
  readonly id: string;
...
export interface CoreV2RegionRelationGeometry {
  readonly id: string;
...
export interface CoreV2RegionHitResult {
 ...
 */
export class CoreV2PointerGestureAuthority {
  private readonly hitTest: CoreV2PointerGestureOptions['hitTest'];
  ...
  public constructor(options: CoreV2PointerGestureOptions) {
  ...
  public beginOwnedGesture(kind: CoreV2OwnedGestureKind, pointerId: number): void {
    if (this.destroyed) throw new Error('Core v2 pointer gesture authority is destroyed');
    validatePointerId(pointerId);
    if (this.ownedGesture !== null) {
      throw new Error('Core v2 pointer gesture authority already owns a gesture');
    }
  ...
  public cancelOwnedGesture(reason: CoreV2GestureCancelReason): CoreV2OwnedGestureTermination | null {
  ...
  public interrupt(reason: CoreV2GestureCancelReason): CoreV2OwnedGestureTermination | null {
  ...
  public probe(): CoreV2PointerGestureProbe {
    return Object.freeze({
  ...
  private pointerDown(input: CoreV2PointerInput): CoreV2PointerDispatchResult {
  ...
  private pointerMove(input: CoreV2PointerInput): CoreV2PointerDispatchResult {
    ...
    if (!this.hoverDuringPress) this.hoverTarget = null;
    const events: CoreV2SemanticPointerEvent[] = [];
  ...
  private pointerUp(input: CoreV2PointerInput): CoreV2PointerDispatchResult {
    const active = this.activePointers.get(input.pointerId);
    if (active === undefined) {
      this.staleGestureCount += 1;
      return emptyDispatchResult(this.hoverTarget);
  ...
  private pointerCancel(input: CoreV2PointerInput): CoreV2PointerDispatchResult {
    const active = this.activePointers.get(input.pointerId);
  ...
  private pointerLeave(input: CoreV2PointerInput): CoreV2PointerDispatchResult {
  ...
  private nextClickCount(active: ActivePointer, input: CoreV2PointerInput): number {
...
export function hitCoreV2BoxRegion(
  entities: readonly CoreV2RegionEntityGeometry[],
  relations: readonly CoreV2RegionRelationGeometry[],
  start: readonly [number, number],
  end: readonly [number, number],
  options: Readonly<{ readonly partialIntersection?: boolean }> = {},
): CoreV2RegionHitResult {
...
export function hitCoreV2PaintRegion(
  entities: readonly CoreV2RegionEntityGeometry[],
  relations: readonly CoreV2RegionRelationGeometry[],
...
function normalizePointerInput(input: CoreV2PointerInput): CoreV2PointerInput {
...
function isPointerInputType(value: string): value is CoreV2PointerInputType {
  return value === 'down' ||
    value === 'move' ||
    value === 'up' ||
    value === 'up-outside' ||
...
function semanticPointerEvent(
  type: CoreV2SemanticPointerEventType,
  input: CoreV2PointerInput,
  targetId: string | null,
  clickCount: number,
): CoreV2SemanticPointerEvent {
  return Object.freeze({
    type,
...
function dispatchResult(
  events: readonly CoreV2SemanticPointerEvent[],
  hoverTarget: string | null,
  clickSuppressed: boolean,
): CoreV2PointerDispatchResult {
  return Object.freeze({
...
function emptyDispatchResult(hoverTarget: string | null): CoreV2PointerDispatchResult {
...
function relationPoints(
  relation: CoreV2RegionRelationGeometry,
