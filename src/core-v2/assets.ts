...
export interface CoreV2PixiAssetBackendOptions {
   ...
   * `createCoreV2AssetIngestionPolicy(profile)` so denied URLs cannot reach
   * the backend lookup/fetch boundary.
   */
  readonly ingestionPolicy?: CoreV2AssetIngestionPolicyProfile;
  readonly inspectDecodedSize?: (
    blob: Blob,
  ) => Promise<Readonly<{ readonly width: number; readonly height: number }>>;
}

export interface CoreV2AssetIngestionPolicyProfile {
...
export interface CoreV2AssetResponseMetadata {
...
export interface CoreV2AssetIngestionDecision {
 ...
 */
export function createCoreV2AssetIngestionPolicy(
...
export function evaluateCoreV2AssetResponsePolicy(
  profile: CoreV2AssetIngestionPolicyProfile,
  metadata: CoreV2AssetResponseMetadata,
): CoreV2AssetIngestionDecision {
  const normalized = normalizeIngestionProfile(profile);
  try {
    ...
    if (
      error instanceof CoreV2AssetError &&
...
export function assertCoreV2AssetResponseAllowed(
  profile: CoreV2AssetIngestionPolicyProfile,
  metadata: CoreV2AssetResponseMetadata,
): void {
  const decision = evaluateCoreV2AssetResponsePolicy(profile, metadata);
  ...
  return Object.freeze({
    ...
    async load(request: CoreV2AssetBackendRequest): Promise<unknown> {
      if (options.ingestionPolicy && !request.packageOwned) {
        await createCoreV2AssetIngestionPolicy(options.ingestionPolicy)(Object.freeze({
          instanceId: 'core-v2-pixi-backend',
...
async function isolatedPixiDescriptor(
  ...
  ingestionPolicy?: CoreV2AssetIngestionPolicyProfile,
  inspectDecodedSize?: (
    blob: Blob,
  ) => Promise<Readonly<{ readonly width: number; readonly height: number }>>,
  ...
  if (ingestionPolicy) {
    ...
    assertCoreV2AssetResponseAllowed(ingestionPolicy, {
...
function rejectedAssetDecision(
  stage: Exclude<CoreV2AssetIngestionDecision['stage'], 'accepted'>,
): CoreV2AssetIngestionDecision {
  return Object.freeze({
    accepted: false,
...
function policyRejected(stage: 'descriptor' | 'redirect'): CoreV2AssetError {
  const error = new CoreV2AssetError('ASSET_POLICY_REJECTED', 'ASSET_FAILURE', false);
