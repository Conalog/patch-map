...
export const CORE_V2_HOST_ASSET_INGESTION_REVISION =
  'core-v2-host-asset-ingestion/1' as const;

export interface CoreV2HostPreparedImage {
...
export type CoreV2HostAssetIngestionInput =
  | Readonly<{
      readonly kind: 'text';
  ...
  | Readonly<{
      ...
      readonly files: readonly CoreV2HostPreparedImage[];
      readonly targetWorld: readonly [number, number];
...
export interface CoreV2HostAssetIngestionProbe {
...
export type CoreV2HostAssetIngestionPlan =
  | PlannedIntake
  | Readonly<{
 ...
 */
export class CoreV2HostAssetIngestionAuthority {
  ...
  public plan(
    ...
    inputValue: CoreV2HostAssetIngestionInput,
  ): CoreV2HostAssetIngestionPlan {
  ...
  public commit(plan: CoreV2HostAssetIngestionPlan): void {
    if (plan.status !== 'planned') return;
    this.textSequence = plan.nextTextSequence;
    this.imageSequence = plan.nextImageSequence;
  }

  public probe(): CoreV2HostAssetIngestionProbe {
    return Object.freeze({
  ...
  return Object.freeze({
    ...
    history: Object.freeze({
      ...
      hostAssetIngestion: Object.freeze({
        revision: CORE_V2_HOST_ASSET_INGESTION_REVISION,
        source,
      }),
    }),
  });
...
function imageElement(
  ...
  const source = file.source === undefined
    ...
    : typeof file.source === 'string'
      ? file.source
      : normalizeCoreV2AssetDescriptor(file.source);
  return Object.freeze({
    type: 'image',
    id,
    source: detachCoreV2MutationJsonValue(source),
...
function normalizeInput(
  value: CoreV2HostAssetIngestionInput,
): CoreV2HostAssetIngestionInput {
...
function normalizeFile(value: unknown, index: number): CoreV2HostPreparedImage {
  ...
  const source = value.source as CoreV2AssetSource | undefined;
  if (source !== undefined) {
    if (typeof source === 'string') nonempty(source, `host image file ${index} source`);
    else normalizeCoreV2AssetDescriptor(source);
  }
  return Object.freeze({
    name,
    mime,
    ...
    ...(source === undefined
      ...
      : {
          source: typeof source === 'string'
            ? source
            : normalizeCoreV2AssetDescriptor(source),
        }),
  });
}
