...
export const CORE_V2_EDITOR_WORKFLOW_REVISION =
  'core-v2-editor-workflow/1' as const;

export const CORE_V2_EDITOR_MUTATION_KINDS = Object.freeze([
  'create',
  'move',
  'resize',
  'rotate',
...
export type CoreV2EditorMutationKind =
  (typeof CORE_V2_EDITOR_MUTATION_KINDS)[number];

export type CoreV2EditorWorkflowMode =
  | 'select'
  | 'grid-edit'
  | 'relation-edit'
  | 'text-edit';

export type CoreV2EditorWorkflowAction =
  | Readonly<{
...
export type CoreV2EditorWorkflowDiagnosticCode =
  | 'INVALID_VALUE'
  | 'INVALID_MUTATION'
  | 'MISSING_TARGET'
  | 'CONFLICT';

export interface CoreV2EditorWorkflowDiagnostic {
  readonly code: CoreV2EditorWorkflowDiagnosticCode;
  readonly path: readonly (string | number)[];
  readonly message: string;
}

export type CoreV2EditorWorkflowFacts =
  Readonly<Record<string, CoreV2MutationJsonValue>>;

interface CoreV2EditorWorkflowPlanBase {
...
export type CoreV2EditorWorkflowPlan =
  | Readonly<CoreV2EditorWorkflowPlanBase & {
      readonly status: 'planned';
      readonly changed: true;
      readonly transaction: CoreV2MutationTransactionRequest | null;
    }>
  | Readonly<CoreV2EditorWorkflowPlanBase & {
  ...
  | Readonly<CoreV2EditorWorkflowPlanBase & {
      ...
      readonly changed: false;
      readonly transaction: null;
      readonly diagnostic: CoreV2EditorWorkflowDiagnostic;
    }>;

export interface CoreV2EditorWorkflowProbe {
  readonly schemaRevision: typeof CORE_V2_EDITOR_WORKFLOW_REVISION;
  readonly mode: CoreV2EditorWorkflowMode;
...
const EMPTY_FACTS: CoreV2EditorWorkflowFacts = Object.freeze({});
 ...
 */
export class CoreV2EditorWorkflowAuthority {
  private state: WorkflowState = INITIAL_STATE;
  private readonly pending = new WeakMap<CoreV2EditorWorkflowPlan, PendingPlan>();
  private destroyed = false;

  public plan(
    materialized: MaterializedCoreV2Dataset,
    input: CoreV2EditorWorkflowAction,
  ): CoreV2EditorWorkflowPlan {
    this.assertAlive();
  ...
  public commit(plan: CoreV2EditorWorkflowPlan): boolean {
  ...
  public discard(plan: CoreV2EditorWorkflowPlan): boolean {
  ...
  public probe(): CoreV2EditorWorkflowProbe {
  ...
  private planSelection(
    materialized: MaterializedCoreV2Dataset,
    action: Extract<CoreV2EditorWorkflowAction, { readonly type: 'select-targets' }>,
  ): CoreV2EditorWorkflowPlan {
  ...
  private planEnterGrid(
    materialized: MaterializedCoreV2Dataset,
    action: Extract<CoreV2EditorWorkflowAction, { readonly type: 'enter-grid-edit' }>,
  ): CoreV2EditorWorkflowPlan {
  ...
  private planRevealGrid(
    action: Extract<CoreV2EditorWorkflowAction, { readonly type: 'reveal-inactive-cells' }>,
  ): CoreV2EditorWorkflowPlan {
  ...
  private planResizeGrid(
    materialized: MaterializedCoreV2Dataset,
    action: Extract<CoreV2EditorWorkflowAction, { readonly type: 'resize-grid' }>,
  ): CoreV2EditorWorkflowPlan {
    const session = this.state.grid;
  ...
  private planGridCell(
    materialized: MaterializedCoreV2Dataset,
    action: Extract<CoreV2EditorWorkflowAction, { readonly type: 'set-grid-cell-active' }>,
  ): CoreV2EditorWorkflowPlan {
    const session = this.state.grid;
    if (session === null) {
      return rejected(action, 'MISSING_TARGET', ['target'], 'grid edit session is not active');
  ...
  private planExitGrid(
    materialized: MaterializedCoreV2Dataset,
    action: Extract<CoreV2EditorWorkflowAction, { readonly type: 'exit-grid-edit' }>,
  ): CoreV2EditorWorkflowPlan {
    const session = this.state.grid;
    if (
      session === null ||
  ...
  private planEnterRelation(
    materialized: MaterializedCoreV2Dataset,
    action: Extract<CoreV2EditorWorkflowAction, { readonly type: 'enter-relation-edit' }>,
  ): CoreV2EditorWorkflowPlan {
  ...
  private planExitRelation(
    materialized: MaterializedCoreV2Dataset,
    action: Extract<CoreV2EditorWorkflowAction, { readonly type: 'exit-relation-edit' }>,
  ): CoreV2EditorWorkflowPlan {
    const session = this.state.relation;
    const relation = relationsElement(materialized, action.relationId);
    if (session === null || session.target !== action.relationId || relation === null) {
  ...
  private planOpenText(
    materialized: MaterializedCoreV2Dataset,
    action: Extract<CoreV2EditorWorkflowAction, { readonly type: 'open-text-editor' }>,
  ): CoreV2EditorWorkflowPlan {
  ...
  private planCommitText(
    materialized: MaterializedCoreV2Dataset,
    action: Extract<CoreV2EditorWorkflowAction, { readonly type: 'commit-text-edit' }>,
  ): CoreV2EditorWorkflowPlan {
    const text = textElement(materialized, action.target);
    const session = this.state.text;
  ...
  private planCancelText(
    action: Extract<CoreV2EditorWorkflowAction, { readonly type: 'cancel-text-edit' }>,
  ): CoreV2EditorWorkflowPlan {
    const session = this.state.text;
    if (session === null || session.target !== action.target) {
  ...
  private planDeleteRequest(
    materialized: MaterializedCoreV2Dataset,
    action: Extract<CoreV2EditorWorkflowAction, { readonly type: 'request-delete-plan' }>,
  ): CoreV2EditorWorkflowPlan {
    const missing = action.targets.find((id) => findElement(materialized.dataset, id) === null);
    if (missing !== undefined) {
  ...
  private planDeleteTransaction(
    materialized: MaterializedCoreV2Dataset,
    action: Extract<CoreV2EditorWorkflowAction, { readonly type: 'delete-transaction' }>,
  ): CoreV2EditorWorkflowPlan {
    const deletion = this.state.deletion;
  ...
  private pendingPlan(
    ...
    transactionValue: CoreV2MutationTransactionRequest | null,
    facts: CoreV2EditorWorkflowFacts,
    next: WorkflowState,
    effects: Readonly<{
      readonly selectionIds?: readonly string[];
      readonly closeHistoryGroup?: boolean;
    ...
    } satisfies CoreV2EditorWorkflowPlan);
...
export function planCoreV2EditorMatrixMutation(
  materialized: MaterializedCoreV2Dataset,
  kind: CoreV2EditorMutationKind,
  companion: CoreV2MutationJsonValue,
): CoreV2MutationTransactionRequest {
  ...
  let operation: CoreV2MutationOperation;
  ...
  switch (kind) {
    ...
    case 'rotate':
      ...
      break;
    case 'grid': {
      const grid = requireElement(materialized, 'matrix-grid', 'grid') as CoreV2GridElement;
    ...
    case 'relation': {
      const links = (relation as CoreV2RelationsElement).links;
    ...
    case 'text':
      operation = mergeElement('text-c', [
        change(['text'], `${(text as CoreV2TextElement).text} matrix`),
      ]);
      break;
    case 'style':
    ...
    case 'duplicate': {
      ...
      const duplicate = detachCoreV2MutationJsonValue(location.element);
  ...
  return Object.freeze({
    ...
    history: detachCoreV2MutationJsonValue(companion, '$.editorMatrixCompanion'),
  });
}
...
function workflowTransaction(
  ...
  editorMode: CoreV2EditorWorkflowMode,
  details: CoreV2MutationJsonValue,
): CoreV2MutationTransactionRequest {
  return Object.freeze({
    ...
    history: Object.freeze({
      ...
      editorWorkflow: Object.freeze({
        revision: CORE_V2_EDITOR_WORKFLOW_REVISION,
        editorMode,
        details,
      }),
    }),
...
function rejected(
  action: CoreV2EditorWorkflowAction,
  code: CoreV2EditorWorkflowDiagnosticCode,
...
function normalizeAction(value: CoreV2EditorWorkflowAction): CoreV2EditorWorkflowAction {
...
function change(
  ...
  return Object.freeze({
    path: Object.freeze([...path]),
    value: detachCoreV2MutationJsonValue(value),
  });
}

function removeElement(id: string): CoreV2MutationOperation {
  return Object.freeze({
    op: 'remove',
    target: Object.freeze({ kind: 'element', id }),
    cascade: 'subtree',
...
function matrixGrid(): Readonly<Record<string, CoreV2MutationJsonValue>> {
  return Object.freeze({
    type: 'grid',
    id: 'matrix-grid',
...
function resizeGridCells(
  source: CoreV2GridElement['cells'],
  rows: number,
  columns: number,
