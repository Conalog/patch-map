...
export const CORE_V2_AUTHORING_REVISION = 'core-v2-authoring/1' as const;

export type CoreV2AuthoringAction =
  | Readonly<{
      readonly type: 'create-element';
      readonly kind: CoreV2ElementType;
      readonly id: string;
      readonly positionWorld: CoreV2PointTuple;
  ...
  | Readonly<{
      ...
      readonly axis: CoreV2AuthoringAlignmentAxis;
      readonly actionId: string;
    }>
  | Readonly<{
      readonly type: 'distribute-targets';
      readonly targets: readonly string[];
      readonly axis: CoreV2AuthoringDistributionAxis;
  ...
  | Readonly<{
      ...
      readonly target: string;
      readonly changes: Readonly<Record<string, CoreV2MutationJsonValue>>;
  ...
  | Readonly<{
      ...
      readonly offsetWorld: CoreV2PointTuple;
...
export type CoreV2AuthoringActionType = CoreV2AuthoringAction['type'];
export type CoreV2AuthoringAlignmentAxis =
  | 'left'
  | 'right'
  | 'top'
  | 'bottom'
  | 'center-x'
  | 'center-y';
export type CoreV2AuthoringDistributionAxis = 'horizontal' | 'vertical';
export type CoreV2AuthoringDiagnosticCode =
  | 'DUPLICATE_ID'
  | 'INVALID_MUTATION'
  | 'INVALID_VALUE'
  | 'MISSING_TARGET';

export interface CoreV2AuthoringDiagnostic {
  readonly code: CoreV2AuthoringDiagnosticCode;
  readonly path: readonly (string | number)[];
  readonly message: string;
}

export interface CoreV2AuthoringPlanningContext {
  readonly selectionIds: readonly string[];
}

export type CoreV2AuthoringFacts = Readonly<Record<string, CoreV2MutationJsonValue>>;

interface CoreV2AuthoringPlanBase {
  readonly schemaRevision: typeof CORE_V2_AUTHORING_REVISION;
  readonly actionType: CoreV2AuthoringActionType | null;
  readonly action: CoreV2AuthoringAction | null;
  readonly facts: CoreV2AuthoringFacts;
}

export type CoreV2AuthoringPlan =
  | Readonly<CoreV2AuthoringPlanBase & {
  ...
  | Readonly<CoreV2AuthoringPlanBase & {
      ...
      readonly changed: false;
      readonly transaction: null;
      readonly diagnostic: CoreV2AuthoringDiagnostic;
    }>;

interface AuthoringElementLocation {
  readonly element: NormalizedCoreV2Element;
  readonly parentId: string | null;
  readonly siblings: readonly NormalizedCoreV2Element[];
  readonly index: number;
...
interface AuthoringGeometry {
  ...
  readonly bounds: CoreV2BoundsTuple;
}
...
const ALIGNMENT_AXIS_SET = new Set<CoreV2AuthoringAlignmentAxis>([
  'left',
  'right',
  'top',
  'bottom',
...
const DISTRIBUTION_AXIS_SET = new Set<CoreV2AuthoringDistributionAxis>([
  'horizontal',
  'vertical',
]);
const EMPTY_FACTS: CoreV2AuthoringFacts = Object.freeze({});
 ...
 */
export function planCoreV2AuthoringAction(
  current: MaterializedCoreV2Dataset,
  actionInput: unknown,
  contextInput: CoreV2AuthoringPlanningContext,
): CoreV2AuthoringPlan {
  let action: CoreV2AuthoringAction;
  let context: CoreV2AuthoringPlanningContext;
  try {
...
function planCreate(
  action: Extract<CoreV2AuthoringAction, { readonly type: 'create-element' }>,
  ...
  const parentPoint = applyCoreV2Affine(
    invertCoreV2Affine(destination.parentAffine),
    action.positionWorld,
  );
  const element = createElementRecord(action.kind, action.id, parentPoint);
  const operation: CoreV2MutationOperation = Object.freeze({
    op: 'add',
...
function planPositionEdit(
  action: Extract<CoreV2AuthoringAction, { readonly type: 'edit-position-angle' }>,
  index: ReadonlyMap<string, AuthoringElementLocation>,
): CoreV2AuthoringPlan {
...
function planAlignment(
  action: Extract<CoreV2AuthoringAction, { readonly type: 'align-targets' }>,
  index: ReadonlyMap<string, AuthoringElementLocation>,
): CoreV2AuthoringPlan {
...
function planDistribution(
  action: Extract<CoreV2AuthoringAction, { readonly type: 'distribute-targets' }>,
  index: ReadonlyMap<string, AuthoringElementLocation>,
): CoreV2AuthoringPlan {
...
function planStyle(
  action: Extract<CoreV2AuthoringAction, { readonly type: 'apply-style' }>,
  index: ReadonlyMap<string, AuthoringElementLocation>,
): CoreV2AuthoringPlan {
  const location = requireLocation(index, action.target, ['target']);
  assertUnlocked(location, ['target']);
...
function planHierarchyMove(
  action: Extract<CoreV2AuthoringAction, { readonly type: 'move-hierarchy' }>,
  index: ReadonlyMap<string, AuthoringElementLocation>,
  context: CoreV2AuthoringPlanningContext,
): CoreV2AuthoringPlan {
...
function planReorder(
  action: Extract<CoreV2AuthoringAction, { readonly type: 'reorder-z' }>,
  index: ReadonlyMap<string, AuthoringElementLocation>,
): CoreV2AuthoringPlan {
  ...
  const operations: CoreV2MutationOperation[] = action.placement === 'front'
...
function planGroup(
  action: Extract<CoreV2AuthoringAction, { readonly type: 'group-targets' }>,
): CoreV2AuthoringPlan {
...
function planDuplicate(
  ...
  const frozenClone = detachCoreV2MutationJsonValue(clone, '$.duplicate');
...
function planUngroup(
  action: Extract<CoreV2AuthoringAction, { readonly type: 'ungroup-target' }>,
  index: ReadonlyMap<string, AuthoringElementLocation>,
  context: CoreV2AuthoringPlanningContext,
): CoreV2AuthoringPlan {
  const location = requireLocation(index, action.target, ['target']);
...
function normalizeAction(value: unknown): CoreV2AuthoringAction {
  ...
  switch (type) {
    case 'create-element': {
      ...
      return Object.freeze({
        type,
        kind: kindValue as CoreV2ElementType,
    ...
    case 'align-targets': {
      ...
      if (!ALIGNMENT_AXIS_SET.has(axis as CoreV2AuthoringAlignmentAxis)) {
        fail('INVALID_VALUE', ['axis'], `Unsupported alignment axis ${axis}`);
      }
      return Object.freeze({
        type,
        targets: stringArray(record.targets, ['targets']),
        axis: axis as CoreV2AuthoringAlignmentAxis,
        actionId: nonEmptyString(record.actionId, ['actionId']),
      });
    }
    case 'distribute-targets': {
      ...
      const axis = nonEmptyString(record.axis, ['axis']);
      if (!DISTRIBUTION_AXIS_SET.has(axis as CoreV2AuthoringDistributionAxis)) {
      ...
      return Object.freeze({
        type,
        targets: stringArray(record.targets, ['targets']),
        axis: axis as CoreV2AuthoringDistributionAxis,
        basis: 'bounds',
        actionId: nonEmptyString(record.actionId, ['actionId']),
      });
    }
    case 'apply-style': {
      ...
      const changes = detachCoreV2MutationJsonValue(record.changes, '$.changes');
...
function normalizeContext(value: unknown): CoreV2AuthoringPlanningContext {
...
function createElementRecord(
  kind: CoreV2ElementType,
  id: string,
  position: CoreV2PointTuple,
...
function defaultElementSize(kind: CoreV2ElementType): CoreV2PointTuple {
  switch (kind) {
    case 'item':
...
function destinationForParent(
  ...
  index: ReadonlyMap<string, AuthoringElementLocation>,
  dataset: readonly NormalizedCoreV2Element[],
): Readonly<{ readonly parentAffine: CoreV2AffineMatrix; readonly childCount: number }> {
  if (parentId === null) {
    return Object.freeze({
...
function indexAuthoringElements(
  ...
  const visit = (
    ...
    elements.forEach((element, elementIndex) => {
      const worldAffine = multiplyCoreV2Affine(parentAffine, elementLocalAffine(element));
...
function elementLocalAffine(element: NormalizedCoreV2Element): CoreV2AffineMatrix {
...
function requireGeometries(
  ...
  return Object.freeze(targets.map((target, targetIndex) => {
    ...
    const corners = coreV2AffineCorners(
...
function elementSize(element: NormalizedCoreV2Element): CoreV2PointTuple | null {
  switch (element.type) {
    case 'item':
    case 'rect':
...
function alignmentAnchor(
  axis: CoreV2AuthoringAlignmentAxis,
...
function validateStyleChanges(
  changes: Readonly<Record<string, CoreV2MutationJsonValue>>,
): void {
...
function buildDuplicateIdMap(
  root: NormalizedCoreV2Element,
  rootId: string,
): ReadonlyMap<string, string> {
  const result = new Map<string, string>();
  const visit = (element: NormalizedCoreV2Element, isRoot: boolean): void => {
...
function offsetClonedRoot(
  ...
  parentAffine: CoreV2AffineMatrix,
  offsetWorld: CoreV2PointTuple,
): void {
  const inverse = invertCoreV2Affine(parentAffine);
...
function componentIds(
  element: Readonly<Record<string, CoreV2MutationJsonValue>>,
): readonly string[] {
  ...
  const components = source as readonly CoreV2MutationJsonValue[];
...
function plannedPlan(
  action: CoreV2AuthoringAction,
  operations: readonly CoreV2MutationOperation[],
  selectedIds: readonly string[],
  planFacts: CoreV2AuthoringFacts,
): CoreV2AuthoringPlan {
  if (operations.length === 0) return unchangedPlan(action, planFacts);
  const transaction: CoreV2MutationTransactionRequest = Object.freeze({
...
function rejectedPlan(
  action: CoreV2AuthoringAction | null,
  planDiagnostic: CoreV2AuthoringDiagnostic,
): CoreV2AuthoringPlan {
  return Object.freeze({
    schemaRevision: CORE_V2_AUTHORING_REVISION,
...
): CoreV2AuthoringFacts {
  const detached = detachCoreV2MutationJsonValue(value, '$.facts');
...
function pathChangeIfDifferent(
  current: unknown,
  next: CoreV2MutationJsonValue,
  path: readonly (string | number)[],
): Readonly<{ readonly path: readonly (string | number)[]; readonly value: CoreV2MutationJsonValue }> | null {
...
function fail(
  code: CoreV2AuthoringDiagnosticCode,
  path: readonly (string | number)[],
  message: string,
): never {
...
class AuthoringValidationFailure extends Error {
  public readonly diagnostic: CoreV2AuthoringDiagnostic;

  public constructor(diagnostic: CoreV2AuthoringDiagnostic) {
