...
export function commitTransactionOperations(
  ...
  return projectTransactionResult(host.transact({
    ...
    ...(options.selectedIds === undefined && options.companion === undefined
      ? {}
      : {
...
function projectTransactionResult(result: PatchMapHostTransactionResult): PatchMapUpdateResult {
...
function projectInstanceResult(
  result: PatchMapHostInstanceBarHeightResult,
): PatchMapUpdateResult {
  return Object.freeze({
    ...
    diagnostic: result.status === 'rejected' ? result.diagnostic ?? null : null,
  });
}
