...
import type {
  PatchMapHostDebugSnapshot,
  PatchMapHostExtractionResult,
  PatchMapHostInstanceBarHeightBatchRequest,
  PatchMapHostInstanceBarHeightResult,
  PatchMapHostLogicalPresentationLayerInput,
  PatchMapHostLoadOptions,
  PatchMapHostLoadResult,
  PatchMapHostQueryResult,
  PatchMapHostPresentationLayerChange,
  PatchMapHostTransactionResult,
  PatchMapHostViewportChangeResult,
  PatchMapHostViewportFitOptions,
  PatchMapHostViewportFitResult,
  PatchMapHostViewportRestoreResult,
  PatchMapHostViewportState,
} from './host-contracts';
import { serializePatchMapDataset } from '../semantic/persistence';
import type {
...
import {
  createPatchMapEditorApi,
  type PatchMapEditorHost,
} from './editor';
import {
  createPatchMapHistoryApi,
  type PatchMapHistoryHost,
} from './history';
import { createPatchMapMutationApi } from './mutations';
import { createPatchMapPresentationApi } from './presentation';
import {
  createPatchMapTransformApi,
  type PatchMapTransformHost,
} from './transform';
...
interface PatchMapApiHost extends PatchMapTransformHost, PatchMapEditorHost, PatchMapHistoryHost {
  ...
  loadDatasetAsync(
    input: unknown,
    options?: PatchMapHostLoadOptions,
  ): Promise<PatchMapHostLoadResult>;
  exportDataset(): readonly unknown[];
  transact(request: PatchMapMutationTransactionRequest): PatchMapHostTransactionResult;
  updateBarHeights(request: PatchMapBarHeightBatchRequest): PatchMapHostTransactionResult;
  updateInstanceBarHeights(
    request: PatchMapHostInstanceBarHeightBatchRequest,
  ): PatchMapHostInstanceBarHeightResult;
  updateTexts(request: PatchMapTextBatchRequest): PatchMapHostTransactionResult;
  queryScene(input?: PatchMapSceneQuery): PatchMapHostQueryResult;
  reuseQueryResult(
    result: PatchMapHostQueryResult,
  ...
  restoreViewport(
    input: unknown,
    fallback?: PatchMapHostViewportFitOptions,
  ): PatchMapHostViewportRestoreResult;
  panViewport(delta: readonly [number, number]): PatchMapHostViewportChangeResult;
  setViewportAbsolute(input: PatchMapViewportSnapshot): PatchMapHostViewportChangeResult;
  ...
  captureManagedPng(): Promise<PatchMapHostExtractionResult>;
  snapshot(): PatchMapHostDebugSnapshot;
  setPresentationLayer(
    input: PatchMapHostLogicalPresentationLayerInput,
  ): PatchMapHostPresentationLayerChange;
  clearPresentationLayer(key: string): PatchMapHostPresentationLayerChange;
}

interface TargetSetAuthority {
  readonly query: PatchMapHostQueryResult;
...
function engineLoadOptions(options: PatchMapDataReplaceOptions): PatchMapHostLoadOptions {
  return Object.freeze({
...
function dataReplaceResult(result: PatchMapHostLoadResult): PatchMapDataReplaceResult {
  return Object.freeze({
...
export function createPatchMapApi(host: PatchMapApiHost): PatchMapApi {
  ...
  const fit = (options: PatchMapFitOptions = {}): PatchMapHostViewportFitResult =>
    host.fitViewport(engineFitOptions(options));

  const replaceFitOptions = (
    options: PatchMapDataReplaceOptions,
  ): PatchMapHostViewportFitOptions | null => options.fit === false
    ? null
  ...
  const editor = createPatchMapEditorApi(host);
  ...
  const transform = createPatchMapTransformApi(
    host,
    (targets) => resolveSelectionIds(targets, targetSets),
  );

  const viewportSettledListeners = new Set<(state: PatchMapHostViewportState) => void>();
  ...
  const viewport = Object.freeze({
    ...
    zoomBy(factor: number, anchor?: readonly [number, number]): PatchMapHostViewportChangeResult {
    ...
    restore(snapshot: PatchMapViewportSnapshot): PatchMapHostViewportChangeResult {
    ...
    onSettled(listener: (state: PatchMapHostViewportState) => void): () => void {
    ...
    get state(): PatchMapHostViewportState {
