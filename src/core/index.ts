...
import type { PatchMapInteractionOverlayPolicy } from '../rendering-port';
...
import { PatchMapRootInteractionAuthority } from './root-interaction-authority';
import { PatchMapBarPresentationAuthority } from './bar-presentation-authority';
import { PatchMapRendererLease } from './renderer-lease';
import {
  PatchMapLoadAuthority,
  type PatchMapCooperativeLoadHooks,
} from './load-authority';
import { PatchMapReconcilePublicationCoordinator } from './reconcile-publication-coordinator';
import {
  resolvePresentationFillOverrides,
  semanticPresentationFillDenseIds,
  semanticSelectionDenseIds,
} from './semantic-dense-planning';
import {
  intrinsicImageProjectionUpdate,
  resolvedIntrinsicImageSizes,
  type PatchMapIntrinsicImageGeometry,
} from './intrinsic-image-projection';
import {
  preparePatchMapIncrementalPreview,
  preparePatchMapSemanticRefresh,
  preparePatchMapTransientDirtyRanges,
} from './transient-projection-planning';
import { PatchMapFramePublicationAuthority } from './frame-publication-authority';
import { PatchMapDenseCommitCoordinator } from './dense-commit-coordinator';
import {
  createPatchMapRuntimeRendererPort,
  type PatchMapRuntimeInitializationMetrics,
  type PatchMapRuntimeRendererBackend,
  type PatchMapRuntimeRendererPort,
...
} from './instance-presentation-coordinator';
import {
...
export class PatchMapRuntime {
  public readonly renderer: PatchMapRuntimeRendererPort;
  public readonly initializationMetrics: PatchMapRuntimeInitializationMetrics;
  ...
  private readonly denseCommit: PatchMapDenseCommitCoordinator;
  ...
  private constructor(renderer: PatchMapRuntimeRendererBackend, options: PatchMapRuntimeOptions) {
    this.runtimeRenderer = createPatchMapRuntimeRendererPort(renderer);
    this.renderer = this.runtimeRenderer;
    ...
    this.denseCommit = new PatchMapDenseCommitCoordinator(
      this.publishedScene,
      this.runtimeRenderer,
      this.barPresentation,
      this.framePublication,
      ...
      this.instancePresentation,
      {
        assertAlive: () => this.assertAlive(),
        readScene: () => this.scene,
        readParseResult: () => this.parseResultValue,
        readSpatialHit: () => this.spatialHit,
        setCurrentView: (view) => {
    ...
    this.reconcilePublication = new PatchMapReconcilePublicationCoordinator(
      ...
      {
        assertAlive: () => this.assertAlive(),
        commitDenseBatch: (batch, rendererDomain) => this.denseCommit.commit(
          batch,
          rendererDomain,
          'semantic-reconcile',
        ),
  ...
  /** Attach a concrete renderer through the neutral Core boundary. */
  ...
  public reconcile(
    ...
    return this.reconcilePublication.reconcile(input, options);
  }
  ...
  public commit(batch: TransactionBatch): CommitResult {
    return this.denseCommit.commit(batch);
  }
