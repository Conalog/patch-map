import { createLayout, layoutNode } from './layout/createLayout';
...
import {
  ...
  createRenderNode,
  groupNodesByFeature,
} from './render-ir/createRenderIR';
import { diffRenderIR } from './render-ir/diffRenderIR';
import { createRenderPlan } from './render-policy/createRenderPlan';
import { RenderScheduler } from './scheduler/RenderScheduler';

export class PatchMapEngine {
  ...
  update(opts = {}) {
    ...
    const updated = updateModel(this.model, opts);
    if (updated.length > 0) {
      if (opts.deferRender) {
        this.dirty = true;
  ...
  #refreshDirtyOwners() {
    ...
    for (const ownerId of this.dirtyOwnerIds) {
      ...
      if (!owner) continue;
      layoutNode(this.model, this.layout.frames, owner);
      const ownerNode = createRenderNode(owner, this.layout.getFrame(owner.id));
  ...
  #refreshDerivedState() {
    ...
    this.model.syncCompatibilityRefs(this.layout, this.store);
    this.renderIR = createRenderIR(this.model, this.layout, {
      theme: this.theme,
    });
