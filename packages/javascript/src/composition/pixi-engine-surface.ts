...
import {
  createPatchMapPixiRuntimeAssembly,
  type PatchMapRuntimeCreationOptions,
} from './runtime';
import type { PatchMapPixiRenderer } from '../rendering/pixi-renderer';
import type {
...
import { PatchMapSurfaceGeometryAuthority } from './surface-geometry-authority';
...
export class PixiEngineSurface implements PatchMapEngineSurface {
  ...
  private readonly pixiRenderer: PatchMapPixiRenderer;
  private readonly geometryAuthority: PatchMapSurfaceGeometryAuthority;
  ...
  public load(input: unknown): void {
    ...
    this.geometryAuthority.invalidateProjection(this.core.visibleProjection);
  }
  ...
  public publishFrame(timeMs: number): void {
    ...
    if (
      ...
      this.geometryAuthority.invalidateProjection(projectionAfter);
    }
  }
  ...
  public resize(width: number, height: number, pixelRatio: number): boolean {
    ...
    if (changed) {
      this.geometryAuthority.invalidateScreen();
    }
    return changed;
  }
  ...
  public setView(view: PatchMapSurfaceView): void {
    ...
    this.geometryAuthority.updateSurfaceView(nextView);
  }
  ...
  public select(ids: readonly string[]): void {
    ...
    this.geometryAuthority.invalidateSelection();
  }
  ...
  public worldGeometrySnapshot(): PatchMapViewportGeometry {
    return this.geometryAuthority.worldGeometrySnapshot();
  }

  public geometrySnapshot(): PatchMapSurfaceGeometrySnapshot {
    return this.geometryAuthority.geometrySnapshot();
  }

  public selectionGeometries(
    selectionIds: readonly string[],
  ): readonly PatchMapSurfaceEntityGeometry[] {
    return this.geometryAuthority.selectionGeometries(selectionIds);
  }

  public queryRegionGeometry(
    bounds: PatchMapScreenRegionBounds,
  ): PatchMapSurfaceRegionGeometryCandidates {
    return this.geometryAuthority.queryRegionGeometry(bounds);
  }
  ...
  public relationHitTestScreen(
    ...
    return this.geometryAuthority.relationHitTestScreen(point, options);
  }
  ...
  public rendererPublicSurfaceProbe(): PatchMapRendererPublicSurfaceProbe {
    return this.pixiRenderer.publicSurfaceProbe();
  }
  ...
  public async destroy(): Promise<boolean> {
    ...
    try {
      ...
      this.geometryAuthority.destroy();
    }
  }
}
