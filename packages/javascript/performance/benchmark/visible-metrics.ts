...
export function createVisibleMetrics(): VisibleMetrics {
  return {
    actionToVisibleMs: [],
    frameGapsMs: [],
    bar: null,
...
export async function measureVisibleMetrics(
  engine: PatchMap,
  spec: BenchmarkSpec,
  ...
  const observation = await applyPatchMapPerformanceBulkPatch(engine, {
    ...
    strict: true,
    timeMs: 48,
    actionId: `benchmark-bulk-${spec.size}`,
    diagnostics: spec.mode === 'smoke',
  });
  visible.actionToVisibleMs.push(observation.actionToVisibleMs);
  visible.frameGapsMs.push(observation.frameGapMs);
