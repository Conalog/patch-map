// Explicit diagnostic seam for performance/mobile's representative workload.
...
import '../../../../performance/mobile/lib/geometry.dart' as reference;
  ...
  test(
    ...
    () async {
      ...
      final report = <String, Object?>{
        'protocol': 'patch-map-full-dart-pipeline-diagnostic/1',
      ...
      final destination = File(
        '../../.artifacts/performance/mobile/full-dart-pipeline-${DateTime.now().millisecondsSinceEpoch}.json',
      );
      addTearDown(() {
      ...
      for (final count in [5000, 10000])
        for (final animated in [false, true]) {
          final inputs = reference.inputs(count, 1, warmups + samples);
          final targets = [
          ...
          for (var block = 0; block < blocks; block++) {
            ...
            try {
              for (var sample = 0; sample < inputs.length; sample++) {
                ...
                if (block.isEven)
                  row['referenceProjectMs'] = _time(
                    () => reference.project(heights, 1),
                  );
                row['commitMs'] = _time(
                  () => c.updateBatch(
                    {
