...
const packageEvidence = JSON.parse(
  await readFile(
    path.join(ROOT, 'performance/patch-map/results/package-consumer.json'),
    'utf8',
  ),
);
...
const GENERATOR = path.join(
  ROOT,
  'scripts/verification/patch-map-native-release-template.mjs',
);
const VERIFIER = path.join(
  ROOT,
  'scripts/verification/patch-map-release-readiness.mjs',
);
