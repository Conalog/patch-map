...
async function runTrial(page, trialIndex) {
  await page.goto(
    `lab/core-v2?scenario=REN-009&size=${SIZE}&seed=${SEED}`,
  ...
  await page.evaluate(() => {
    ...
    window.__CORE_V2_BAR_PAN_SAMPLE__ = sample;
    requestAnimationFrame(tick);
  });

  const action = await page.evaluate(async () => {
    const started = performance.now();
    const result = await window.__PATCH_MAP_CORE_V2_MANUAL_LAB__.run('animate-all');
    return {
  ...
  const activeAnimationsAfterAction = await page.evaluate(() =>
    window.__PATCH_MAP_CORE_V2_MANUAL_LAB__.state().activeAnimations);

  await page.evaluate(() => {
    window.__CORE_V2_BAR_PAN_SAMPLE__.panStart = performance.now();
  });
  ...
  const panWallMs = await page.evaluate(() => {
    const sample = window.__CORE_V2_BAR_PAN_SAMPLE__;
  ...
  const captured = await page.evaluate(() => {
    ...
    delete window.__CORE_V2_BAR_PAN_SAMPLE__;
    return result;
  });
...
async function main() {
  ...
  try {
    ...
    if (!baseUrl) throw new Error('Core v2 bar/pan performance server has no URL');
    ...
    for (const profile of PROFILES) {
      ...
      for (let index = 0; index < total; index += 1) {
        process.stdout.write(
          `[core-v2-bar-pan] ${profile.id} ${index + 1}/${total}\n`,
        );
    ...
    const output = Object.freeze({
      ...
      comparison: Object.freeze({
        ...
        coreV1: 'not-comparable: no identical persistent manual Lab gesture harness',
    ...
    for (const profile of profiles) {
      process.stdout.write(
        `[core-v2-bar-pan] ${profile.id} action median/p95=`
    ...
    if (output.status !== 'pass') {
      throw new Error(
        `Core v2 bar/pan checkpoint failed; evidence saved to ${OUTPUT_PATH}`,
      );
    }
    process.stdout.write(`PASS: Core v2 5000-bar animation plus pan 2+7\n`);
  } finally {
