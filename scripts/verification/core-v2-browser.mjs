...
const failures = [];
try {
  await page.goto(new URL('lab/performance-v2/', baseUrl).href, { waitUntil: 'networkidle' });
  ...
  const assetNextFrame = await page.evaluate(() => {
    ...
    if (!runtime) throw new Error('missing Core v2 runtime after asset unload');
  ...
  if (!box) throw new Error('Core v2 canvas has no browser bounds');
  ...
  await page.mouse.up();
  const afterPan = await page.evaluate(() => window.__PATCH_MAP_CORE_V2_LAB__.getRuntime()?.view);
  check(
  ...
  check(
    await page.evaluate(() => window.__PATCH_MAP_CORE_V2_LAB__.getRuntime()?.selection().refs.length) === 0,
  ...
  check(
    layerLabels.some((label) => label.includes('PATCH MAP Core v2')) && layerLabels.some((label) => label.includes('world')),
    'Pixi DevTools exposes Application stage and aggregate world labels',
    checks,
    failures,
  );

  await page.evaluate(() => window.__PATCH_MAP_CORE_V2_LAB__.loadProduction());
  await waitReady(page);
  ...
  await action(page, 'prepare');
  await action(page, 'asset-unload');
  await page.evaluate(() => window.__PATCH_MAP_CORE_V2_LAB__.getRuntime()?.flush('particle-post-unload-frame'));
...
function labState(page) {
  return page.evaluate(() => window.__PATCH_MAP_CORE_V2_LAB__.state);
}

function assetState(page) {
  return page.evaluate(() => {
    const runtime = window.__PATCH_MAP_CORE_V2_LAB__.getRuntime();
    if (!runtime) throw new Error('missing Core v2 runtime for asset state');
    const debug = runtime.debugSnapshot().renderer;
    return {
      loadedAssetCount: debug.loadedAssetCount,
      unresolvedAssetCount: debug.unresolvedAssetCount,
...
function textState(page) {
  return page.evaluate(() => {
    const runtime = window.__PATCH_MAP_CORE_V2_LAB__.getRuntime();
    if (!runtime) throw new Error('missing Core v2 runtime for text state');
    const debug = runtime.debugSnapshot().renderer;
    return {
...
function viewState(page) {
  return page.evaluate(() => {
    const runtime = window.__PATCH_MAP_CORE_V2_LAB__.getRuntime();
    if (!runtime) throw new Error('missing Core v2 runtime for view state');
    return {
      view: runtime.view,
...
function fittedViewState(page) {
  return page.evaluate(() => {
    const runtime = window.__PATCH_MAP_CORE_V2_LAB__.getRuntime();
    if (!runtime) throw new Error('missing Core v2 runtime for fitted view state');
...
function barState(page) {
  return page.evaluate(() => {
    const runtime = window.__PATCH_MAP_CORE_V2_LAB__.getRuntime();
    if (!runtime) throw new Error('missing Core v2 runtime for bar state');
    const debug = runtime.debugSnapshot();
    return {
      values: runtime.snapshot().entities
...
function waitForBarAnimationEvidence(page, before) {
  return page.evaluate(async (initial) => new Promise((resolve, reject) => {
    ...
    const observe = () => {
      const runtime = window.__PATCH_MAP_CORE_V2_LAB__.getRuntime();
      if (!runtime) {
        reject(new Error('Core v2 runtime disappeared during bar animation'));
        return;
      }
      const debug = runtime.debugSnapshot();
