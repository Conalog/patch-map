...
const contractRoot = 'docs/reference/core-v2-functional-contract/';
...
assert(manifest.$schema === 'core-v2-decision-evidence-manifest/1', 'manifest schema');
assert(fixtures.$schema === 'core-v2-decision-fixtures/1', 'fixture schema');
assert(expected.$schema === 'core-v2-decision-normalized-expected/1', 'expected schema');
...
const expectedDecisions = Array.from({ length: 38 }, (_, index) => `OQ-${String(index + 1).padStart(3, '0')}`);
...
assert(fixtures.cases.length === 38, '38 fixture records');
assert(expected.cases.length === 38, '38 expected records');
assert(manifest.cases.length === 38, '38 manifest records');
...
assert(approved === 38, '38 contract-approved records');
assert(pending === 0, 'no pending-external-evidence records');
assert(manifest.reviewSummary.decisionCount === 38, 'review decision count');
...
console.log(`Core v2 decision evidence verified: ${approved} approved, ${pending} pending, ${manifestById.size} total`);
...
function validateSemanticGuardrails(fixtureById, expectedById) {
  ...
  const rollback = expectedById.get('DEC-OQ-027-CANARY-ROLLBACK').expected.rollback;
  assert(rollback.nextRemountEngine === 'previous' && rollback.activeLifecycles === 1 && rollback.canvasCount === 1, 'OQ-027 rollback remount');
  const canaryFixture = fixtureById.get('DEC-OQ-027-CANARY-ROLLBACK');
  const blockerExpansion = canaryFixture.actionTrace.find((action) => action.action === 'expand-one-fresh-rollout-per-blocker');
  assert(blockerExpansion?.expanderRevision === 'canary-blocker-matrix/1', 'OQ-027 blocker expander revision');
  assertSameSet(blockerExpansion.blockers, canaryFixture.setup.promotionBlockers, 'OQ-027 independent blocker rollouts');
  ...
  const placementComponent = fixtureById.get('DEC-OQ-007-PLACEMENT-NONE').setup.dataset[0].components[0];
  assert(placementComponent.type === 'bar' && placementComponent.placement === 'none' && placementComponent.animation === false, 'OQ-007 deterministic placement component');
...
async function validateExternalEvidence(fixtureById, expectedById) {
  for (const id of ['DEC-OQ-001-LEGACY-ROOT', 'DEC-OQ-031-INTERNATIONAL-TEXT']) {
  ...
  assert(unicodeById.get('unicode-missing-glyph').missingGlyphs[0].identity === 'core-v2-missing-glyph-box/1', 'OQ-031 deterministic missing glyph');
