...
import { inspectPatchMapUpdateConflictActuals } from '../patch-map-contract/update-conflict-actuals.mjs';
...
function assertGpuEvidence(caseId, gpu, runLabel, actualObservation) {
  ...
  invariant(gpu.revision === 'patch-map-webgl-browser-probe/1', `${prefix} revision`);
...
function assertComponentAssetFocusedUi(caseId, ui, runLabel) {
  ...
  if (caseId === 'REN-008') {
    for (const facts of phaseFacts) {
      ...
      invariant(
        facts['authored-size'] === 'null',
        `REN-008 ${runLabel} has no independent background size`,
      );
      invariant(
...
export function invariant(condition, message) {
  if (!condition) throw new Error(`PatchMap render browser checkpoint failed: ${message}`);
}
