...
if (fixtures.cases.length !== 173 || expected.cases.length !== 173) throw new Error('Expected 173 paired catalog records');
...
const registry = {
  $schema: 'core-v2-catalog-review-registry/1',
  contractRevision: fixtures.contractRevision,
...
await mkdir(`${root}docs/reference/core-v2-functional-contract/evidence/reviews`, { recursive: true });
for (const report of reviewReports) {
  await writeFile(`${root}docs/reference/core-v2-functional-contract/${report.reportPath}`, report.bytes);
}
await writeFile(`${root}${catalogReviewPath}`, serialized(registry));
