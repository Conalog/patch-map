...
const ARTIFACT_DIRECTORY = path.resolve(
  process.env.CORE_V2_EXTRACTION_PERF_ARTIFACT_DIR
...
async function main() {
  ...
  try {
    ...
    if (!baseUrl) throw new Error('Core v2 extraction performance server has no URL');
    const pageUrl = new URL('performance/core-v2/index.html', baseUrl);
    ...
    await page.waitForFunction(
      () => typeof window.__PATCH_MAP_CORE_V2_BENCHMARK__?.runExtraction === 'function',
      undefined,
      { timeout: 30_000 },
    );
    ...
    for (const [index, scale] of SCALES.entries()) {
      process.stdout.write(
        `[core-v2-extraction-perf] mesh/${String(scale)}: `
        + `${WARMUPS}+${MEASURED} x ${EXTRACTIONS_PER_TRIAL}\n`,
      );
      const result = await page.evaluate(
        (spec) => window.__PATCH_MAP_CORE_V2_BENCHMARK__.runExtraction(spec),
        {
          strategy: 'mesh',
          scale,
    ...
    const output = {
      $schema: 'core-v2-extraction-performance-checkpoint/1',
    ...
    if (output.status !== 'pass') {
      throw new Error(`Core v2 extraction browser errors: ${JSON.stringify(errors)}`);
    }
    for (const run of runs) {
      const total = run.summary.extractionTotalMs;
      const repeat = run.summary.repeatExtractionMedianMs;
      process.stdout.write(
        `[core-v2-extraction-perf] ${String(run.scale)} `
    ...
    process.stdout.write(`PASS: Core v2 extraction performance 2+7 across six scales\n`);
  } finally {
    await context?.close();
    await browser?.close();
    await server.close();
