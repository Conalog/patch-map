...
function projectRelationProbe(value) {
  ...
  return {
    ...
    revisionLags: nullableRevisionTuple(probe.revisionLags, 'relation probe revision lags'),
...
function nullableRevisionTuple(value, label) {
  ...
  return {
    scene: normalizeNumber(tuple.scene, `${label}.scene`),
    view: normalizeNumber(tuple.view, `${label}.view`),
    interaction: normalizeNumber(tuple.interaction, `${label}.interaction`),
  };
}
...
function assert(condition, message) {
  if (!condition) throw new Error(`PatchMap render-relations handler invalid: ${message}`);
}
