...
function queryBoundsAction(context, actionRecord) {
  ...
  return {
    actual: {
      ...
      revisionLags: revisionTuple(geometry.revisionLags, 'geometry revision lags'),
      product,
    },
  };
}
...
function requireGeometry(value, label) {
  ...
  revisionTuple(value.revisionLags, `${label}.revisionLags`);
...
function assert(condition, message) {
  if (!condition) throw new Error(`PatchMap render-bounds handler invalid: ${message}`);
}
