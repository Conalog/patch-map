...
function requireDatasetElement(dataset, id) {
  ...
  throw new Error(`PatchMap pointer/selection handler invalid: missing dataset element ${id}`);
}
...
function transformHandleForKind(kind) {
  ...
  throw new Error(`PatchMap pointer/selection handler invalid: transform kind ${kind}`);
}
...
function keyNudgeDelta(key, distance) {
  ...
  throw new Error(`PatchMap pointer/selection handler invalid: unsupported nudge key ${key}`);
}
...
function assert(condition, message) {
  if (!condition) throw new Error(`PatchMap pointer/selection handler invalid: ${message}`);
}
