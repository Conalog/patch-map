...
const ACTUAL_DELTA_SCHEMA = 'patch-map-semantic-observation-delta/1';
const EXECUTION_SCHEMA = 'patch-map-contract-case-execution/1';
const HOST_DELTA_SCHEMA = 'patch-map-host-seam-delta/1';
...
async function createEngine(state, role) {
  ...
  if (state.closing) {
    ...
    throw new Error(`PatchMap execution invalid: ${role} engine created after execution closed`);
  }
  return record;
}
...
function assert(condition, message) {
  if (!condition) throw new Error(`PatchMap execution invalid: ${message}`);
}
