...
export const EDITOR_WORKFLOW_FOLD_REVISION =
  'patch-map-editor-workflow-fold/1';
...
function projectCase(caseId, execution) {
  switch (caseId) {
    ...
    default:
      throw new Error(`Unsupported PatchMap editor fold case ${String(caseId)}`);
  }
}
...
function assert(condition, message) {
  if (!condition) throw new Error(`Invalid PatchMap editor workflow fold: ${message}`);
}
