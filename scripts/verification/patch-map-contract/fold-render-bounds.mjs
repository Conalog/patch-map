...
export const RENDER_BOUNDS_FOLD_REVISION = 'patch-map-render-bounds-fold/1';
...
function projectGeometry(queried) {
  ...
  const geometry = {
    ...
    bounds: {
      ...
      revisionLags: revisionTuple(queried.revisionLags, 'geometry revision lags'),
    },
  };
...
function assert(condition, message) {
  if (!condition) throw new Error(`PatchMap render-bounds fold invalid: ${message}`);
}
