...
function projectCase(plan, execution) {
  switch (plan.id) {
    ...
    default:
      throw new Error(`Unsupported Core v2 accessibility case ${String(plan.id)}`);
  }
}
...
function validateProductCleanup(cleanup, caseId) {
  ...
  assert(
    product.revision === 'core-v2-accessibility-cleanup/1',
...
function assert(condition, message) {
  if (!condition) {
    throw new Error(`Invalid Core v2 accessibility fold: ${message}`);
  }
}
