...
/** Re-hash and validate one completed run without importing or launching Core v2. */
...
export async function loadCanonicalCatalogBinding() {
  ...
  assert(
    manifest.$schema === 'core-v2-contract-catalog-evidence-manifest/1',
...
function parseJson(bytes, label) {
  try {
    ...
    throw new Error(`Core v2 result verification failed: ${label} is not JSON`, { cause: error });
  }
}
...
function assert(condition, message) {
  if (!condition) throw new Error(`Core v2 result verification failed: ${message}`);
}
