...
function runConsumerFlowAction(state, evidence, _context, action) {
  ...
  const checks = {
    ...
    load:
      positiveInteger(esm.parsedEntities, 'parsed entity count')
        === positiveInteger(esm.loadedEntities, 'loaded entity count'),
    update: esm.transactionStatus === 'committed',
    destroy:
      esm.destroyed === true
...
function runRedesignedHostAdapterAction(state, evidence, _context, action) {
  const operands = exactOperands(action, ['capabilities', 'originalImports']);
  const requestedCapabilities = stringArray(operands.capabilities, 'adapter capabilities');
  const requestedOriginalImports = stringArray(operands.originalImports, 'original imports');
  const packageMatrix = recordValue(evidence.packageMatrix, 'package matrix');
  ...
  state.projection.hostAdapter = {
    ...
    requestedOriginalImports,
    reachedCapabilities: stringArray(
      hostAdapter.reachedCapabilities,
      'reached capabilities',
    ),
    originalImportCount: nonNegativeInteger(
      audit.originalImportCount,
...
function validateDocumentationDigestAction(state, evidence, _context, action) {
  ...
  state.projection.documentationDigest =
    missingDocs.length === 0 && publicDocs.length > 0
...
function moduleTargetPassed(evidence, target, strictTypeScript) {
  if (target === 'esm') {
    ...
    return result.immutable === true
      && result.hierarchyImmutable === true
  ...
  if (target === 'cjs') {
    ...
    return positiveInteger(result.entities, 'CJS entity count') > 0
      && result.authoringPlannerType === 'function'
      && result.extractionType === 'function';
  }
  if (target === 'types') {
...
function assert(condition, message) {
  if (!condition) throw new Error(`Core v2 package handler invalid: ${message}`);
}
