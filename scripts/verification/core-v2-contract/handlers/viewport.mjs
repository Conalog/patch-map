...
async function viewGestureSeriesAction(product, state, context, action) {
  ...
  for (const gestureValue of gestures) {
    ...
    switch (gesture.type) {
      ...
      default:
        throw new Error(`Core v2 viewport handler invalid: unknown gesture ${String(gesture.type)}`);
    }
  }
...
function probeDisabledViewportPolicy(engine, policy, inputValue) {
  ...
  throw new Error(`Core v2 viewport handler invalid: cannot probe policy ${policy}`);
}
...
function assert(condition, message) {
  if (!condition) throw new Error(`Core v2 viewport handler invalid: ${message}`);
}
