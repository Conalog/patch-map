...
export function createExecutorPlan(catalog, options) {
  ...
  return deepFreeze({
    $schema: 'core-v2-executor-plan/1',
...
function materializeSelectedCase(record, routeParams) {
  ...
  const expectedTemplate = `/lab/core-v2?scenario=${record.id}&size=<SIZE>&seed=<SEED>`;
...
function assert(condition, message) {
  if (!condition) throw new Error(`Core v2 materialization invalid: ${message}`);
}
