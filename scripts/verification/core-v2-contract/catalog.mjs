...
const DEFAULT_EVIDENCE_ROOT = fileURLToPath(
  new URL('../../../docs/reference/core-v2-functional-contract/evidence/', import.meta.url),
);
...
export async function loadExecutorCatalog(options = {}) {
  ...
  return deepFreeze({
    $schema: 'core-v2-executor-catalog/1',
...
async function readEvidenceFile(evidenceRoot, fileName) {
  ...
  try {
    ...
    throw new Error(`Core v2 executor catalog invalid: ${fileName} is not JSON`, { cause: error });
  }
}

function validateDocumentHeaders(files) {
  const expectedSchemas = {
    manifest: 'core-v2-contract-catalog-evidence-manifest/1',
    priorities: 'core-v2-catalog-priorities/1',
    profiles: 'core-v2-catalog-fixture-profiles/1',
    actions: 'core-v2-catalog-action-contract/1',
    observations: 'core-v2-catalog-observation-contract/1',
    reviews: 'core-v2-catalog-review-registry/1',
    fixtures: 'core-v2-contract-catalog-fixtures/1',
  };
...
function validateCaseBindings({ record, fixture, review, fixtures, manifest, index }) {
  ...
  const expectedRoute = `/lab/core-v2?scenario=${record.id}&size=<SIZE>&seed=<SEED>`;
...
function validateInventory(manifest, reviews, inventory) {
  for (const [field, expected] of Object.entries(EXPECTED_INVENTORY)) {
    assert(inventory[field] === expected, `inventory ${field}: expected ${expected}, received ${inventory[field]}`);
  }
  assert(inventory.capabilityCases === 135, 'inventory capability cases');
  assert(inventory.consumerJourneys === 38, 'inventory consumer journeys');
  assert(inventory.priorities.P0 === 121 && inventory.priorities.P1 === 52, 'inventory priorities');
...
function assert(condition, message) {
  if (!condition) throw new Error(`Core v2 executor catalog invalid: ${message}`);
}
