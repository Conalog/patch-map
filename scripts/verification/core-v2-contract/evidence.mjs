...
export const ACTUAL_ARTIFACT_SCHEMA = 'core-v2-actual-observation/1';
export const COMPARISON_ARTIFACT_SCHEMA = 'core-v2-comparison-evidence/1';
export const EVIDENCE_SCHEMA = 'core-v2-execution-evidence/1';
export const EXECUTION_OVERLAY_SCHEMA = 'core-v2-execution-overlay/1';
...
/**
 * Assemble one complete, append-only Core v2 execution run.
 *
...
export function validateCatalogBinding(value) {
  ...
  assert(value.observationRevision === 'core-v2-semantic-observation/1', 'observation revision');
  for (const field of [
    'catalogManifestSha256',
    'reviewRegistrySha256',
    'actionSchemaSha256',
...
export function validatePackageBinding(value) {
  ...
  assert(value.subpath === '@conalog/patch-map/core-v2', 'package subpath');
...
export function validateRunner(value, runId) {
  ...
  assert(value.id === 'core-v2-contract-runner', 'runner ID');
...
export function validateInput(value, caseId) {
  ...
  assert(
    value.route === `/lab/core-v2?scenario=${caseId}&size=${value.size}&seed=${value.seed}`,
...
export function aggregateComparisonSha256(document) {
  return canonicalSha256({
    $schema: 'core-v2-two-fresh-session-comparison/1',
...
function fail(message) {
  throw new Error(`Core v2 execution evidence invalid: ${message}`);
}
