...
const OUTPUT_PATH = path.resolve(
  process.env.PATCH_MAP_WEBGPU_OUTPUT
...
try {
  ...
  if (!baseUrl) throw new Error('PatchMap WebGPU Lab server has no URL');
  ...
  await page.goto(
    new URL(
      `lab/patch-map/?dataset=${DATASET}&strategy=mesh&backend=webgpu`,
      baseUrl,
    ).href,
  ...
  await page.waitForFunction(
    () => {
      const status = window.__PATCH_MAP_LAB__?.state.status;
  ...
  const initial = await page.evaluate(async () => {
    const bridge = window.__PATCH_MAP_LAB__;
  ...
  record(
    ...
    'PatchMap activates WebGPU without a silent WebGL fallback',
    initial.state,
  );
  if (initial.state.activeBackend !== 'webgpu') {
    throw new Error(
      `PatchMap WebGPU activation failed without fallback allowance: ${JSON.stringify(initial.state)}`,
    );
  }
  record(
  ...
  await page.waitForFunction(
    () => (window.__PATCH_MAP_LAB__.getRuntime()?.activeAnimations ?? 0) === 0,
    undefined,
    { timeout: 10_000 },
  );
  ...
  if (!box) throw new Error('PatchMap WebGPU canvas has no browser bounds');
  const target = await page.evaluate(() => {
    const runtime = window.__PATCH_MAP_LAB__.getRuntime();
    if (!runtime) throw new Error('PatchMap WebGPU runtime is missing');
    ...
    throw new Error('PatchMap WebGPU scene has no on-screen interactive target');
  });
  ...
  const viewBefore = await page.evaluate(() => window.__PATCH_MAP_LAB__.getRuntime()?.view);
  await page.mouse.move(box.x + zoomPoint.x, box.y + zoomPoint.y);
  await page.mouse.wheel(0, -180);
  await page.waitForFunction(
    (scale) => (window.__PATCH_MAP_LAB__.getRuntime()?.view.scale ?? scale) > scale,
    viewBefore.scale,
  );
  const interaction = await page.evaluate(() => {
    const runtime = window.__PATCH_MAP_LAB__.getRuntime();
    if (!runtime) throw new Error('PatchMap WebGPU runtime is missing after wheel');
  ...
  const widthBefore = await page.evaluate(
    () => window.__PATCH_MAP_LAB__.getRuntime()?.renderer.width ?? null,
  );
  await page.setViewportSize({ width: 1_180, height: 840 });
  await page.waitForFunction(
    (width) => window.__PATCH_MAP_LAB__.getRuntime()?.renderer.width !== width,
    widthBefore,
  );
  ...
  const evidence = {
    $schema: 'patch-map-webgpu-experimental/1',
  ...
  if (status === 'pass') {
    process.stdout.write(
      `PASS: ${checks.length} PatchMap WebGPU experimental checks; active backend webgpu; errors 0\n`,
    );
  } else if (status === 'unsupported') {
    process.stdout.write(`UNSUPPORTED: PatchMap WebGPU adapter unavailable; evidence saved to ${OUTPUT_PATH}\n`);
  } else {
    process.stderr.write(`FAIL: PatchMap WebGPU experimental smoke; evidence saved to ${OUTPUT_PATH}\n`);
    process.exitCode = 1;
  }
}
...
async function action(activePage, testId) {
  ...
  const state = await activePage.evaluate(() => window.__PATCH_MAP_LAB__.state);
...
function assetState(activePage) {
  return activePage.evaluate(() => {
    const runtime = window.__PATCH_MAP_LAB__.getRuntime();
    if (!runtime) throw new Error('PatchMap WebGPU runtime is missing for asset state');
...
function barState(activePage) {
  return activePage.evaluate(() => {
    const runtime = window.__PATCH_MAP_LAB__.getRuntime();
    if (!runtime) throw new Error('PatchMap WebGPU runtime is missing for bar state');
    const bars = runtime.snapshot().entities.filter((entity) => entity.kind === 'bar');
...
function textState(activePage) {
  return activePage.evaluate(() => {
    const runtime = window.__PATCH_MAP_LAB__.getRuntime();
    if (!runtime) throw new Error('PatchMap WebGPU runtime is missing for text state');
