...
export const contractRoot = 'docs/reference/core-v2-functional-contract/';
...
const observationDomainsByPrefix = Object.freeze({
  ...
  MIG: ['revisions', 'scene', 'outcome', 'resources'],
});
...
const fixtureProfilesByPrefix = Object.freeze({
  ...
  MIG: ['cutover-canary-rollback-matrix'],
});
...
const sharedProfileSources = Object.freeze({
  ...
  'hostile-asset-and-package-matrix': ['production-readiness.md', 'scenarios/security-accessibility-operations-migration.md'],
  'logical-accessibility-tree': ['semantic-observation.md', 'scenarios/security-accessibility-operations-migration.md'],
  'bounded-diagnostics-matrix': ['semantic-observation.md', 'scenarios/security-accessibility-operations-migration.md'],
  'cutover-canary-rollback-matrix': ['production-readiness.md', 'scenarios/security-accessibility-operations-migration.md'],
...
export async function buildCatalog() {
  ...
  const expected = {
    $schema: 'core-v2-contract-catalog-normalized-expected/1',
...
async function readTypedRegistry(sources) {
  ...
  assert(document.$schema === 'core-v2-typed-case-contracts/1', 'typed case registry schema');
  const byId = new Map();
...
async function readActionSchemaRegistry() {
  ...
  const document = JSON.parse(bytes);
  assert(document.$schema === 'core-v2-catalog-action-contract/1', 'independent action contract schema');
...
async function readObservationSchemaRegistry() {
  ...
  const document = JSON.parse(bytes);
  assert(document.$schema === 'core-v2-catalog-observation-contract/1', 'independent observation contract schema');
  return { document, sha256: sha256(bytes) };
}
...
async function readProfileRegistry() {
  ...
  const document = JSON.parse(bytes);
  assert(document.$schema === 'core-v2-catalog-fixture-profiles/1', 'fixture profile registry schema');
  return { document, sha256: sha256(bytes) };
}
...
async function readReviewRegistry() {
  try {
    ...
    const document = JSON.parse(bytes);
    assert(document.$schema === 'core-v2-catalog-review-registry/1', 'review registry schema');
...
async function readPriorityRegistry() {
  ...
  const document = JSON.parse(bytes);
  assert(document.$schema === 'core-v2-catalog-priorities/1', 'priority registry schema');
...
function toFixture(source, typed, sharedProfiles) {
  ...
  const base = {
    ...
    automationOwner: `core-v2-contract/${source.id}`,
...
function toManifest(
  ...
  return {
    $schema: 'core-v2-contract-catalog-evidence-manifest/1',
    contractRevision: fixtures.contractRevision,
    observationRevision: fixtures.observationRevision,
    generatedAt: '2026-07-16',
    scope: 'all capability scenarios and patch-service P0 consumer journeys; contract evidence only',
    sourceCatalog: {
      capabilityRoot: 'scenarios/',
      consumerJourneys: 'consumer-journeys.md',
      capabilityCount,
...
const domainAliases = Object.freeze({
  ...
  diagnostics: 'outcome', result: 'outcome', callback: 'events', callbacks: 'events', animation: 'paint', assets: 'resources', package: 'resources', security: 'outcome', migration: 'outcome', performance: 'outcome',
...
function routeFor(id) {
  return `/lab/core-v2?scenario=${id}&size=<SIZE>&seed=<SEED>`;
}
...
function assert(condition, message) {
  if (!condition) throw new Error(`Core v2 catalog extraction failed: ${message}`);
}
