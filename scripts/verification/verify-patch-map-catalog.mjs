...
assert(fixtures.$schema === 'patch-map-contract-catalog-fixtures/1', 'fixture schema');
assert(expected.$schema === 'patch-map-contract-catalog-normalized-expected/1', 'expected schema');
assert(manifest.$schema === 'patch-map-contract-catalog-evidence-manifest/1', 'manifest schema');
assert(fixtures.cases.length > 0, 'non-empty fixture catalog');
assert(expected.cases.length === fixtures.cases.length, 'paired normalized expected records');
assert(manifest.cases.length === fixtures.cases.length, 'paired manifest records');
assert(
  manifest.sourceCatalog.capabilityCount + manifest.sourceCatalog.consumerJourneyCount
...
assert(
  (await readJson(`${contractRoot}${manifest.actionSchemaFile.path}`)).$schema === 'patch-map-catalog-action-contract/1',
  'independent action contract schema',
);
assert(
  (await readJson(`${contractRoot}${manifest.observationSchemaFile.path}`)).$schema === 'patch-map-catalog-observation-contract/1',
  'independent observation contract schema',
);
assert((await readJson(`${contractRoot}${manifest.priorityFile.path}`)).$schema === 'patch-map-catalog-priorities/1', 'priority registry schema');
assert(
...
assert(manifestById.size === fixtures.cases.length && labRoutes.size === fixtures.cases.length, 'complete unique catalog');
assert(manifest.reviewSummary.contractApproved === fixtures.cases.length, 'all contract records reviewed');
...
console.log(
  `PatchMap contract catalog verified: ${manifest.sourceCatalog.capabilityCount} capabilities + ${manifest.sourceCatalog.consumerJourneyCount} journeys = ${fixtures.cases.length} approved records`,
);
...
function assert(condition, message) {
  if (!condition) throw new Error(`PatchMap catalog verification failed: ${message}`);
}
