...
async function command(page, name) {
  return page.evaluate(async (commandName) => {
    ...
    const sample = window.__PATCH_MAP_INTERACTION_SAMPLE__;
...
async function instrument(page) {
  await page.evaluate(() => {
    ...
    window.__PATCH_MAP_INTERACTION_SAMPLE__ = { calls, publications };
  });
}

async function callCursor(page, name) {
  return page.evaluate((method) => {
    const samples = window.__PATCH_MAP_INTERACTION_SAMPLE__?.calls[method] ?? [];
...
async function firstPublicationAfter(page, cursor) {
  await page.waitForFunction(
    (start) =>
      (window.__PATCH_MAP_INTERACTION_SAMPLE__?.publications.length ?? 0) > start,
    cursor,
    { timeout: 5_000 },
  );
  return page.evaluate((start) =>
    window.__PATCH_MAP_INTERACTION_SAMPLE__.publications[start], cursor);
}
...
async function runTrial(page, baseUrl, trialIndex) {
  ...
  facts.shiftClickSelectionCount = (
    await page.evaluate(() => window.__PATCH_MAP_MANUAL_LAB__.state())
  ).selectedIds.length;
...
async function main() {
  ...
  try {
    ...
    for (const profile of PROFILES) {
      ...
      for (let index = 0; index < WARMUPS + MEASURED; index += 1) {
        process.stdout.write(
          `[patch-map-interactions] ${profile.id} ${index + 1}/${WARMUPS + MEASURED}\n`,
        );
    ...
    const output = Object.freeze({
      $schema: 'patch-map-interaction-performance/2',
    ...
    for (const profile of profiles) {
      process.stdout.write(
        `[patch-map-interactions] ${profile.id} click/shift/box/paint p95=`
    ...
    process.stdout.write(`PASS: PatchMap 5,000 interaction 2+7 checkpoint\n`);
  } finally {
