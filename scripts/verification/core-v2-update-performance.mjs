 ...
 */
export async function runCoreV2UpdatePerformanceCheckpoint({
  ...
  try {
    ...
    for (const scale of UPDATE_PERFORMANCE_PROTOCOL.scales) {
      ...
      for (let index = 0; index < total; index += 1) {
        const trial = await runCoreV2UpdateTransactionTrial(runtime, scale, index);
    ...
    const output = Object.freeze({
      ...
      scope: Object.freeze({
        measured: Object.freeze([
          'public CoreV2Engine transaction validation',
        ...
        rendererEvidence: 'canonical Core v2 Chromium 4x matrix; not rerun by this checkpoint',
    ...
    validateCoreV2UpdatePerformanceOutput(output);
...
export function validateCoreV2UpdatePerformanceOutput(output) {
...
async function loadRuntime() {
  ...
  try {
    const core = await vite.ssrLoadModule('/src/core-v2/index.ts');
    const workloads = await vite.ssrLoadModule('/performance/core-v2/workloads.ts');
    assert(typeof core.CoreV2Engine === 'function', 'CoreV2Engine public export is unavailable');
    assert(
      typeof workloads.createSyntheticPatchMap === 'function',
      'Core v2 synthetic workload factory is unavailable',
    );
    return Object.freeze({
      CoreV2Engine: core.CoreV2Engine,
...
export async function runCoreV2UpdateTransactionTrial(runtime, scale, trial) {
  ...
  const engine = new runtime.CoreV2Engine({ surfaceFactory: async () => surface });
...
function assert(condition, message) {
  if (!condition) throw new Error(`Core v2 update performance checkpoint: ${message}`);
}

async function main() {
  const outputPath = argumentValue('--output') ?? DEFAULT_OUTPUT;
  const output = await runCoreV2UpdatePerformanceCheckpoint({ outputPath: path.resolve(outputPath) });
