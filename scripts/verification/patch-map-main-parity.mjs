...
import { compareObservations } from './patch-map-main-parity/compare.mjs';
import { buildPatchMapMainParityContractMap } from './patch-map-main-parity/contract-map.mjs';
import { comparePngBuffers } from './patch-map-main-parity/image-metrics.mjs';
import { FIRST_PARITY_TRANCHE } from './patch-map-main-parity/scenarios.mjs';
...
const OUTPUT_ROOT = path.resolve(
  process.env.PATCH_MAP_MAIN_PARITY_OUTPUT
    ?? path.join(ROOT, 'docs/tasks/2026/07-29/patch-map-main-parity/artifacts/first-tranche'),
);
const mainSha = process.env.PATCH_MAP_MAIN_SHA ?? '0aaaa98b479c86939b049581734a7fb0745ba70e';
const coreSha = process.env.PATCH_MAP_CODE_COMMIT ?? 'working-tree';
...
try {
  ...
  const patchMapPage = await coreContext.newPage();
  bindErrors(mainPage, runtimeErrors.main);
  bindErrors(patchMapPage, runtimeErrors.patchMap);
  ...
  await Promise.all([
    ...
    patchMapPage.goto(new URL('lab/main-parity/patch-map.html', baseUrl).href, {
      waitUntil: 'networkidle',
    }),
  ]);
  await Promise.all([
    mainPage.waitForFunction(() => document.body.dataset.oracleReady === 'true'),
    patchMapPage.waitForFunction(() => document.body.dataset.oracleReady === 'true'),
  ]);
  ...
  for (const scenario of selectedScenarios) {
    ...
    const checkpoints = [];
    await invokeBoth(mainPage, patchMapPage, 'initialize', [{ width: 800, height: 600, pixelRatio: 1 }]);
    const loaded = await invokeBoth(mainPage, patchMapPage, 'load', [structuredClone(dataset)]);
    if (!invocationsFulfilled(loaded)) {
      checkpoints.push(rejectedCheckpoint(0, 'loaded', loaded));
      const destroyed = await invokeBoth(mainPage, patchMapPage, 'destroy', []);
    ...
    for (const action of scenario.actions) {
      const observed = await performActionBoth(mainPage, patchMapPage, action);
...
const contractMap = await buildPatchMapMainParityContractMap({
...
const report = Object.freeze({
  schemaRevision: 'patch-map-main-parity/2026-07-29.1',
  ...
  runtimeErrors: Object.freeze({
    ...
    patchMap: freezeErrors(runtimeErrors.patchMap),
  }),
...
async function invokeBoth(mainPage, patchMapPage, method, args) {
  const [main, core] = await Promise.all([
    invokeOracle(mainPage, method, args),
    invokeOracle(patchMapPage, method, args),
  ]);
  return Object.freeze({ main, core });
}

async function performActionBoth(mainPage, patchMapPage, action) {
  if (action.type === 'browser-click') {
    ...
    await Promise.all([
      ...
      patchMapPage.mouse.click(action.x, action.y, options),
    ]);
    return invokeBoth(mainPage, patchMapPage, 'act', [structuredClone(action)]);
  }
  if (action.type === 'browser-drag') {
    const steps = action.steps ?? 8;
    await Promise.all([
      dragMouse(mainPage, action, steps),
      dragMouse(patchMapPage, action, steps),
    ]);
    return invokeBoth(mainPage, patchMapPage, 'act', [structuredClone(action)]);
  }
  if (action.type === 'browser-wheel') {
    await Promise.all([
      ...
      wheelMouse(patchMapPage, action),
    ]);
    return invokeBoth(mainPage, patchMapPage, 'act', [structuredClone(action)]);
  }
  return invokeBoth(mainPage, patchMapPage, 'act', [structuredClone(action)]);
...
async function captureCheckpoint({
  ...
  const [mainPng, corePng] = await Promise.all([
    ...
    patchMapPage.getByTestId('oracle-host').screenshot({
      ...
      findings: Object.freeze([Object.freeze({
        ...
        detail:
          classification === 'main-rejected-approved-input'
            ? 'main cannot serve as a visual oracle for this approved PatchMap input'
