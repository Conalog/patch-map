...
export const RENDER_COMPONENT_ASSETS_FOLD_REVISION =
  'patch-map-render-component-assets-fold/1';
...
function renderLaneForRole(role) {
  ...
  throw new Error(`PatchMap render-component-assets fold invalid: unsupported render role ${String(role)}`);
}
...
function findExportedElement(elements, id) {
  ...
  throw new Error(`PatchMap render-component-assets fold invalid: missing exported owner ${id}`);
}
...
function assert(condition, message) {
  if (!condition) {
    throw new Error(`PatchMap render-component-assets fold invalid: ${message}`);
  }
}
