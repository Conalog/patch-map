...
export const RENDER_FOUNDATION_FOLD_REVISION = 'patch-map-render-foundation-fold/1';
...
function actionProduct(actual) {
  ...
  throw new Error('PatchMap render-foundation fold invalid: action lacks product observation');
}
...
function findElement(elements, id) {
  ...
  throw new Error(`PatchMap render-foundation fold invalid: missing element ${id}`);
}
...
function resolveDimension(value, reference) {
  ...
  throw new Error('PatchMap render-foundation fold invalid: unsupported projected dimension');
}
...
function elementWorldPosition(elements, id, parentX = 0, parentY = 0) {
  ...
  throw new Error(`PatchMap render-foundation fold invalid: missing world position ${id}`);
}
...
function assert(condition, message) {
  if (!condition) throw new Error(`PatchMap render-foundation fold invalid: ${message}`);
}
