...
export const SECURITY_OPERATIONS_FOLD_REVISION =
  'patch-map-security-operations-fold/1';
...
function projectCase(plan, execution) {
  switch (plan.id) {
    ...
    default:
      throw new Error(`Unsupported PatchMap security/operations case ${String(plan.id)}`);
  }
}
...
function validateProductCleanup(cleanup, caseId) {
  ...
  assert(
    product.revision === 'patch-map-security-operations-cleanup/1',
...
function assert(condition, message) {
  if (!condition) throw new Error(`Invalid PatchMap security/operations fold: ${message}`);
}
