...
export const HISTORY_FOLD_REVISION = 'patch-map-history-fold/1';
...
function projectCaseDomains(caseId, execution) {
  switch (caseId) {
    ...
    default:
      throw new Error(`PatchMap history fold invalid: unsupported case ${caseId}`);
  }
}
...
function assert(condition, message) {
  if (!condition) throw new Error(`PatchMap history fold invalid: ${message}`);
}
