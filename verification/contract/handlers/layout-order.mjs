...
export const LAYOUT_ORDER_HANDLER_REVISION = 'patch-map-layout-order-handlers/1';
...
function projectPlacementEvidence(geometryValue, itemId, placements) {
  ...
  return {
    ...
    revisionLags: nullableRevisionTuple(geometry.revisionLags, 'geometry revision lags'),
    owner,
    order: [...placements],
    rows,
  };
...
function assert(condition, message) {
  if (!condition) throw new Error(`PatchMap layout-order handler invalid: ${message}`);
}
