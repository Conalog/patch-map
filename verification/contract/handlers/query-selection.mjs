...
export const QUERY_SELECTION_HANDLER_REVISION = 'patch-map-query-selection-handlers/1';
...
function productQuery(definition) {
  ...
  if (definition.predicate !== undefined) {
    ...
    if (Object.hasOwn(predicate, 'labelStartsWith')) {
      ...
      throw new Error('PatchMap query/selection handler invalid: unsupported query predicate');
    }
  }
  return query;
}
...
function assert(condition, message) {
  if (!condition) throw new Error(`PatchMap query/selection handler invalid: ${message}`);
}
