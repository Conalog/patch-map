...
function clone(value, seen = new WeakMap()) {
  ...
  if (seen.has(value)) throw new Error('PatchMap asset ingestion handler cannot clone cycles');
  seen.set(value, true);
...
function assert(condition, message) {
  if (!condition) throw new Error(`PatchMap asset ingestion handler: ${message}`);
}
