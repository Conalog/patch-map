...
function placementProductEvidence(value, label) {
  ...
  const revisionLags = nullableRevisionTuple(
    geometryProbe.revisionLags,
    `${label}.geometry revision lags`,
  );
  assert(geometryRevision !== null && geometryRevision > 0, `${label}.geometry revision`);
  assert(
    revisionLags !== null && geometryRevision + revisionLags.scene === sceneRevision,
  ...
  assert(runtime.revision === 'patch-map-layout-order-runtime/1', `${label}.runtime revision`);
...
function placementEvidence(value, label) {
  ...
  assertExactKeys(evidence, ['order', 'owner', 'revision', 'revisionLags', 'rows'], label);
  ...
  return {
    ...
    revisionLags: nullableRevisionTuple(evidence.revisionLags, `${label}.revisionLags`),
    owner,
    order,
    rows,
  };
...
function validateProductProjection(product, evidence, authored, label) {
  ...
  assert(sameJson(geometry.revisionLags, evidence.revisionLags), `${label}.geometry lag correlation`);
...
function nullableRevisionTuple(value, label) {
  if (value === null) return null;
  const tuple = recordValue(value, label);
  assertExactKeys(tuple, ['interaction', 'scene', 'view'], label);
  return {
    scene: finiteNumber(tuple.scene, `${label}.scene`),
    view: finiteNumber(tuple.view, `${label}.view`),
    interaction: finiteNumber(tuple.interaction, `${label}.interaction`),
  };
}
