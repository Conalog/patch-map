...
export const FOUNDATION_FOLD_REVISION = 'patch-map-foundation-fold/1';
...
const PRODUCT_PROBE_REVISION = 'patch-map-semantic-probe/1';
const BROWSER_PROBE_REVISION = 'patch-map-browser-probe/1';
const HOST_PROBE_REVISION = 'patch-map-packed-host-probe/1';
...
function projectHostEvidence({ plan, execution, hostProbe, provenance, extensions }) {
  ...
  extensions.foundationHostSeam = {
    $schema: 'patch-map-foundation-host-seam/1',
    promotionEligible: false,
...
function assert(condition, message) {
  if (!condition) throw new Error(`PatchMap foundation fold invalid: ${message}`);
}
