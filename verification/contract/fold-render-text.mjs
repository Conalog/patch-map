...
export const RENDER_TEXT_FOLD_REVISION = 'patch-map-render-text-fold/1';
...
function validateTextCleanupProbe(value, caseId) {
  ...
  assert(
    cleanup.revision === 'patch-map-text-runtime-cleanup/1',
...
function projectItemText(actual, execution) {
  ...
  actual.text.texts = {
    ...
    bidi: {
      visibleText: stringOrEmpty(terminalRows[2].projection.visibleText, 'bidi visible text'),
      lines: cloneArray(terminalRows[2].projection.lines, 'bidi lines'),
      layoutBounds: boundsObject(terminalRows[2].projection.layoutBounds, 'bidi layout bounds'),
      staleGlyphCount: nonNegativeInteger(
        terminalRows[2].renderer.staleGlyphCount,
...
function validateSupplementalAuthoredFacts(value, id, datasetId, target) {
  ...
  assert(
    authored.revision === 'patch-map-render-text-authored-facts/1',
...
function validateTextRuntimeProbe(value, label) {
  ...
  assert(resources.revision === 'patch-map-text-runtime-probe/1', `${label} revision`);
...
function historicalTextRoute(route) {
  ...
  // Immutable PatchMap normalized evidence predates the unambiguous Pixi object name.
...
function assert(condition, message) {
  if (!condition) throw new Error(`PatchMap render-text fold invalid: ${message}`);
}
