...
- [10,000개 렌더 전략 재검토](render-strategies-10000.md): JS 최적화와 Canvas
  기법을 검토하고 다섯 후보를 실험했다. 반복 성능 이득이 입증되지 않아 후보를
  제거했으며, 회전 경계 오류 수정과 확장한 측정 도구·실패 기록만 남겼다.

- [10,000개 viewport culling 실험](viewport-culling-10000.md): 직접 그리기 제외와
  화면 밖 텍스트 준비 지연을 분리 검증했다. 확대 화면의 그리기 비용은 줄었지만
  전체 보기의 성능 비회귀가 확보되지 않아 기본 렌더러에는 적용하지 않았다.
