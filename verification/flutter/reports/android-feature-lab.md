...
## 후속 결함: 서비스 blueprint의 드래그 선택

같은 날 사용자 제보로 박스 선택 옵션을 켠 뒤 실제 adb 드래그를 실행하자,
`Bad state: No element` 오류로 선택이 완료되지 않는 것을 재현했다.
위 최초 검증은 드래그 **이동**과 점 선택을 확인했으며 박스 선택은 빠져 있었다.

그룹·그리드의 집계 geometry는 quad가 비어 있는데, 네이티브 박스 선택이 이를
일반 패널처럼 처리하며 빈 좌표 목록에 `reduce`를 호출한 것이 원인이다.
영역 판정에서 quad 없는 집계 대상을 제외하고 실제 자식 패널을 선택하도록
수정했다. 포인터 이동·애니메이션 경로에는 추가 작업을 넣지 않았다.

- 수정 전 중첩 그룹·그리드 회귀 테스트에서 동일 예외를 확인했다.
- 수정 후 부분 교차/완전 포함 옵션의 회귀 테스트를 포함한 native pointer
  테스트 15개가 통과했다. 선택 완료 이벤트 1회, marquee 정리, viewport 유지도
  확인했다.
- `example/integration_test/box_selection_test.dart`를 같은 실기기 profile 모드로
  실행했다. 서비스 5,000개 패널 장면에서 UI로 박스 선택을 켜고 터치 드래그해
  `g0.0.0`, `g0.0.1`, `g0.1.0`, `g0.1.1`만 선택하며 viewport는 유지됨을 확인했다.
- 원시 결과는 기존 실행 디렉터리의 `box-selection.json`과
  `box-selection.log`에 보관한다.
- 일반 앱에도 수정본을 설치하고, 오류를 재현했던 동일 adb 드래그로 24개
  패널이 선택되는 것과 선택 테두리를 확인했다. 증거는
  `box-fixed-selected.xml`, `box-fixed-selected.png`이며 박스 선택을 켠 채로 두었다.

데모는 기본적으로 드래그 이동 모드다. 조작 탭 아래의 **드래그로 박스 선택**을
켜면 한 손가락 드래그가 영역 선택으로 바뀐다. 이 데모의 설정 변경은 장면을
초기화한다.
