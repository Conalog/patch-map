...
Canonical input is an ordered array of map elements. Array order and nested child order
are observable through hierarchy, stacking, and deterministic selection.
...
PatchMap must not mutate the caller's objects, arrays, nested styles, components, or
...
`display` and nested `metadata` to coexist with visual fields. PatchMap interprets the
...
| `scaleX`, `scaleY` | finite signed local scale for each axis |
...
fails atomically; neither field silently wins. `scale`, `skew`, `pivot`, and the axis
aliases `skewX`, `skewY`, `pivotX`, and `pivotY` are reserved rather than host metadata
and reject at their exact `attrs` path.
...
Each link contains `source` and `target` string identities. `style` accepts exactly
`color`, `width`, and `alpha`; omitted values resolve to black, `1`, and `1`.
Advanced PixiJS stroke keys are not relation inputs because relation rendering does not
project them. Link geometry follows endpoint movement, hierarchy transforms, viewport
orientation, redraw, and history.
...
A standalone rectangle requires size and accepts fill, stroke, one uniform nonnegative
radius, visibility, and world transform attributes. Per-corner radius belongs only to
rectangular component texture sources, whose semantic projection preserves each corner.
...
| relation | black stroke; width `1`, alpha `1` |
...
- Placement: `left`, `left-top`, `left-bottom`, `top`, `right`, `right-top`,
  `right-bottom`, `bottom`, or `center`.
- Margin/padding: one number, `{ x, y }`, or `{ top, right, bottom, left }`; explicit
  edge values override values expanded from an axis. Every value is nonnegative.
- Radius: standalone rects use one number; rectangular component textures use one
  number or per-corner values.
...
| margin/padding | nonnegative finite scalar/axis/edge values; missing entries default `0` |
| standalone rect `radius` | nonnegative finite scalar |
| rectangular texture `radius` | nonnegative scalar, four-entry tuple, or nonnegative `topLeft`, `topRight`, `bottomRight`, `bottomLeft` |
| animation | boolean, plus finite nonnegative duration in milliseconds; default `true` and `200` |
| text split | nonnegative integer; `0` means no split and positive `n` inserts a line break after each `n` Unicode grapheme clusters |
...
- An inline asset descriptor is strict: `{src: string, data?: object, format?: string,
  parser?: string}`. It does not accept a caller alias or unknown descriptor field.
- A rectangular texture source is `{type: 'rect', fill?, borderWidth?, borderColor?,
  radius?}`. Fill defaults transparent, border width `0`,
  border color black, and radius `0`; border width and radius are nonnegative.
- A relation style accepts exactly `color`, `width`, and `alpha`. Color defaults to
  theme `black`; width and alpha default to `1`.
- Rectangle and text stroke styles accept exactly the keys in the table below. Their
  advanced fields are not accepted by relation styles.
...
### Accepted rectangle and text stroke style keys
...
The exact error wording is not public contract, but a host application must be able to
distinguish invalid input from asset failure, missing runtime target, and a destroyed
lifecycle.
