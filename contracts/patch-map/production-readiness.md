...
The catalog count is not a completion claim. PatchMap advances only through these
...
| release-verified | required platform/security/performance/memory gates pass and review approves evidence |
...
- schema roundtrip, package inventory/SBOM, and documentation example tests.
...
| host integration | packed PatchMap in Dashboard/Editor/Report adapter for all CSM cases |
...
Renderer context/device loss is a classified lifecycle event. PatchMap either rebuilds
...
Dashboard/Editor/Report examples, host/engine responsibility guides,
