...
```sh
node scripts/verification/generate-patch-map-catalog-evidence.mjs --pending-review --output-dir .perf-results/contract-catalog-current
npm run verify:contract:gates
npx vitest run --maxWorkers=2 tests/patch-map/contract-package-integration.test.ts tests/patch-map/contract-security-operations.test.ts tests/patch-map/accessibility-product.test.ts tests/patch-map/engine-operations.test.ts tests/patch-map/page-lifecycle.test.ts
```
