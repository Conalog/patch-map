...
- `options` (optional, object)
  - `padding` (optional, number \| { x?: number, y?: number } \| { top?: number, right?: number, bottom?: number, left?: number }) - fit 대상 주변에 추가 여백을 적용합니다. `fit()`은 기본적으로 상하좌우 `16`의 고정 패딩을 사용하고, 전달한 값은 지정한 방향만 덮어씁니다.
```js
// 전체 캔버스 객체를 기준으로 fit
patchmap.fit()
...
// 일부 방향의 패딩을 직접 지정하여 fit (미지정 방향은 기본값 16px 유지)
