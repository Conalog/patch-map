...
const SUMMARY_PATH = path.join(
  ROOT,
  'performance/patch-map/results/contract-performance.json',
);
const LATEST_RAW_PATH = path.join(
  ROOT,
  'performance/patch-map/results/contract-performance-raw-latest.json',
);
...
async function main() {
  ...
  const rawPath = path.join(
    ...
    path.basename(summary.rawArtifact.path),
  );
