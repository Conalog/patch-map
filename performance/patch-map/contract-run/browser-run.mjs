...
export async function executeContractBrowserRun(root, options) {
  ...
  try {
    ...
    return {
      ...
      environment: {
        ...
        windowsNative: options.nativeWindows ? 'measured-candidate-unreviewed' : 'pending',
...
async function startServer(root, explicitUrl) {
  ...
  try {
    ...
    return {
      pageUrl: new URL('performance/patch-map/contract-index.html', baseUrl).href,
      close: () => server.close(),
    };
  } catch (error) {
    await server.close().catch(() => undefined);
