...
const SUMMARY_PATH = path.join(
  ROOT,
  'contracts/evidence/qualification/contract-performance.json',
);
const QUALIFICATION_ROOT = path.join(ROOT, 'contracts/evidence/qualification');
const MANIFEST_PATH = path.join(
  ROOT,
  'contracts/evidence/catalog-evidence-manifest.v1.json',
);
...
async function main() {
  ...
  const rawPath = path.resolve(ROOT, summary.rawArtifact.path);
  assert(
    path.dirname(rawPath) === QUALIFICATION_ROOT,
    'raw artifact stays in qualification evidence',
  );
  const [rawText, manifestText] = await Promise.all([
